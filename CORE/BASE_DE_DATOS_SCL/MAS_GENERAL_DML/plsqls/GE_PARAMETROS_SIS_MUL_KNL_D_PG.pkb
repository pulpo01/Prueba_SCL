CREATE OR REPLACE PACKAGE BODY GE_PARAMETROS_SIS_MUL_KNL_D_PG
IS
  TIPO_PARAMETRO_SISTEMA CONSTANT NUMBER := 1;
  ...
  FUNCTION GE_ELIMINAR_PADRE_FN
	...
	<param nom="EN_codigo_modulo" Tipo="STRING>Codigo del modulo</param>
	</Entrada>
	<Salida>
	</Salida>
	</Parametros>
  ...
  IS
	  ...
	  EXCEPTION
	      WHEN ERROR_PADRE_CON_HIJOS THEN
	              RAISE_APPLICATION_ERROR(-20102, 'Error en GE_PARAMETROS_MULTIPLES_D_PG 20102: El registro a eliminar posee dependencia con otros registros.');
	      WHEN OTHERS THEN
		      RAISE_APPLICATION_ERROR(-20100, 'Error en GE_PARAMETROS_MULTIPLES_D_PG ' || to_char(SQLCODE) || ': ' || SQLERRM);
  ...
  IS
	  ...
	  EXCEPTION
	      WHEN ERROR_PADRE_CON_HIJOS THEN
	              RAISE_APPLICATION_ERROR(-20102, 'Error en GE_PARAMETROS_SISTEMA_MUL_D_PG 20102: El registro a eliminar posee dependencia con otros registros.');
	      WHEN OTHERS THEN
		      RAISE_APPLICATION_ERROR(-20100, 'Error en GE_PARAMETROS_SISTEMA_MUL_D_PG ' || to_char(SQLCODE) || ': ' || SQLERRM);
  END GE_ELIMINAR_HIJO_FN;

 END GE_PARAMETROS_SIS_MUL_KNL_D_PG;
/
SHOW ERRORS
