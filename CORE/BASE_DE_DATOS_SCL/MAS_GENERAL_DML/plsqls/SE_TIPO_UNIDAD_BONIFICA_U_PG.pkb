CREATE OR REPLACE PACKAGE BODY SE_TIPO_UNIDAD_BONIFICA_U_PG
IS
  TIPO_DOMINIO_MODIFICABLE CONSTANT NUMBER(1) := 2;

  FUNCTION SE_MODIFICAR_FN
               (
...
/*<Documentacion TipoDoc = "Funci¢n">
	<Elemento Nombre = "SE_AGREGAR_FN" Lenguaje="PL/SQL" Fecha="" Versi¢n"1.0.0" Dise¤ador"DAVID SUTHERLAND" Programador="JAVIER LAMAS" Ambiente="BD">
  ...
  BEGIN
     LN_registros_afectados := GE_VALORES_DOMINIOS_KNL_U_PG.GE_MODIFICAR_FN( TIPO_DOMINIO_MODIFICABLE, 'TIPO_UNIDAD_BONIFICA', EV_valor, EV_estado, EV_descripcion_valor, EV_usu_modificacion );
     ...
     RETURN LN_registros_afectados;
	  ...
	  EXCEPTION
	      WHEN OTHERS THEN
	       ...
	       RAISE_APPLICATION_ERROR(-20100, 'Error en SE_TIPO_UNIDAD_BONIFICA_U_PG ' || to_char(SQLCODE) || ': ' || SQLERRM);
  END SE_MODIFICAR_FN;


 END SE_TIPO_UNIDAD_BONIFICA_U_PG;
/
SHOW ERRORS
