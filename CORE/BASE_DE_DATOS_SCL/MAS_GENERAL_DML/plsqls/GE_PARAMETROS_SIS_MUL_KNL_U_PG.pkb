CREATE OR REPLACE PACKAGE BODY GE_PARAMETROS_SIS_MUL_KNL_U_PG
IS
	...
	LN_total NUMBER(5);
	  BEGIN
	       ...
	       WHERE  cod_parametro_padre = EN_codigo_padre
			   AND tipo_parametro = TIPO_PARAMETRO_SISTEMA_MUL
	  ...
	  EXCEPTION
	  	  WHEN ERROR_PADRE_CON_HIJOS THEN
		      RAISE_APPLICATION_ERROR(-20110, 'Error en GE_PARAMETROS_SISTEMA_MUL_U_PG 20100: No se puede modificar dominio o tipo por existir valores asociados');
		  WHEN ERROR_PARAMETROS THEN
	            RAISE_APPLICATION_ERROR(-20101, 'Error en GE_PARAMETROS_SISTEMA_MUL_U_PG 20101: Error en parametros');
	      WHEN OTHERS THEN
		      RAISE_APPLICATION_ERROR(-20100, 'Error en GE_PARAMETROS_SISTEMA_MUL_U_PG ' || to_char(SQLCODE) || ': ' || SQLERRM);
 ...
 END GE_PARAMETROS_SIS_MUL_KNL_U_PG;
/
SHOW ERRORS
