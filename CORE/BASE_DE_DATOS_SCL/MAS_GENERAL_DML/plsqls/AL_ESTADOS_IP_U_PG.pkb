CREATE OR REPLACE PACKAGE BODY AL_ESTADOS_IP_U_PG
IS
...
/*<Documentacion TipoDoc = "Funci¢n">
  ...
  BEGIN
     LN_registros_afectados := GE_VALORES_DOMINIOS_KNL_U_PG.GE_MODIFICAR_FN( TIPO_DOMINIO_MODIFICABLE, EV_cod_dominio, EV_valor, EV_estado, EV_descripcion_valor, EV_usu_modificacion );
     ...
     RETURN LN_registros_afectados;
	  ...
	  EXCEPTION
	      WHEN OTHERS THEN
	       ...
	       RAISE_APPLICATION_ERROR(-20100, 'Error en AL_ESTADOS_IP_U_PG ' || to_char(SQLCODE) || ': ' || SQLERRM);
  END GE_MODIFICAR_FN;
