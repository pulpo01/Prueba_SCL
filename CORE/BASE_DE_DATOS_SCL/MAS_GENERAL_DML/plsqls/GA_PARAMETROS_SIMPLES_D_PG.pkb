CREATE OR REPLACE PACKAGE BODY GA_PARAMETROS_SIMPLES_D_PG
IS
  ...
  FUNCTION GA_ELIMINAR_PADRE_FN
      ...
      RETURN NUMBER
        ...
        <Elemento Nombre = "GA_ELIMINAR_PADRE_FN" Lenguaje="PL/SQL" Fecha="" Versi¢n"1.0.0" Dise?ador"DAVID SUTHERLAND" Programador="JAVIER LAMAS" Ambiente="BD">
        <Retorno>NUMBER</Retorno>
        <DeGAripcion>Elimina logicamente  un parametro del tipo multiple (con hijos)/DeGAripci¢n>
        <Parametros>
        <Entrada>
  ...
  IS
      LN_registros_afectados NUMBER;
          ...
          EXCEPTION
              WHEN OTHERS THEN
                      RAISE_APPLICATION_ERROR(-20100, 'Error en GA_PARAMETROS_SIMPLES_D_PG ' || to_char(SQLCODE) || ': ' || SQLERRM);
  END GA_ELIMINAR_PADRE_FN;
