CREATE OR REPLACE PACKAGE BODY GE_PARAMETROS_SIM_KNL_I_PG
IS
  TIPO_PARAMETRO_MODIFICABLE_SIM CONSTANT VARCHAR(2) := 'MS';
  ...
  FUNCTION GE_AGREGAR_PADRE_FN
	...
	<Descripcion>Agrega un parametro del tipo simple </Descripci¢n>>
	<Parametros>
	<Entrada>
	...
	<param nom="EV_tipo_param" Tipo="STRING">Tipo de parametro</param>
	</Entrada>
	<Salida>
	</Salida>
	</Parametros>
  ...
  IS
	  BEGIN
	     ...
	     END IF;
			   ...
			   ,EV_descripcion
			   ,EV_codigo_modulo
			   ,TIPO_PARAMETRO_MODIFICABLE_SIM
			   ,EV_cod_dominio
                           ,NULL
			   ,NULL
			   ,NULL
	  ...
	  EXCEPTION
          WHEN ERROR_PARAMETROS THEN
	            RAISE_APPLICATION_ERROR(-20101, 'Error en GE_PARAMETROS_SIMPLES_I_PG 20101: Error en parametros');
	      WHEN OTHERS THEN
		      RAISE_APPLICATION_ERROR(-20100, 'Error en GE_PARAMETROS_SIMPLES_I_PG ' || to_char(SQLCODE) || ': ' || SQLERRM);
     ...
     LN_total_registros NUMBER(5);
	  BEGIN
		...
		IF EV_cod_operadora IS NULL THEN
			...
			AND tipo_parametro = TIPO_PARAMETRO_MODIFICABLE_SIM
			AND ind_hijos = 0;
		ELSE
			SELECT COUNT(1)
			INTO LN_total_registros
	  ...
	  EXCEPTION
	      WHEN ERROR_REGISTRO_CON_DETALLE THEN
		        RAISE_APPLICATION_ERROR(-20101, 'Error en GE_PARAMETROS_SIMPLES_I_PG 20101: Error registro ya tiene detalle');
	      WHEN ERROR_PARAMETROS THEN
	            RAISE_APPLICATION_ERROR(-20101, 'Error en GE_PARAMETROS_SIMPLES_I_PG 20101: Error en parametros');
	      WHEN OTHERS THEN
		      RAISE_APPLICATION_ERROR(-20100, 'Error en GE_PARAMETROS_SIMPLES_I_PG ' || to_char(SQLCODE) || ': ' || SQLERRM);
  END GE_AGREGAR_HIJO_FN;
 END GE_PARAMETROS_SIM_KNL_I_PG;
/
SHOW ERRORS
