CREATE OR REPLACE PACKAGE BODY VE_PARAMETROS_SIMPLES_U_PG
IS

  CODIGO_MODULO CONSTANT VARCHAR2(2) := 'VE';

  FUNCTION VE_ACTUALIZAR_PADRE_FN
  ...
  IS
      LN_registros_afectados NUMBER;
          BEGIN
            ...
            LN_registros_afectados := GE_PARAMETROS_SIM_KNL_U_PG.GE_ACTUALIZAR_PADRE_FN
               (EN_codigo_padre
                            ,EV_descripcion
                                ,EV_cod_dominio
                                ,EV_tipo_valor
  ...
  END VE_ACTUALIZAR_PADRE_FN;

  FUNCTION VE_ACTUALIZAR_HIJO_FN
      ...
      RETURN NUMBER
         /*<Documentacion TipoDoc = "Funci¢n">
        <Elemento Nombre = "VE_ACTUALIZAR_HIJO_FN" Lenguaje="PL/SQL" Fecha="" Versi¢n"1.0.0" Dise?ador"DAVID SUTHERLAND" Programador="JAVIER LAMAS" Ambiente="BD">
        <Retorno>NUMBER</Retorno>
        <Descripcion>Actualiza informacion de parametros vigentes del tipo multiple (con hijos)/Descripci¢n>
        <Parametros>
        <Entrada>
  ...
  IS
      LN_registros_afectados NUMBER;
         ...
         RETURN LN_registros_afectados;
          ...
          EXCEPTION
              WHEN OTHERS THEN
                      RAISE_APPLICATION_ERROR(-20100, 'Error en VE_PARAMETROS_SIMPLES_U_PG ' || TO_CHAR(SQLCODE) || ': ' || SQLERRM);
  END VE_ACTUALIZAR_HIJO_FN;


END VE_PARAMETROS_SIMPLES_U_PG;
/
SHOW ERRORS
