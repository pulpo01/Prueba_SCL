CREATE OR REPLACE PACKAGE BODY SC_PARAMETROS_SIMPLES_U_PG
IS

  CODIGO_MODULO CONSTANT VARCHAR2(2) := 'SC';

  FUNCTION SC_ACTUALIZAR_PADRE_FN
  ...
  END SC_ACTUALIZAR_PADRE_FN;

  FUNCTION SC_ACTUALIZAR_HIJO_FN
      ...
      RETURN NUMBER
	 /*<Documentacion TipoDoc = "Funci¢n">
	<Elemento Nombre = "SC_ACTUALIZAR_HIJO_FN" Lenguaje="PL/SQL" Fecha="" Versi¢n"1.0.0" Dise¤ador"DAVID SUTHERLAND" Programador="JAVIER LAMAS" Ambiente="BD">
	<Retorno>NUMBER</Retorno>
	<Descripcion>Actualiza informacion de parametros vigentes del tipo multiple (con hijos)/Descripci¢n>
	<Parametros>
	<Entrada>
  ...
  IS
	  ...
	  EXCEPTION
	      WHEN OTHERS THEN
		      RAISE_APPLICATION_ERROR(-20100, 'Error en SC_PARAMETROS_SIMPLES_U_PG ' || to_char(SQLCODE) || ': ' || SQLERRM);
  END SC_ACTUALIZAR_HIJO_FN;


 END SC_PARAMETROS_SIMPLES_U_PG;
/
SHOW ERRORS
