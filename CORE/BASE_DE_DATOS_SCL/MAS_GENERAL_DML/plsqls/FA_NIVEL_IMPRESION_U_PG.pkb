CREATE OR REPLACE PACKAGE BODY FA_NIVEL_IMPRESION_U_PG
IS
...
/*<Documentacion TipoDoc = "Funci¢n">
  ...
  BEGIN
     LN_registros_afectados := GE_VALORES_DOMINIOS_KNL_U_PG.GE_MODIFICAR_FN( TIPO_DOMINIO_MODIFICABLE, 'NIVEL_IMPRESION', EV_valor, EV_estado, EV_descripcion_valor, EV_usu_modificacion );
     ...
     RETURN LN_registros_afectados;
	  ...
	  EXCEPTION
	      WHEN OTHERS THEN
	       ...
	       RAISE_APPLICATION_ERROR(-20100, 'Error en FA_NIVEL_IMPRESION_U_PG ' || to_char(SQLCODE) || ': ' || SQLERRM);
  END FA_MODIFICAR_FN;


 END FA_NIVEL_IMPRESION_U_PG;
/
SHOW ERRORS
