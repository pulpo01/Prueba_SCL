CREATE OR REPLACE PACKAGE BODY SE_CATEGORIAS_DESTINO_U_PG
IS
...
/*<Documentacion TipoDoc = "Funci¢n">
  ...
  BEGIN
     LN_registros_afectados := GE_VALORES_DOMINIOS_KNL_U_PG.GE_MODIFICAR_FN( TIPO_DOMINIO_MODIFICABLE, 'CATEGORIAS_DESTINO', EV_valor, EV_estado, EV_descripcion_valor, EV_usu_modificacion );
     ...
     RETURN LN_registros_afectados;
	  ...
	  EXCEPTION
	      WHEN OTHERS THEN
	       ...
	       RAISE_APPLICATION_ERROR(-20100, 'Error en SE_CATEGORIAS_DESTINO_U_PG ' || to_char(SQLCODE) || ': ' || SQLERRM);
  END SE_MODIFICAR_FN;


 END SE_CATEGORIAS_DESTINO_U_PG;
/
SHOW ERRORS
