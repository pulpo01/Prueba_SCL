CREATE OR REPLACE PACKAGE BODY SE_TIPO_ACCION_U_PG
IS
...
/*<Documentacion TipoDoc = "Funci¢n">
  ...
  BEGIN
     LN_registros_afectados := GE_VALORES_DOMINIOS_KNL_U_PG.GE_MODIFICAR_FN( TIPO_DOMINIO_MODIFICABLE, 'TIPO_ACCION', EV_valor, EV_estado, EV_descripcion_valor, EV_usu_modificacion );
     ...
     RETURN LN_registros_afectados;
	  ...
	  EXCEPTION
	      WHEN OTHERS THEN
	       ...
	       RAISE_APPLICATION_ERROR(-20100, 'Error en SE_TIPO_ACCION_U_PG ' || to_char(SQLCODE) || ': ' || SQLERRM);
  END SE_MODIFICAR_FN;


 END SE_TIPO_ACCION_U_PG;
/
SHOW ERRORS
