CREATE OR REPLACE PACKAGE BODY FA_PARAMETROS_SIMPLES_U_PG
IS

  CODIGO_MODULO CONSTANT VARCHAR2(2) := 'FA';

  FUNCTION FA_ACTUALIZAR_PADRE_FN
  ...
  RETURN NUMBER
        ...
        <Elemento Nombre = "FA_ACTUALIZAR_PADRE_FN" Lenguaje="PL/SQL" Fecha="" Versi¢n"1.0.0" Dise?ador"DAVID SUTHERLAND" Programador="JAVIER LAMAS" Ambiente="BD">
  ...
  IS
      LN_registros_afectados NUMBER;
          ...
          EXCEPTION
              WHEN OTHERS THEN
                      RAISE_APPLICATION_ERROR(-20100, 'Error en FA_PARAMETROS_SIMPLES_U_PG ' || TO_CHAR(SQLCODE) || ': ' || SQLERRM);
  END FA_ACTUALIZAR_PADRE_FN;

  FUNCTION FA_ACTUALIZAR_HIJO_FN
      ...
      RETURN NUMBER
         /*<Documentacion TipoDoc = "Funci¢n">
        <Elemento Nombre = "FA_ACTUALIZAR_HIJO_FN" Lenguaje="PL/SQL" Fecha="" Versi¢n"1.0.0" Dise?ador"DAVID SUTHERLAND" Programador="JAVIER LAMAS" Ambiente="BD">
        <Retorno>NUMBER</Retorno>
        <Descripcion>Actualiza informacion de parametros vigentes del tipo multiple (con hijos)/Descripci¢n>
        <Parametros>
        <Entrada>
  ...
  IS
      LN_registros_afectados NUMBER;
          BEGIN
           ...
           LN_registros_afectados := GE_PARAMETROS_SIM_KNL_U_PG.GE_ACTUALIZAR_HIJO_FN
               (EN_codigo_padre
                ,EN_codigo_hijo
                ,EN_valor_numerico
                ,EV_valor_texto
         ...
         RETURN LN_registros_afectados;
          ...
          EXCEPTION
              WHEN OTHERS THEN
                      RAISE_APPLICATION_ERROR(-20100, 'Error en FA_PARAMETROS_SIMPLES_U_PG ' || TO_CHAR(SQLCODE) || ': ' || SQLERRM);
  END FA_ACTUALIZAR_HIJO_FN;


 END FA_PARAMETROS_SIMPLES_U_PG;
/
SHOW ERRORS
