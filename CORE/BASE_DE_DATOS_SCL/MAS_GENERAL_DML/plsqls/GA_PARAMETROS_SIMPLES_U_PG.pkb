CREATE OR REPLACE PACKAGE BODY GA_PARAMETROS_SIMPLES_U_PG
IS
  ...
  FUNCTION GA_ACTUALIZAR_PADRE_FN
               (EN_codigo_padre IN ge_parametros_td.cod_parametro%TYPE
                            ,EV_deGAripcion IN ge_parametros_td.des_parametro%TYPE
  ...
  RETURN NUMBER
        ...
        <DeGAripcion>Actualiza informacion de parametros vigentes del tipo multiple (con hijos)/DeGAripci¢n>
        <Parametros>
        <Entrada>
        <param nom="EN_codigo_padre" Tipo="NUMBER">Codigo de parametro padre para ser asociado al registro</param>
        <param nom="EV_deGAripcion" Tipo="STRING">Codigo de modulo que referencia el parametro</param>
  ...
  IS
      LN_registros_afectados NUMBER;
         ...
         RETURN LN_registros_afectados;
          ...
          EXCEPTION
              WHEN OTHERS THEN
                      RAISE_APPLICATION_ERROR(-20100, 'Error en GA_PARAMETROS_SIMPLES_U_PG ' || to_char(SQLCODE) || ': ' || SQLERRM);
  END GA_ACTUALIZAR_HIJO_FN;


 END GA_PARAMETROS_SIMPLES_U_PG;
/
SHOW ERRORS
