CREATE OR REPLACE PACKAGE BODY IC_PARAMETROS_MULTIPLES_U_PG
IS
  ...
  RETURN NUMBER
        ...
        <Elemento Nombre = "IC_ACTUALIZAR_PADRE_FN" Lenguaje="PL/SQL" Fecha="" Versi¢n"1.0.0" Dise?ador"DAVID SUTHERLAND" Programador="JAVIER LAMAS" Ambiente="BD">
  ...
  IS
      LN_registros_afectados NUMBER;
          ...
          EXCEPTION
              WHEN OTHERS THEN
                      RAISE_APPLICATION_ERROR(-20100, 'Error en IC_PARAMETROS_MULTIPLES_U_PG ' || TO_CHAR(SQLCODE) || ': ' || SQLERRM);
  END IC_ACTUALIZAR_PADRE_FN;
  ...
  FUNCTION IC_ACTUALIZAR_HIJO_FN
      ...
      RETURN NUMBER
         /*<Documentacion TipoDoc = "Funci¢n">
        <Elemento Nombre = "IC_ACTUALIZAR_HIJO_FN" Lenguaje="PL/SQL" Fecha="" Versi¢n"1.0.0" Dise?ador"DAVID SUTHERLAND" Programador="JAVIER LAMAS" Ambiente="BD">
        <Retorno>NUMBER</Retorno>
        <Descripcion>Actualiza informacion de parametros vigentes del tipo multiple (con hijos)/Descripci¢n>
        <Parametros>
        <Entrada>
