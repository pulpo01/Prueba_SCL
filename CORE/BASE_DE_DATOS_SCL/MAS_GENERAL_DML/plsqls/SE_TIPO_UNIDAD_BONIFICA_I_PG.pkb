CREATE OR REPLACE PACKAGE BODY SE_TIPO_UNIDAD_BONIFICA_I_PG
IS
...
/*<Documentacion TipoDoc = "Funcion">
  ...
  BEGIN
     LN_registros_afectados := GE_VALORES_DOMINIOS_KNL_I_PG.GE_AGREGAR_FN( TIPO_DOMINIO_MODIFICABLE, 'TIPO_UNIDAD_BONIFICA', EV_valor, EV_estado, EV_descripcion_valor, EV_usu_creacion );
     ...
     RETURN LN_registros_afectados;
	  ...
	  EXCEPTION
	      WHEN OTHERS THEN
	       ...
	       RAISE_APPLICATION_ERROR(-20100, 'Error en SE_TIPO_UNIDAD_BONIFICA_I_PG ' || to_char(SQLCODE) || ': ' || SQLERRM);
  END SE_AGREGAR_FN;
