CREATE OR REPLACE PACKAGE BODY AL_PARAMETROS_SIMPLES_I_PG
IS

  CODIGO_MODULO CONSTANT VARCHAR2(2) := 'AL';

  FUNCTION AL_AGREGAR_PADRE_FN
      ...
      RETURN NUMBER
        ...
        <Elemento Nombre = "AL_AGREGAR_PADRE_FN" Lenguaje="PL/SQL" Fecha="" Versi¢n"1.0.0" Dise?ador"DAVID SUTHERLAND" Programador="JAVIER LAMAS" Ambiente="BD">
  ...
  IS
      LN_registros_afectados NUMBER;
          BEGIN
              ...
              LN_registros_afectados := GE_PARAMETROS_SIM_KNL_I_PG.GE_AGREGAR_PADRE_FN
               (EN_codigo
                ,EV_nombre
                                ,EV_descripcion
                                ,EV_cod_dominio
          ...
          EXCEPTION
              WHEN OTHERS THEN
                      RAISE_APPLICATION_ERROR(-20100, 'Error en AL_PARAMETROS_SIMPLES_I_PG ' || TO_CHAR(SQLCODE) || ': ' || SQLERRM);
  END AL_AGREGAR_PADRE_FN;


 FUNCTION AL_AGREGAR_HIJO_FN
      ...
      RETURN NUMBER
         /*<Documentacion TipoDoc = "Funci¢n">
        <Elemento Nombre = "AL_AGREGAR_HIJO_FN" Lenguaje="PL/SQL" Fecha="" Versi¢n"1.0.0" Dise?ador"DAVID SUTHERLAND" Programador="JAVIER LAMAS" Ambiente="BD">
        <Retorno>NUMBER</Retorno>
        <Descripcion>Agrega un parametro del tipo multiple (con hijos)/Descripci¢n>
        <Parametros>
        <Entrada>
  ...
  LN_registros_afectados NUMBER;
          BEGIN
                  LN_registros_afectados := GE_PARAMETROS_SIM_KNL_I_PG.GE_AGREGAR_HIJO_FN
               (EN_codigo_padre
                        ,EN_codigo_hijo
                        ,EN_valor_numerico
                        ,EV_valor_texto
          ...
          EXCEPTION
              WHEN OTHERS THEN
                      RAISE_APPLICATION_ERROR(-20100, 'Error en AL_PARAMETROS_SIMPLES_I_PG ' || TO_CHAR(SQLCODE) || ': ' || SQLERRM);
  END AL_AGREGAR_HIJO_FN;
 END AL_PARAMETROS_SIMPLES_I_PG;
/
SHOW ERRORS
