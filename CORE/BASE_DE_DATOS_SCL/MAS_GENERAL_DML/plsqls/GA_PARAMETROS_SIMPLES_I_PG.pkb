CREATE OR REPLACE PACKAGE BODY GA_PARAMETROS_SIMPLES_I_PG
IS

  CODIGO_MODULO CONSTANT VARCHAR2(2) := 'SC';

  FUNCTION GA_AGREGAR_PADRE_FN
      ...
      RETURN NUMBER
        ...
        <Elemento Nombre = "GA_AGREGAR_PADRE_FN" Lenguaje="PL/SQL" Fecha="" Versi¢n"1.0.0" Dise?ador"DAVID SUTHERLAND" Programador="JAVIER LAMAS" Ambiente="BD">
  ...
  IS
      LN_registros_afectados NUMBER;
          ...
          EXCEPTION
              WHEN OTHERS THEN
                      RAISE_APPLICATION_ERROR(-20100, 'Error en GA_PARAMETROS_SIMPLES_I_PG ' || to_char(SQLCODE) || ': ' || SQLERRM);
  END GA_AGREGAR_PADRE_FN;


 FUNCTION GA_AGREGAR_HIJO_FN
      ...
      RETURN NUMBER
         /*<Documentacion TipoDoc = "Funci¢n">
        <Elemento Nombre = "GA_AGREGAR_HIJO_FN" Lenguaje="PL/SQL" Fecha="" Versi¢n"1.0.0" Dise?ador"DAVID SUTHERLAND" Programador="JAVIER LAMAS" Ambiente="BD">
        <Retorno>NUMBER</Retorno>
        <Descripcion>Agrega un parametro del tipo multiple (con hijos)/Descripci¢n>
        <Parametros>
        <Entrada>
  ...
  LN_registros_afectados NUMBER;
          ...
          EXCEPTION
              WHEN OTHERS THEN
                      RAISE_APPLICATION_ERROR(-20100, 'Error en GA_PARAMETROS_SIMPLES_I_PG ' || to_char(SQLCODE) || ': ' || SQLERRM);
  END GA_AGREGAR_HIJO_FN;
 END GA_PARAMETROS_SIMPLES_I_PG;
/
SHOW ERRORS
