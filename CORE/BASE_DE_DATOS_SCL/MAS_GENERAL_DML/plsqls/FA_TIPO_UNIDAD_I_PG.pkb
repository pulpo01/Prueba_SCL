CREATE OR REPLACE PACKAGE BODY FA_TIPO_UNIDAD_I_PG
IS
...
/*<Documentacion TipoDoc = "Funci¢n">
  ...
  BEGIN
     LN_registros_afectados := GE_VALORES_DOMINIOS_KNL_I_PG.GE_AGREGAR_FN( TIPO_DOMINIO_MODIFICABLE, 'TIPO_UNIDAD', EV_valor, EV_estado, EV_descripcion_valor, EV_usu_creacion );
     ...
     RETURN LN_registros_afectados;
	  ...
	  EXCEPTION
	      WHEN OTHERS THEN
	       ...
	       RAISE_APPLICATION_ERROR(-20100, 'Error en FA_TIPO_UNIDAD_I_PG ' || to_char(SQLCODE) || ': ' || SQLERRM);
  END FA_AGREGAR_FN;


 END FA_TIPO_UNIDAD_I_PG;
/
SHOW ERRORS
