CREATE OR REPLACE PACKAGE BODY GA_CAUSAS_DESCUENTOS_I_PG
IS
...
/*<Documentacion TipoDoc = "Funci¢n">
	<Elemento Nombre = "GA_AGREGAR_FN" Lenguaje="PL/SQL" Fecha="" Versi¢n"1.0.0" Dise¤ador"DAVID SUTHERLAND" Programador="JAVIER LAMAS" Ambiente="BD">
  ...
  BEGIN
     LN_registros_afectados := GE_VALORES_DOMINIOS_KNL_I_PG.GE_AGREGAR_FN( TIPO_DOMINIO_MODIFICABLE, 'CAUSAS_DESCUENTOS', EV_valor, EV_estado, EV_descripcion_valor, EV_usu_creacion );
     ...
     RETURN LN_registros_afectados;
	  ...
	  EXCEPTION
	      WHEN OTHERS THEN
	       ...
	       RAISE_APPLICATION_ERROR(-20100, 'Error en GA_CAUSAS_DESCUENTOS_I_PG ' || to_char(SQLCODE) || ': ' || SQLERRM);
  END GA_AGREGAR_FN;
