CREATE OR REPLACE PACKAGE BODY SC_PARAMETROS_MULTIPLES_U_PG
IS
  ...
  IS
	  ...
	  EXCEPTION
	      WHEN OTHERS THEN
		      RAISE_APPLICATION_ERROR(-20100, 'Error en GE_PARAMETROS_MULTIPLES_U_PG ' || to_char(SQLCODE) || ': ' || SQLERRM);
  ...
  IS
	  ...
	  EXCEPTION
	      WHEN OTHERS THEN
		      RAISE_APPLICATION_ERROR(-20100, 'Error en SC_PARAMETROS_MULTIPLES_U_PG ' || to_char(SQLCODE) || ': ' || SQLERRM);
  END SC_ACTUALIZAR_HIJO_FN;


 END SC_PARAMETROS_MULTIPLES_U_PG;
/
SHOW ERRORS
