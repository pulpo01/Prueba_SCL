CREATE OR REPLACE PACKAGE BODY GE_TIPO_VENDEDOR_I_PG
IS
...
/*<Documentacion TipoDoc = "Funcion">
  ...
  BEGIN
     LN_registros_afectados := GE_VALORES_DOMINIOS_KNL_I_PG.GE_AGREGAR_FN( TIPO_DOMINIO_MODIFICABLE, 'TIPO_VENDEDOR', EV_valor, EV_estado, EV_descripcion_valor, EV_usu_creacion );
     ...
     RETURN LN_registros_afectados;
          ...
          EXCEPTION
              WHEN OTHERS THEN
               ...
               RAISE_APPLICATION_ERROR(-20100, 'Error en GE_TIPO_VENDEDOR_I_PG ' || to_char(SQLCODE) || ': ' || SQLERRM);
  END GE_AGREGAR_FN;


 END GE_TIPO_VENDEDOR_I_PG;
/
SHOW ERRORS
