CREATE OR REPLACE PACKAGE BODY FA_TIPO_GRUPO_D_PG
IS
  ...
  BEGIN
     LN_registros_afectados := GE_VALORES_DOMINIOS_KNL_D_PG.GE_ELIMINAR_FN( TIPO_DOMINIO_MODIFICABLE, 'TIPO_GRUPO', EV_valor );
     ...
     RETURN LN_registros_afectados;
	  ...
	  EXCEPTION
	      WHEN OTHERS THEN
	       ...
	       RAISE_APPLICATION_ERROR(-20100, 'Error en FA_TIPO_GRUPO_D_PG ' || to_char(SQLCODE) || ': ' || SQLERRM);
  END FA_ELIMINAR_FN;


 END FA_TIPO_GRUPO_D_PG;
/
SHOW ERRORS
