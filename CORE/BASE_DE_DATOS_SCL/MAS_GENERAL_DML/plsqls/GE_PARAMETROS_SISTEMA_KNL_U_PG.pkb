CREATE OR REPLACE PACKAGE BODY GE_PARAMETROS_SISTEMA_KNL_U_PG
IS
  ...
  IS
	  ...
	  EXCEPTION
	     WHEN ERROR_REGISTRO_CON_DETALLE THEN
		        RAISE_APPLICATION_ERROR(-20101, 'Error en GE_PARAMETROS_SISTEMA_U_PG 20101: Error registro ya existe');
	      WHEN ERROR_PARAMETROS THEN
	            RAISE_APPLICATION_ERROR(-20101, 'Error en GE_PARAMETROS_SISTEMA_U_PG 20101: Error en parametros');
	      WHEN OTHERS THEN
		      RAISE_APPLICATION_ERROR(-20100, 'Error en GE_PARAMETROS_SISTEMA_U_PG ' || to_char(SQLCODE) || ': ' || SQLERRM);
  END GE_ACTUALIZAR_HIJO_FN;


 END GE_PARAMETROS_SISTEMA_KNL_U_PG;
/
SHOW ERRORS
