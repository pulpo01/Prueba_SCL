CREATE OR REPLACE PACKAGE BODY PF_CANAL_DISTRIBUCION_I_PG
IS
  TIPO_DOMINIO_MODIFICABLE CONSTANT NUMBER(1) := 2;

  FUNCTION PF_AGREGAR_FN
               (
...
/*<Documentacion TipoDoc = "Funcion">
	<Elemento Nombre = "PF_AGREGAR_FN" Lenguaje="PL/SQL" Fecha="" Versi¢n"1.0.0" Dise¤ador"DAVID SUTHERLAND" Programador="JAVIER LAMAS" Ambiente="BD">
  ...
  BEGIN
     LN_registros_afectados := GE_VALORES_DOMINIOS_KNL_I_PG.GE_AGREGAR_FN( TIPO_DOMINIO_MODIFICABLE, 'CANAL_DISTRIBUCION', EV_valor, EV_estado, EV_descripcion_valor, EV_usu_creacion );
     ...
     RETURN LN_registros_afectados;
	  ...
	  EXCEPTION
	      WHEN OTHERS THEN
	       ...
	       RAISE_APPLICATION_ERROR(-20100, 'Error en PF_CANAL_DISTRIBUCION_I_PG ' || to_char(SQLCODE) || ': ' || SQLERRM);
  END PF_AGREGAR_FN;


 END PF_CANAL_DISTRIBUCION_I_PG;
/
SHOW ERRORS
