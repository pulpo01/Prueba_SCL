CREATE OR REPLACE PACKAGE BODY GE_PARAMETROS_MUL_KNL_U_PG
IS
  TIPO_PARAMETRO_MODIFICABLE CONSTANT VARCHAR2(2) := 'MM';
	...
	LN_total NUMBER(5);
	  ...
	  EXCEPTION
	  	  WHEN ERROR_PADRE_CON_HIJOS THEN
		      RAISE_APPLICATION_ERROR(-20110, 'Error en GE_PARAMETROS_MULTIPLES_U_PG 20100: No se puede modificar dominio o tipo por existir valores asociados');
		  WHEN ERROR_PARAMETROS THEN
	            RAISE_APPLICATION_ERROR(-20101, 'Error en GE_PARAMETROS_MULTIPLES_U_PG 20101: Error en parametros');
	      WHEN OTHERS THEN
 ...
 END GE_PARAMETROS_MUL_KNL_U_PG;
/
SHOW ERRORS
