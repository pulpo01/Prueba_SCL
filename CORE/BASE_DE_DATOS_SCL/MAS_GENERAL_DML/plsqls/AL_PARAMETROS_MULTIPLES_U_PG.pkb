CREATE OR REPLACE PACKAGE BODY AL_PARAMETROS_MULTIPLES_U_PG
IS

  CODIGO_MODULO CONSTANT VARCHAR2(2) := 'AL';

  FUNCTION AL_ACTUALIZAR_PADRE_FN
  ...
  RETURN NUMBER
        ...
        <Elemento Nombre = "AL_ACTUALIZAR_PADRE_FN" Lenguaje="PL/SQL" Fecha="" Versi¢n"1.0.0" Dise?ador"DAVID SUTHERLAND" Programador="JAVIER LAMAS" Ambiente="BD">
  ...
  IS
      LN_registros_afectados NUMBER;
          ...
          EXCEPTION
              WHEN OTHERS THEN
                      RAISE_APPLICATION_ERROR(-20100, 'Error en AL_PARAMETROS_MULTIPLES_U_PG ' || TO_CHAR(SQLCODE) || ': ' || SQLERRM);
  END AL_ACTUALIZAR_PADRE_FN;

  FUNCTION AL_ACTUALIZAR_HIJO_FN
      ...
      RETURN NUMBER
         /*<Documentacion TipoDoc = "Funci¢n">
        <Elemento Nombre = "AL_ACTUALIZAR_HIJO_FN" Lenguaje="PL/SQL" Fecha="" Versi¢n"1.0.0" Dise?ador"DAVID SUTHERLAND" Programador="JAVIER LAMAS" Ambiente="BD">
        <Retorno>NUMBER</Retorno>
        <Descripcion>Actualiza informacion de parametros vigentes del tipo multiple (con hijos)/Descripci¢n>
        <Parametros>
        <Entrada>
      ...
      LN_registros_afectados NUMBER;
         ...
         RETURN LN_registros_afectados;
          ...
          EXCEPTION
              WHEN OTHERS THEN
                      RAISE_APPLICATION_ERROR(-20100, 'Error en AL_PARAMETROS_MULTIPLES_U_PG ' || TO_CHAR(SQLCODE) || ': ' || SQLERRM);
  END AL_ACTUALIZAR_HIJO_FN;


 END AL_PARAMETROS_MULTIPLES_U_PG;
/
SHOW ERRORS
