CREATE OR REPLACE PACKAGE BODY VE_PARAMETROS_SIMPLES_I_PG
IS

  CODIGO_MODULO CONSTANT VARCHAR2(2) := 'VE';
  ...
  FUNCTION VE_AGREGAR_PADRE_FN
      ...
      RETURN NUMBER
        ...
        <Elemento Nombre = "VE_AGREGAR_PADRE_FN" Lenguaje="PL/SQL" Fecha="" Versi¢n"1.0.0" Dise?ador"DAVID SUTHERLAND" Programador="JAVIER LAMAS" Ambiente="BD">
  ...
  IS
      LN_registros_afectados NUMBER;
          ...
          EXCEPTION
              WHEN OTHERS THEN
                      RAISE_APPLICATION_ERROR(-20100, 'Error en VE_PARAMETROS_SIMPLES_I_PG ' || TO_CHAR(SQLCODE) || ': ' || SQLERRM);
  END VE_AGREGAR_PADRE_FN;
 ...
 FUNCTION VE_AGREGAR_HIJO_FN
      ...
      RETURN NUMBER
         /*<Documentacion TipoDoc = "Funci¢n">
        <Elemento Nombre = "VE_AGREGAR_HIJO_FN" Lenguaje="PL/SQL" Fecha="" Versi¢n"1.0.0" Dise?ador"DAVID SUTHERLAND" Programador="JAVIER LAMAS" Ambiente="BD">
        <Retorno>NUMBER</Retorno>
        <Descripcion>Agrega un parametro del tipo multiple (con hijos)/Descripci¢n>
        <Parametros>
        <Entrada>
