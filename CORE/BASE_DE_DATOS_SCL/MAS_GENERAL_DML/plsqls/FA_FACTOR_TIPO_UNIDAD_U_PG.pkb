CREATE OR REPLACE PACKAGE BODY FA_FACTOR_TIPO_UNIDAD_U_PG
IS
...
/*<Documentacion TipoDoc = "Funci¢n">
        <Elemento Nombre = "FA_AGREGAR_FN" Lenguaje="PL/SQL" Fecha="" Versi¢n"1.0.0" Dise?ador"DAVID SUTHERLAND" Programador="JAVIER LAMAS" Ambiente="BD">
  ...
  BEGIN
     LN_registros_afectados := GE_VALORES_DOMINIOS_KNL_U_PG.GE_MODIFICAR_FN( TIPO_DOMINIO_MODIFICABLE, 'FACTOR_TIPO_UNIDAD', EV_valor, EV_estado, EV_descripcion_valor, EV_usu_modificacion );
     ...
     RETURN LN_registros_afectados;
          ...
          EXCEPTION
              WHEN OTHERS THEN
               ...
               RAISE_APPLICATION_ERROR(-20100, 'Error en FA_FACTOR_TIPO_UNIDAD_U_PG ' || to_char(SQLCODE) || ': ' || SQLERRM);
  END FA_MODIFICAR_FN;
