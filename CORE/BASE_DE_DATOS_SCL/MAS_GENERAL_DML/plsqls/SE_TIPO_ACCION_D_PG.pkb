CREATE OR REPLACE PACKAGE BODY SE_TIPO_ACCION_D_PG
IS
  ...
  BEGIN
     LN_registros_afectados := GE_VALORES_DOMINIOS_KNL_D_PG.GE_ELIMINAR_FN( TIPO_DOMINIO_MODIFICABLE, 'TIPO_ACCION', EV_valor );
     ...
     RETURN LN_registros_afectados;
	  ...
	  EXCEPTION
	      WHEN OTHERS THEN
	       ...
	       RAISE_APPLICATION_ERROR(-20100, 'Error en SE_TIPO_ACCION_D_PG ' || to_char(SQLCODE) || ': ' || SQLERRM);
  END SE_ELIMINAR_FN;


 END SE_TIPO_ACCION_D_PG;
/
SHOW ERRORS
