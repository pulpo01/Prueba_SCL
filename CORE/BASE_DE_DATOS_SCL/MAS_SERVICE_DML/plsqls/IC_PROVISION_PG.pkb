CREATE OR REPLACE PACKAGE BODY IC_PROVISION_PG AS
------------------------------------------------------------
...
EXCEPTION
         WHEN NO_DATA_FOUND THEN
             ...
             SN_EVENTO := Ge_Errores_Pg.GrabarPL(SN_EVENTO, CV_cod_modulo, SQLERRM, CV_version,USER, 'IC_PROVISION_PG.IC_BUSCA_CLIENTE_PR', LV_sSql, SN_ERROR, SV_MENSAJE );
         WHEN OTHERS THEN
             SN_ERROR := SQLCODE;
             SV_MENSAJE := SQLERRM;
             SN_EVENTO := Ge_Errores_Pg.GrabarPL(SN_EVENTO, CV_cod_modulo, SQLERRM, CV_version,USER, 'IC_PROVISION_PG.IC_BUSCA_CLIENTE_PR', LV_sSql, SN_ERROR, SV_MENSAJE );
...
EXCEPTION
         WHEN NO_DATA_FOUND THEN
             ...
             SN_EVENTO := Ge_Errores_Pg.GrabarPL(SN_EVENTO,  CV_cod_modulo, SQLERRM, CV_version,USER, 'IC_PROVISION_PG.IC_BUSCA_SERVICIO_PR', LV_sSql, SN_ERROR, SV_MENSAJE );
         WHEN OTHERS THEN
             SN_ERROR := SQLCODE;
             SV_MENSAJE := SQLERRM;
             SN_EVENTO := Ge_Errores_Pg.GrabarPL(SN_EVENTO,  CV_cod_modulo, SQLERRM, CV_version,USER, 'IC_PROVISION_PG.IC_BUSCA_SERVICIO_PR', LV_sSql, SN_ERROR, SV_MENSAJE );
...
EXCEPTION
         WHEN NO_DATA_FOUND THEN
             SN_ERROR := 149;
             SV_MENSAJE := 'No se ha podido recuperar datos de Cliente para Producto Contratado';
             SN_EVENTO := Ge_Errores_Pg.GrabarPL(SN_EVENTO,  CV_cod_modulo, SQLERRM, CV_version,USER, 'IC_PROVISION_PG.IC_BUSCA_ABONADOS_PR', LV_sSql, SN_ERROR, SV_MENSAJE );
         WHEN OTHERS THEN
             SN_ERROR := SQLCODE;
             SV_MENSAJE := SQLERRM;
             SN_EVENTO := Ge_Errores_Pg.GrabarPL(SN_EVENTO,  CV_cod_modulo, SQLERRM, CV_version,USER, 'IC_PROVISION_PG.IC_BUSCA_ABONADOS_PR', LV_sSql, SN_ERROR, SV_MENSAJE );
...
EXCEPTION
        WHEN ERROR_CONTROLADO THEN
           ...
           END IF;
           SN_EVENTO := Ge_Errores_Pg.GrabarPL( SN_EVENTO, CV_cod_modulo, SN_ERROR || ' ' || SV_MENSAJE, CV_version, USER, 'IC_PROVISION_PG.IC_INSERTA_PR', LV_Sql, SN_ERROR, SV_MENSAJE );
        WHEN OTHERS THEN
                SN_ERROR := SQLCODE;
                SV_MENSAJE := SQLERRM;
                SN_EVENTO := Ge_Errores_Pg.GrabarPL(SN_EVENTO, CV_cod_modulo, 'Error en centrales '||SN_ERROR||' '||SV_MENSAJE, CV_version,USER, 'IC_PROVISION_PG.IC_INSERTA_PR', LV_Sql, SN_ERROR, SV_MENSAJE );
END IC_INSERTA_PR;
...
EXCEPTION
    WHEN ERROR_CONTROLADO THEN
        LV_DES_ERROR := 'IC_PROVISION_PG.IC_PROV_ABONADO_PR('||SV_MENSAJE||'['||SQLERRM||'])';
        IF NOT Ge_Errores_Pg.MensajeError(SN_ERROR,SV_MENSAJE) THEN
            SV_MENSAJE := CV_error_no_clasif||' ( '||SV_MENSAJE||' )';
        END IF;
    SN_EVENTO := Ge_Errores_Pg.GrabarPL( SN_EVENTO, CV_cod_modulo, SN_ERROR || ' ' || SV_MENSAJE, CV_version, USER, 'IC_PROVISION_PG.IC_PROV_ABONADO_PR', LV_Sql, SN_ERROR, LV_DES_ERROR );
        WHEN OTHERS THEN
                SN_ERROR := SQLCODE;
                SV_MENSAJE := SQLERRM;
                LV_DES_ERROR := 'IC_PROVISION_PG.IC_PROV_ABONADO_PR('||SQLERRM||')';
                SN_EVENTO := Ge_Errores_Pg.GrabarPL(SN_EVENTO, CV_cod_modulo, SQLERRM, CV_version,USER, 'IC_PROVISION_PG.IC_PROV_ABONADO_PR', LV_Sql, SN_ERROR, LV_DES_ERROR );
...
EXCEPTION
        WHEN ERROR_CONTROLADO THEN
           IF NOT Ge_Errores_Pg.MensajeError(SN_ERROR,SV_MENSAJE) THEN
              SV_MENSAJE := CV_error_no_clasif||' ( '||SV_MENSAJE||' )';
           END IF;
           SN_EVENTO := Ge_Errores_Pg.GrabarPL( SN_EVENTO, CV_cod_modulo, SN_ERROR || ' ' || SV_MENSAJE, CV_version, USER, 'IC_PROVISION_PG.IC_OBTIENE_ACTUACION_PR', LV_Sql, SN_ERROR, SV_MENSAJE );
        WHEN OTHERS THEN
                SN_ERROR := SQLCODE;
                SV_MENSAJE := SQLERRM;
                SN_EVENTO := Ge_Errores_Pg.GrabarPL(SN_EVENTO, CV_cod_modulo, SQLERRM, CV_version,USER, 'IC_PROVISION_PG.IC_OBTIENE_ACTUACION_PR', LV_Sql, SN_ERROR, SV_MENSAJE );
...
EXCEPTION
        WHEN ERROR_CONTROLADO THEN
           IF NOT Ge_Errores_Pg.MensajeError(SN_ERROR,SV_MENSAJE) THEN
              SV_MENSAJE := CV_error_no_clasif||' ( '||SV_MENSAJE||' )';
           END IF;
           SN_EVENTO := Ge_Errores_Pg.GrabarPL( SN_EVENTO, CV_cod_modulo, SN_ERROR || ' ' || SV_MENSAJE, CV_version, USER, 'IC_PROVISION_PG.IC_OBTIENE_CODPERIODICIDAD_PR', LV_Sql, SN_ERROR, SV_MENSAJE );
        WHEN OTHERS THEN
                SN_ERROR := SQLCODE;
                SV_MENSAJE := SQLERRM;
                SN_EVENTO := Ge_Errores_Pg.GrabarPL(SN_EVENTO, CV_cod_modulo, SQLERRM, CV_version,USER, 'IC_PROVISION_PG.IC_OBTIENE_CODPERIODICIDAD_PR', LV_Sql, SN_ERROR, SV_MENSAJE );
...
EXCEPTION
        WHEN ERROR_CONTROLADO THEN
           IF NOT Ge_Errores_Pg.MensajeError(SN_ERROR,SV_MENSAJE) THEN
              SV_MENSAJE := CV_error_no_clasif||' ( '||SV_MENSAJE||' )';
           END IF;
           SN_EVENTO := Ge_Errores_Pg.GrabarPL( SN_EVENTO, CV_cod_modulo, SN_ERROR || ' ' || SV_MENSAJE, CV_version, USER, 'IC_PROVISION_PG.IC_CODPERIODICIDAD_PR', LV_Sql, SN_ERROR, SV_MENSAJE );
        WHEN OTHERS THEN
                SN_ERROR := SQLCODE;
                SV_MENSAJE := SQLERRM;
                SN_EVENTO := Ge_Errores_Pg.GrabarPL(SN_EVENTO, CV_cod_modulo, SQLERRM, CV_version,USER, 'IC_PROVISION_PG.IC_CODPERIODICIDAD_PR', LV_Sql, SN_ERROR, SV_MENSAJE );
...
EXCEPTION
        WHEN ERROR_CONTROLADO THEN
           IF NOT Ge_Errores_Pg.MensajeError(SN_ERROR,SV_MENSAJE) THEN
              SV_MENSAJE := CV_error_no_clasif||' ( '||SV_MENSAJE||' )';
           END IF;
           SN_EVENTO := Ge_Errores_Pg.GrabarPL( SN_EVENTO, CV_cod_modulo, SN_ERROR || ' ' || SV_MENSAJE, CV_version, USER, 'IC_PROVISION_PG.IC_ELIMINA_PR', LV_Sql, SN_ERROR, SV_MENSAJE );
        WHEN OTHERS THEN
                SN_ERROR := SQLCODE;
                SV_MENSAJE := SQLERRM;
                SN_EVENTO := Ge_Errores_Pg.GrabarPL( SN_EVENTO, CV_cod_modulo, SQLERRM, CV_version,USER, 'IC_PROVISION_PG.IC_ELIMINA_PR', LV_Sql, SN_ERROR, SV_MENSAJE );
END IC_ELIMINA_PR;
