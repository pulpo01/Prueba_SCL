CREATE OR REPLACE FUNCTION cr_fn_genera_arch_abocel_tmm RETURN VARCHAR2 IS
-- PL/SQL Specification
--
-- *************************************************************
-- * Funcion            : CR_FN_GENERA_ARCH_ABOCEL_TMM
-- * Salida             : query para carga de archivo AboCel
...
BEGIN
    --
        v_ssql:= 'SELECT B.COD_CLIENTE, B.NUM_EVENTO, c.cod_tipdocum, C.DES_TIPDOCUM,B.PREF_PLAZA, B.NUM_FOLIO, '||
                         ...
                         'E.NOM_CALLE , E.NUM_CALLE, E.NUM_PISO, G.DES_COMUNA, H.DES_CIUDAD, E.ZIP, I.NUM_CELULAR '||
                         'FROM CRH_EVENTOS BB, CRT_DETALLES B, GE_CLIENTES D, GE_DIRECCIONES E, GA_DIRECCLI F, GE_COMUNAS G, GE_CIUDADES H, '||
...
EXCEPTION
   ...
   WHEN VALUE_ERROR THEN
                RETURN 'ERROR 1 cr_fn_genera_arch_abocel_tmm, SQLCODE:'||TO_CHAR(SQLCODE)||' SQLERRM:'||SQLERRM;

   WHEN INVALID_NUMBER THEN
                RETURN 'ERROR 2 cr_fn_genera_arch_abocel_tmm, SQLCODE:'||TO_CHAR(SQLCODE)||' SQLERRM:'||SQLERRM;

   WHEN OTHERS THEN
      RETURN 'ERROR 3 cr_fn_genera_arch_abocel_tmm, SQLCODE:'||TO_CHAR(SQLCODE)||' SQLERRM:'||SQLERRM;
END;
/
SHOW ERRORS
