CREATE OR REPLACE PROCEDURE        VE_P2_LIQUIDA_TRAFICO_IND(
...
vp_procedure                 VARCHAR2(30) := 'VE_P2_LIQUIDA_TRAFICO_IND';
...
CURSOR C1 IS
   SELECT TMP1.COD_VENDEDOR, TMP1.COD_TIPCOMIS, TMP1.COD_CTOLIQ, TMP1.IMP_PORCENTAJE,
         TMP1.COD_CLIENTE, TMP1.NUM_ABONADO, TMP2.NUM_SECUENCI, TMP2.COD_TIPDOCUM,
         TMP2.COD_VENDEDOR_AGENTE, TMP2.LETRA, TMP2.COD_CENTREMI
    FROM VE_TMP_PAGOSCLIENTE TMP2, VE_TMP_ABONADOSDEALER TMP1
...
--*** GENERACION DE TABLA ABONADOS POR MASTER DEALER ***
   BEGIN
      ...
      vp_accion := 'I';
      INSERT
       INTO VE_TMP_ABONADOSDEALER(COD_VENDEDOR,COD_TIPCOMIS,COD_CTOLIQ,IMP_PORCENTAJE,COD_CLIENTE,NUM_ABONADO)
...
-- ******************************************************************************************************
          ...
          AND TA_PLANTARIF.TIP_PLANTARIF       = vp_I   /* INDIVIDUAL */
   ...
   BEGIN
      ...
      INSERT
       ...
       SELECT TMP3.COD_CLIENTE, CAN.NUM_SECUENCI, CAN.COD_TIPDOCUM, CAN.COD_VENDEDOR_AGENTE,
        ...
        FROM CO_CANCELADOS CAN , VE_TMP_CLIENTESDEALER TMP3
   ...
   BEGIN
      ...
      INSERT
       ...
       SELECT HD.COD_CLIENTE, HD.NUM_SECUENCI, HD.COD_TIPDOCUM, HD.COD_VENDEDOR_AGENTE,
        ...
        FROM FA_HISTDOCU HD, VE_TMP_CLIENTESDEALER TMP3
...
--        GRABAR RESULTADO DEL VENDEDOR ANTERIOR
          BEGIN
             ...
             INSERT
              ...
              VALUES(vp_I, vp_num_liquidacion, vp_cod_tipcomis, vp_cod_vendedor,
...
--    Construye query dinamico: suma conceptos por trafico de la factura
...
--    Recupera la columna de la unica fila del cursor
         ...
         END IF;
--    Se suma el monto de la factura al total del vendedor
...
IF vp_cod_vendedor <> 0 THEN /* Si al menos se process un vendedor:            */
   ...
   BEGIN
      vp_importe_resultado := nvl(vp_importe_total,0) * vp_imp_porcentaje / 100.0;
      vp_tabla := 'VE_RESTRAFICO';
      vp_accion := 'I';
      INSERT
       INTO VE_RESTRAFICO(TIP_PLANTARIF, NUM_LIQUIDACION, COD_TIPCOMIS, COD_VENDEDOR,
