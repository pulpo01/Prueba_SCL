CREATE OR REPLACE PROCEDURE P_CAMBIO_PLANSERV(
...
--
-- *************************************************************
-- * procedimiento      : P_CAMBIO_PLANSERV
-- * Descripcisn        : Procedimiento que refleja el cambio y efectividad del nuevo plan
...
BEGIN
   VP_PROC := 'P_CAMBIO_PLANSERV';
   BEGIN
      VP_TABLA := 'FA_CICLFACT';
      VP_ACT := 'S';
      --SELECT FEC_DESDECFIJOS,COD_CICLFACT
      --  INTO V_FECHADCF,V_CICLFACT
      --  FROM FA_CICLFACT
      -- WHERE COD_CICLO = VP_CICLO
      --   AND TRUNC(VP_FECSYS) <= TRUNC(FEC_DESDECFIJOS)
  ...
  WHEN NO_DATA_FOUND THEN
      ...
      ELSIF V_INDCAR <> 0 THEN
         ...
         VP_ACT := 'U';
         UPDATE GA_INTARCEL
            SET COD_PLANSERV = VP_PLANSERV,NUM_IMSI=V_NUM_IMSI
          WHERE COD_CLIENTE = VP_CLIENTE
            AND NUM_ABONADO = VP_ABONADO
            AND FEC_DESDE >= V_FECHADCF;
         VP_ERROR := '0';
            ...
            ELSIF V_FECHADLL < V_FECHADCF THEN
               ...
               INSERT INTO GA_INTARCEL
                   ...
                   VALUES (V_CLIENTE,V_ABONADO,V_NUMERO,
                           ...
                           V_TIPPLANTARIF,V_PLANTARIF,V_SERIE,
                           V_CELULAR,V_CARGOBASICO,V_CICLO,
                           V_PLANCOM,VP_PLANSERV,V_GRPSERV,
         ...
         ELSIF V_FECHADLL < V_FECHADCF THEN
            ...
            INSERT INTO GA_INTARTREK
                ...
                VALUES (V_CLIENTE,V_ABONADO,V_FECHADCF,
                        ...
                        VP_PLANSERV,V_GRPSERV,V_SERIETREK,
                        V_GRUPO);
         END IF;
      ELSE
         VP_TABLA := 'GA_INTARTREK';
