CREATE OR REPLACE PACKAGE BODY GE_CONSULTADIRECCION_PG AS

PROCEDURE GE_CONSDIRCLI_PR(EN_NUMCELULAR     IN  GA_ABOCEL.NUM_CELULAR%TYPE,
...
/*
<NOMBRE>  		  		  :GE_CONSDIRCLI_PR</NOMBRE>
...
<PARAMSAL>				  :DESCRIPCION TIPO DIRECCION, NOMBRE CALLE, NUMERO CALLE, NUMERO PISO, CODIGO REGION,
                           ...
                           CODIGO ERROR, DESCRIPCION ERROR, NUMERO EVENTO</PARAMSAL>
*/
...
BEGIN
  ...
  ELSE
     IF VN_CODTIPDIR IS NULL THEN
	    BEGIN
		   LV_sSql:= 'SELECT VAL_PARAMETRO FROM GED_PARAMETROS WHERE COD_PRODUCTO='||GV_cod_producto||' AND COD_MODULO='||GV_cod_modulo_ga||' AND NOM_PARAMETRO='||GV_nom_parametro;
	       SELECT VAL_PARAMETRO
		   INTO   VN_CODTIPDIR
		   FROM   GED_PARAMETROS
		   WHERE  COD_PRODUCTO=GV_cod_producto--1
			      AND COD_MODULO=GV_cod_modulo_ga --'GA'
			      AND NOM_PARAMETRO=GV_nom_parametro;--'TIP_DIRECCION';

		   EXCEPTION
		      WHEN NO_DATA_FOUND THEN
		         SN_cod_retorno:=12;
		         SV_mens_retorno:='no se encuentra parametro tip_direccion';
				 RAISE FIN;
		END;
	 END IF;
     LV_sSql:= 'GE_OBTIENE_ABONADO_PR('|| VN_NUMCELULAR ||')';
     GE_OBTIENE_ABONADO_PR(VN_NUMCELULAR,
	   	  		  		   VN_NUMABONADO,
						   VN_CODCLIENTE,
						   VV_CODTECNO,
						   VD_FECHAACT,
    ...
    IF SN_cod_retorno != 0 OR SV_mens_retorno IS NOT NULL THEN
	   RAISE FIN;
	END IF;
    LV_sSql:= 'GE_OBTIENE_CLIENTE_PR('|| VN_CODCLIENTE ||')';
	GE_OBTIENE_CLIENTE_PR(VN_CODCLIENTE,
						  VV_NOMCLIENTE,
						  SN_cod_retorno,
						  SV_mens_retorno);
	...
	END IF;
	LV_sSql:= 'GE_OBTIENE_CLIENTE_PR('|| VN_CODCLIENTE ||')';
    GE_OBTIENE_DIRECCLI_PR(VN_CODCLIENTE,
						   VN_CODTIPDIR,
						   VN_CODDIR,
						   SN_cod_retorno,
						   SV_mens_retorno);
    ...
    IF SN_cod_retorno != 0 OR SV_mens_retorno IS NOT NULL THEN
	   RAISE FIN;
	END IF;
    LV_sSql:= 'GE_OBTIENE_DESDIREC_PR('|| VN_CODDIR ||','|| VN_CODTIPDIR ||')';
	GE_OBTIENE_DESDIREC_PR(VN_CODDIR,
	                       VN_CODTIPDIR,
						   VV_NOMCALLE,
						   VV_NUMCALLE,
						   VV_NUMPISO,
  ...
  EXCEPTION
     WHEN PARAMVACIO THEN
	    SN_cod_retorno:=12;
		SV_mens_retorno:='numero de celular invalido';
		LV_des_error := 'GE_CONSDIRCLI_PR('||EN_NUMCELULAR||','||EN_CODTIPDIR||'); - ' || SQLERRM;
	    SN_num_evento := Ge_Errores_Pg.Grabarpl( SN_num_evento, GV_cod_modulo, SV_mens_retorno, '1.0', USER, 'GE_CONSULTADIRECCION_PG.GE_CONSDIRCLI_PR', LV_sSql, SN_cod_retorno, LV_des_error );

	 WHEN FIN THEN
	    NULL;

END GE_CONSDIRCLI_PR;

-----------------------------------------------------------------------------------------------------------------------------------
PROCEDURE GE_OBTIENE_ABONADO_PR(EN_NUMCELULAR      IN  GA_ABOCEL.NUM_CELULAR%TYPE,
...
/*
<NOMBRE>  		  		  :GE_OBTIENE_ABONADO_PR</NOMBRE>
<FECHACREA>				  :18/08/2004</FECHACREA>
<MODULO>				  :POSTVENTA</MODULO>
<AUTOR>					  :ROBERTO PÉREZ VARAS</AUTOR>
<VERSION>				  :1.0</VERSION>
...
END GE_OBTIENE_ABONADO_PR;

-----------------------------------------------------------------------------------------------------------------------------------
PROCEDURE GE_OBTIENE_CLIENTE_PR(EN_CODCLIENTE IN GE_CLIENTES.COD_CLIENTE%TYPE,
	   	  		  				SV_NOMCLIENTE OUT VARCHAR2,
								SN_CODERROR   OUT NUMBER,
								SV_DESERROR   OUT VARCHAR2) IS

/*
<NOMBRE>  		  		  :GE_OBTIENE_CLIENTE_PR</NOMBRE>
<FECHACREA>				  :18/08/2004</FECHACREA>
<MODULO>				  :POSTVENTA</MODULO>
<AUTOR>					  :ROBERTO PÉREZ VARAS</AUTOR>
<VERSION>				  :1.0</VERSION>
...
END GE_OBTIENE_CLIENTE_PR;

-----------------------------------------------------------------------------------------------------------------------------------
PROCEDURE GE_OBTIENE_DIRECCLI_PR(EN_CODCLIENTE   IN GE_CLIENTES.COD_CLIENTE%TYPE,
...
/*
<NOMBRE>  		  		  :GE_OBTIENE_DIRECCLI_PR</NOMBRE>
<FECHACREA>				  :18/08/2004</FECHACREA>
<MODULO>				  :POSTVENTA</MODULO>
<AUTOR>					  :ROBERTO PÉREZ VARAS</AUTOR>
<VERSION>				  :1.0</VERSION>
...
END GE_OBTIENE_DIRECCLI_PR;

-----------------------------------------------------------------------------------------------------------------------------------
PROCEDURE GE_OBTIENE_DESDIREC_PR(EN_CODDIRECCION   IN GE_DIRECCIONES.COD_DIRECCION%TYPE,
...
/*
<NOMBRE>  		  		  :GE_OBTIENE_DESDIREC_PR</NOMBRE>
<FECHACREA>				  :18/08/2004</FECHACREA>
<MODULO>				  :POSTVENTA</MODULO>
<AUTOR>					  :ROBERTO PÉREZ VARAS</AUTOR>
<VERSION>				  :1.0</VERSION>
...
END GE_OBTIENE_DESDIREC_PR;

-----------------------------------------------------------------------------------------------------------------------------------
PROCEDURE GE_CONSREGION_PR ( SR_REGIONES	   OUT REGIONES,
						     SN_cod_retorno    OUT NOCOPY      ge_errores_td.cod_msgerror%TYPE,
						     SV_mens_retorno   OUT NOCOPY      ge_errores_td.det_msgerror%TYPE,
						     SN_num_evento     OUT NOCOPY      ge_errores_pg.Evento
                           )
AS
/*
<NOMBRE>  		  		  :GE_CONSREGION_PR</NOMBRE>
<FECHACREA>				  :18/08/2004</FECHACREA>
<MODULO>				  :POSTVENTA</MODULO>
<AUTOR>					  :ROBERTO PÉREZ VARAS</AUTOR>
<VERSION>				  :1.0</VERSION>
...
<FECHAMOD>				  :</FECHAMOD>
<PARAMENTR>				  :</PARAMENTR>
<PARAMSAL>				  :CODIGO REGION, DESCRIPCION REGION, CODIGO ERROR, DESCRIPCION ERROR, NUMERO EVENTO</PARAMSAL>
*/
LV_sSql       ge_errores_pg.vQuery;
LV_des_error  ge_errores_pg.DesEvent;
BEGIN
	...
	EXCEPTION
      WHEN OTHERS THEN
		 ...
		 LV_des_error := 'GE_CONSREGION_PR();-' || SQLERRM;
	     SN_num_evento := Ge_Errores_Pg.Grabarpl( SN_num_evento, GV_cod_modulo, SV_mens_retorno, '1.0', USER, 'GE_CONSULTADIRECCION_PG.GE_CONSREGION_PR', LV_sSql, SN_cod_retorno, LV_des_error );

END GE_CONSREGION_PR;

-----------------------------------------------------------------------------------------------------------------------------------
PROCEDURE GE_CONSPROVINCIA_PR (EN_CODREGION      IN  GE_DIRECCIONES.COD_REGION%TYPE,
...
/*
<NOMBRE>  		  		  :GE_CONSPROVINCIA_PR</NOMBRE>
<FECHACREA>				  :18/08/2004</FECHACREA>
<MODULO>				  :POSTVENTA</MODULO>
<AUTOR>					  :ROBERTO PÉREZ VARAS</AUTOR>
<VERSION>				  :1.0</VERSION>
...
<PARAMSAL>				  :CODIGO PROVINCIA, DESCRIPCION PROVINCIA, CODIGO ERROR, DESCRIPCION ERROR, NUMERO EVENTO</PARAMSAL>
*/
LV_sSql       	 ge_errores_pg.vQuery;
LV_des_error  	 ge_errores_pg.DesEvent;
VN_CODREGION     GE_DIRECCIONES.COD_REGION%TYPE;
...
BEGIN
  ...
  EXCEPTION
      WHEN OTHERS THEN
	     SN_cod_retorno := SQLCODE;
         SV_mens_retorno := SQLERRM;
		 LV_des_error := 'GE_CONSPROVINCIA_PR('||EN_CODREGION||');-' || SQLERRM;
	     SN_num_evento := Ge_Errores_Pg.Grabarpl( SN_num_evento, GV_cod_modulo, SV_mens_retorno, '1.0', USER, 'GE_CONSULTADIRECCION_PG.GE_CONSPROVINCIA_PR', LV_sSql, SN_cod_retorno, LV_des_error );
END GE_CONSPROVINCIA_PR;

-----------------------------------------------------------------------------------------------------------------------------------
PROCEDURE GE_CONSCIUDAD_PR (EN_CODREGION      IN  GE_DIRECCIONES.COD_REGION%TYPE,
...
/*
<NOMBRE>  		  		  :GE_CONSCIUDAD_PR</NOMBRE>
<FECHACREA>				  :18/08/2004</FECHACREA>
<MODULO>				  :POSTVENTA</MODULO>
<AUTOR>					  :ROBERTO PÉREZ VARAS</AUTOR>
<VERSION>				  :1.0</VERSION>
...
<PARAMSAL>				  :CODIGO CIUDAD, DESCRIPCION CIUDAD, CODIGO ERROR, DESCRIPCION ERROR, NUMERO EVENTO</PARAMSAL>
*/
LV_sSql       	 ge_errores_pg.vQuery;
LV_des_error  	 ge_errores_pg.DesEvent;
VN_CODREGION     GE_DIRECCIONES.COD_REGION%TYPE;
...
BEGIN
  ...
  EXCEPTION
      WHEN OTHERS THEN
	     SN_cod_retorno := SQLCODE;
         SV_mens_retorno := SQLERRM;
		 LV_des_error := 'GE_CONSCIUDAD_PR('||EN_CODREGION||','||EN_CODPROVINCIA||');-' || SQLERRM;
	     SN_num_evento := Ge_Errores_Pg.Grabarpl( SN_num_evento, GV_cod_modulo, SV_mens_retorno, '1.0', USER, 'GE_CONSULTADIRECCION_PG.GE_CONSCIUDAD_PR', LV_sSql, SN_cod_retorno, LV_des_error );
END GE_CONSCIUDAD_PR;

-----------------------------------------------------------------------------------------------------------------------------------
PROCEDURE GE_CONSCOMUNA_PR (EN_CODREGION      IN  GE_DIRECCIONES.COD_REGION%TYPE,
		  				    EN_CODPROVINCIA   IN  GE_DIRECCIONES.COD_PROVINCIA%TYPE,
...
/*
<NOMBRE>  		  		  :GE_CONSCOMUNA_PR</NOMBRE>
<FECHACREA>				  :18/08/2004</FECHACREA>
<MODULO>				  :POSTVENTA</MODULO>
<AUTOR>					  :ROBERTO PÉREZ VARAS</AUTOR>
<VERSION>				  :1.0</VERSION>
...
<PARAMSAL>				  :CODIGO COMUNA, DESCRIPCION COMUNA, CODIGO ERROR, DESCRIPCION ERROR, NUMERO EVENTO</PARAMSAL>
*/
LV_sSql       	 ge_errores_pg.vQuery;
LV_des_error  	 ge_errores_pg.DesEvent;
VN_CODREGION	GE_DIRECCIONES.COD_REGION%TYPE;
...
BEGIN
   ...
   EXCEPTION
      WHEN OTHERS THEN
	     SN_cod_retorno := SQLCODE;
         SV_mens_retorno := SQLERRM;
		 LV_des_error := 'GE_CONSCOMUNA_PR('||EN_CODREGION||','||EN_CODPROVINCIA||','||EN_CODCIUDAD||');-' || SQLERRM;
	     SN_num_evento := Ge_Errores_Pg.Grabarpl( SN_num_evento, GV_cod_modulo, SV_mens_retorno, '1.0', USER, 'GE_CONSULTADIRECCION_PG.GE_CONSCOMUNA_PR', LV_sSql, SN_cod_retorno, LV_des_error );
END GE_CONSCOMUNA_PR;
...
v_salida_version VARCHAR2(100);
	...
	select '000' into subver from dual;
