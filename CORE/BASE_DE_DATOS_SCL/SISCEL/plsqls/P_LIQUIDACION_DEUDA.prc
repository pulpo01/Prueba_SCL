CREATE OR REPLACE PROCEDURE P_LIQUIDACION_DEUDA(
V_SECUENCIA  IN VARCHAR2,
V_NUMVENTA IN VARCHAR2 ) IS
...
VP_ERROR		   	   	VARCHAR2(1);
V_POSEE_NC		   	   	BOOLEAN;
V_PREF_PLAZA		   	FA_HISTDOCU.PREF_PLAZA%TYPE;
r_num_filas				PLS_INTEGER;
CURSOR c_num_filas ( p_folio FA_HISTDOCU.NUM_FOLIO%TYPE
                     , p_cliente FA_HISTDOCU.COD_CLIENTE%TYPE
                     , p_tipdocum FA_HISTDOCU.COD_TIPDOCUM%TYPE)
IS
 	SELECT COUNT(1)
	FROM FA_AJUSTES
	WHERE NUM_FOLIO= p_folio
	AND COD_CLIENTE= p_cliente
	AND COD_TIPDOCUM= p_tipdocum
	AND ROWNUM=1;
	...
	BEGIN
	    BEGIN
			   ...
			   V_VENDEDOR_AGENTE,V_LETRA,V_CENTREMI,V_FOLIO,V_TOT_FACTURA,
 			   V_SUPERTEL,V_CLIENTE, V_PREF_PLAZA
        	   FROM FA_HISTDOCU
		...
		IF V_SIGUE AND V_TOT_FACTURA > 0 AND V_FOLIO > 0 AND V_SUPERTEL=0 THEN
		   ...
		   EXCEPTION
		   		WHEN NO_DATA_FOUND THEN
					 V_NUM_FILAS:=0;
					 BEGIN
						OPEN c_num_filas ( V_FOLIO, V_CLIENTE, V_TIPDOCUM );
						FETCH c_num_filas INTO r_num_filas;
						IF c_num_filas%NOTFOUND OR r_num_filas = 0 THEN
						   --NOTA CREDITO TOTAL
						   FA_PRE_NOTACREDINTER (V_NUM_TRANS_NC,V_IND_ORDENTOTAL,
						...
						ELSE --NOTA CREDITO PARCIAL
							...
							IF V_SIGUE THEN
                                ...
                                SSQL:=SSQL || ' FA_AJUSTECONC A, ' || V_TABLA_CONCEP || ' F ';
                                ...
                                -- CH-24042003595 Soporte 13 Mayo 2003 Se adiciono DEFINE_COLUMN
