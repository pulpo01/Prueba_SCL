CREATE OR REPLACE procedure PV_TRASPASOPROPIEDADINTAR_PR
...
--
-- *************************************************************
-- * procedimiento      : PV_TRASPASOPROPIEDADINTAR_PR
...
--
   ...
   vCantAbonados NUMBER; /* Modificacion by SAQL/Soporte 17/07/2006 - CO-200607050216 */
   
   /* INICIO 186035 BRC - 06-07-2012*/
	count_cliente number;
	/* FIN 186035 BRC - 06-07-2012*/

BEGIN
   /* INICIO 186035 BRC - 06-07-2012*/
          VP_CLIENEW:=VP_CLIENTED;
            ...
            count(1)
            into count_cliente 
            from ga_abocel where cod_cliente=VP_CLIENTED;
            
            if (count_cliente>0) then 
            V_TABORIGEN:='ABOCEL';
            end if;
          /* FIN 186035 BRC - 06-07-2012*/


   VP_PROC := 'PV_TRASPASOPROPIEDADINTAR_PR';
   VP_ERROR := '0';
   ...
   IF VP_PRODUCTO = 1 THEN
      ...
      IF vCantAbonados = 0 THEN
         BEGIN
            ...
            FEC_HASTA = VP_FECSYS - (1/(24*60*60))
            WHERE COD_CLIENTE = VP_CLIENTE
            AND NUM_ABONADO = 0;
         EXCEPTION
            WHEN OTHERS THEN
