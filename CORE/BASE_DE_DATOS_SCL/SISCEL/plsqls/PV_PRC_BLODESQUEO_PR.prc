CREATE OR REPLACE procedure PV_PRC_BLODESQUEO_PR
	 ...
	 -- Proyecto VPN Gestor Corporativo Dic 2003
  	 ...
  	 BEGIN
		...
		IF VP_NUMTRANSABABO IS NULL THEN -- Sin Gestor --
  	 	   PV_PRC_INS_MOVIMIENTO_PR(0,VP_NUM_ABONADO,VP_COD_ACTABO,VP_NOM_USUARIO,0,VP_SUSPREHA,'',V_NUM_MOVIMIENTO,VP_SQLCODE,VP_SQLERRM,VP_ERROR);
		ELSE -- Con Gestor de Contactos --
		   PV_PRC_INS_MOVIMIENTO_PR(TO_NUMBER(VP_NUMTRANSABABO),VP_NUM_ABONADO,VP_COD_ACTABO,VP_NOM_USUARIO,0,VP_SUSPREHA,'',V_NUM_MOVIMIENTO,VP_SQLCODE,VP_SQLERRM,VP_ERROR);
		END IF;
