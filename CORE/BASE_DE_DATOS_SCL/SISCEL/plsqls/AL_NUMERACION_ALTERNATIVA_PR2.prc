CREATE OR REPLACE PROCEDURE AL_NUMERACION_ALTERNATIVA_PR2 (p_orden al_cab_numeracion.num_numeracion%type) 
IS
...
v_numeracion al_cab_numeracion.tip_numeracion%type;
...
v_seriesNumeradas al_lin_numeracion.can_numera%type;
   ...
   IS
	  SELECT a.num_numeracion, a.num_serie
	  FROM tmp_numeracion_alter a
	  WHERE a.num_numeracion=p_orden
	  AND not exists (SELECT 1 FROM al_ser_numeracion b WHERE b.num_numeracion=a.num_numeracion AND b.lin_numeracion=1 AND b.num_serie=a.num_serie)
	  ;
...
BEGIN
	 ...
	 IF v_error = 0 THEN
    	  ...
    	  IF (v_nrosSistema > 0  AND v_nrosSistema >= v_cantnum)  THEN
        	  ...
        	  -- Query dinamico que validara cada serie     
              squery := 'SELECT  1 FROM al_series a ';
			  squery := squery || '  WHERE a.num_serie = :serie' ;
              squery := squery || '  AND a.cod_bodega =:bodega'; 
			  squery := squery || '  AND a.cod_articulo    = :articulo';
              squery := squery || '  AND a.tip_stock = :stock'; 
			  squery := squery || '  AND a.cod_uso = :uso ';
			  squery := squery || '  AND a.cod_estado   =:estado';
              IF (v_terminalGSM = v_terminal AND v_hlr is not null) THEN
                squery := squery || '  AND a.cod_hlr   = :hlr' ;
              END IF;
              squery := squery || '  AND a.cod_estado not in (' || v_estadoReserva || ',' || v_estadoTemporal || ',' || v_estadoTransito || ',' ||  v_estadoReservaVta || ')' ;
	    	  squery := squery || '  AND a.ind_telefono <> 8';
              squery := squery || '  AND not exists (SELECT d.num_serie'; 
	          squery := squery || '  FROM al_cab_numeracion c , al_lin_numeracion b, al_ser_numeracion d';
              squery := squery || '  WHERE c.cod_estado =''' || sEstado || '''';
              squery := squery || '  AND d.num_numeracion >= 0 AND d.lin_numeracion >= 0';
              squery := squery || '  AND c.num_numeracion = b.num_numeracion ';
			  squery := squery || '  AND b.num_numeracion=d.num_numeracion';
              squery := squery || '  AND b.lin_numeracion=d.lin_numeracion';
			  squery := squery || '  AND d.num_serie = a.num_serie )';
         	  IF (v_numeracion = 0) THEN
                 squery := squery || ' AND a.num_telefono is not null';
              END IF;
 
              IF (v_numeracion <> 0) THEN
                 squery := squery || ' AND a.num_telefono is null ';
              END IF;	

			 
        	  v_serieBuenas:=0;
        	  --Validacion de todas las series cargadas   
			  OPEN c_validaseries(p_orden);
			  LOOP
              ...
              EXIT WHEN c_validaseries%NOTFOUND;
                   ...
                   BEGIN
                       	IF (v_terminalGSM = v_terminal AND v_hlr is not null) THEN
                       		  ...
                       		  INTO v_valida
                       		  USING v_validaseries.num_serie,v_bodega,v_articulo,v_stock,v_uso,v_estado,v_hlr;
                       	ELSE
                       		  EXECUTE IMMEDIATE squery
                       		  INTO v_valida
                       		  USING v_validaseries.num_serie,v_bodega,v_articulo,v_stock,v_uso,v_estado;		
                       	END IF;  
                   EXCEPTION 
                  		WHEN no_data_found THEN
			  ...
			  IF v_serieBuenas > 0 THEN
            	  ...
            	  INSERT INTO al_ser_numeracion (
		  				  		 num_numeracion ,lin_numeracion,   num_serie, num_serhex, ind_numerado, cod_producto, num_telefono, cod_central, cod_subalm, cod_cat,   carga, plan)
            	  SELECT  a.num_numeracion,          1, a.num_serie,  0    , v_numeracion, v_producto,         null,   v_central,   v_subalm,   v_cat, v_carga, v_plan
            	  FROM tmp_numeracion_alter a
            	  WHERE num_numeracion=p_orden
				  AND not exists (SELECT 1 FROM al_ser_numeracion b WHERE b.num_numeracion=a.num_numeracion AND b.lin_numeracion=1 AND b.num_serie=a.num_serie)
                  AND a.ind_serie=0;
    			  ...
    			  v_seriesNumeradas:=v_serieBuenas;
				  p_asigna_numero_ordenes2(p_orden,1,v_subalm,v_central,v_uso,v_producto,v_seriesNumeradas,v_cat,v_error);
				  
     			  IF v_error <> 0 THEN
				      --Eliminar series de al_ser_numeracion y el trigger se encarga de devolver los numeros  
					    
					  DELETE FROM al_ser_numeracion
					  WHERE num_numeracion=p_orden
					  AND lin_numeracion=1
					  ...
					  COMMIT;					  
         	          ...
         	          v_error_msg:= 'Error p_asigna_numero_ordenes.'||' Orden:'||to_char(p_orden)||' SubAlm:'||v_subalm||' Central:'||to_char(v_central)||' Uso:'||to_char(v_uso)||' Categoria:'||to_char(v_cat) ;
     			  ...
     			  ELSE
    			     -- dbms_output.put_line('COMMIT');
                	  ...
                	  --COMMIT;
					  dbms_output.put_line('Proceso Finalizado. Se numeraron '||to_char(v_serieBuenas)||' Series.');
    			  END IF;
			  END IF;	  
         ELSE
		    v_error:=2;
