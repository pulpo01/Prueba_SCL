CREATE OR REPLACE PROCEDURE P_DEL_CAMSERIE (
...
IS
   ...
   --v_filas_nue_equ      NUMBER(1); -- PAGC 05-05-2004 Hom. TM-200403150568
   ...
   v_actcenSS             ga_actabo.cod_actcen%TYPE;
   ...
   v_camb_tec_ss                   ged_parametros.VAL_PARAMETRO%type;
   ...
   vp_indsusp                                           number(1):=0; --ahott 18-12-2003 CH-021220031546
   v_siniestro                   ga_siniestros.NUM_SINIESTRO%type; -- ahott 19-12-2003 CH-021220031546
   v_causa_sinies                ga_siniestros.COD_CAUSA%type; -- ahott 19-12-2003 CH-021220031546
   sFORMATO_SEL2                 ged_parametros.VAL_PARAMETRO%type; --ahott 22-12-2003 CH-021220031546
   v_cod_servicio                ga_susprehabo.cod_servicio%type; --ahott 22-12-2003 CH-021220031546
   v_cod_tipmodi                                 ga_modabocel.cod_tipmodi%type; -- ahott 26-01-2004 CH-200401231637
   v_cod_causa                                   ga_modabocel.cod_causa%type; -- ahott 19-02-2001 CH-200402161668
   --TMM-04026 Ini
   v_tec_dma                    ga_equipaboser.COD_TECNOLOGIA%type;
   v_tec_gsm                    ga_equipaboser.COD_TECNOLOGIA%type:='GSM'; --COL-36436|04-01-2007|AHA
   --TMM-04026 Fin
   CURSOR c1
   IS
   ...
   IS
      SELECT a.num_abonado, a.num_celular, a.num_serie, a.num_seriehex,
             a.tip_terminal, a.cod_central
        FROM ga_abocel a
       WHERE a.cod_situacion = 'CSP'
         AND TRUNC (a.fec_ultmod) <=   SYSDATE
                                     - 1
      ...
      SELECT a.num_abonado, a.num_celular, a.num_serie, a.num_seriehex,
        ...
        FROM ga_aboamist a
                 WHERE a.cod_situacion = 'CSP'
         AND TRUNC (a.fec_ultmod) <=   SYSDATE - 1
         AND a.num_serie NOT IN (SELECT num_serie
                                   FROM ga_equipaboser b
                                  WHERE a.num_abonado = b.num_abonado
                                    AND a.num_serie = b.num_serie);*/
...
BEGIN
   ...
   LOOP
     ...
     -- Inicio ahott 19-02-2004 CH-200402161668
     SELECT COD_CAUSA INTO v_cod_causa
     FROM GA_MODABOCEL
     WHERE NUM_ABONADO=v_abonado AND
     FEC_MODIFICA=(SELECT MAX(FEC_MODIFICA)
                     FROM GA_MODABOCEL
                    WHERE NUM_ABONADO=v_abonado AND Cod_tipmodi in ('RE','SA','CE'));
    -- Fin ahott 19-02-2004 CH-200402161668
    IF v_cod_causa <> 'EP' THEN -- Ahott 19-02-2004 CH-200402161668
      -- busca movimiento
      ...
      THEN -- no encuentra movimiento
         BEGIN
            ...
            BEGIN
               vp_tabla := 'GA_ABOCEL';
               vp_act := 'S';
               SELECT num_serie, tip_terminal, cod_cliente, cod_central, num_imei, cod_cuenta,cod_tipcontrato,num_contrato,num_anexo,clase_servicio
                 INTO v_serie_nue, v_terminal_nue, v_cliente, v_central, v_imei_nue,v_cuenta,v_tipcontrato,v_contrato,v_anexo,v_clase_servicio
                 FROM ga_abocel
               ...
               BEGIN
                  ...
                  THEN
                     BEGIN
                        ...
                        IF v_estado = 7
                        THEN
                           --Inicio ahott 20-11-2003 HB-311020030133_HB-311020030134
                           BEGIN
                                   SELECT ROWID
                                     INTO v_fila_reserva
                                     FROM ga_reservart
                            ...
                            EXCEPTION
                                WHEN NO_DATA_FOUND THEN
                                     v_fila_reserva := '.';
                            END;
                            --Fin ahott 20-11-2003 HB-311020030133_HB-311020030134
                        ELSE
                           v_fila_reserva := '.';
                        END IF;
                        ...
                        THEN
                           BEGIN
                              ...
                              SELECT MAX (ROWID)
                                ...
                                FROM ga_modabocel
                               WHERE num_abonado = v_abonado
                                 AND cod_tipmodi in ('CE', 'TG', 'GT', 'ET', 'EG','SA','RE') --ahott 17-12-2003 CH-021220031546
                                 ...
                                 AND TRUNC (fec_modifica) = TRUNC (v_fec_cambio)
                                                                 AND ROWNUM=1 --Evitar que seleccione más de un registro en el caso que exista
                                                                 Order by fec_modifica desc;--más de un cambio de serie en un día
                                                                        ...
                                                                        P_GA_INTERAL(v_numtransacabo, --v_transac_x,
                                                                                                                                                                 ...
                                                                                                                                                                 BEGIN
                                                                                                                                                                         ...
                                                                                                                                                                         if v_cod_retorno = 0 then
                                                                                                                                                                                if v_des_cadena is null then
                                                                                                                                                                                   dbms_output.put_line('Error ejecutando actualización de stock.');
                                                                                                                                                                                   raise error_proceso;
                                                                                                                                                                                end if;
                                                                                                                                                                         else
                                                                                                                                                                                 dbms_output.put_line('Error ejecutando actualización de stock.['||v_numtransacabo||':'||v_des_cadena||']');
                                                                                                                                                                                 raise error_proceso;
                                                                                                                                                                         end if;
                                                                                         ...
                                                                                         where num_serie=v_num_simcard
                                                                                                   and rownum=1;
                                                                                                                                                 ...
                                                                                                                                                 if v_fila_reserva <> '.' then --ahott 26-01-2004 CH-200401231637
                                                                         delete ga_reservart
                                                                         where rowid = v_fila_reserva;
                                                                                                                                                 end if; --ahott 26-01-2004 CH-200401231637
                              ...
                              p_ins_audrest (
                                 ...
                                 vp_error
                                                          ...
                                                          --Inicio ahott 22-12-2003 CH-021220031546
                                                          begin
                                      SELECT cod_tipmodi -- ahott 26-01-2004 CH-200401231637
                                        INTO v_cod_tipmodi -- ahott 26-01-2004 CH-200401231637
                                        FROM ga_modabocel
                                       WHERE num_abonado = v_abonado
                                  ...
                                  begin
                                          ...
                                          DELETE GA_HDETSINIE where NUM_SINIESTRO = v_siniestro;
                                                          ...
                                                          exception
                                                                  when no_data_found then
                                                                           dbms_output.put_line('Cambio de serie');
                                                          end;
                                                          --Fin ahott 22-12-2003 CH-021220031546
...
--                                                        if v_imei_ant is Null then
                              ...
                              vp_act := 'S';
                                                          ...
                                                          BEGIN
                                                           ...
                                                           Exception
                                                                                when no_data_found then--No hubo cambio número contrato
                              ...
                              UPDATE ga_abocel A
                                 SET num_serie        = v_serie_ant,
                                     ...
                                     fec_prorroga     = nvl(fec_prorroga_modabo,A.FEC_PRORROGA),
                                                                         ...
                                                                         --clase_servicio = A.perfil_abonado
...
--                                                                       fec_prorroga       = nvl(fec_prorroga_modabo,A.FEC_PRORROGA),
                               ...
                               WHERE num_abonado        = v_abonado;
                                                           ...
                                                           -- Solo si existió un cambio de tecnologia
                              ...
                              END IF;
                                                          ...
                                                          --inicio ahott 23-12-2003 CH-021220031546
                                                          begin
                                      SELECT cod_tipmodi -- ahott 26-01-2004 CH-200401231637
                                        INTO v_cod_tipmodi -- ahott 26-01-2004 CH-200401231637
                                        FROM ga_modabocel
                                       WHERE num_abonado = v_abonado
                                         ...
                                         AND TRUNC (fec_modifica) = TRUNC (v_fec_cambio)
                                                                         AND ROWNUM=1 --Evitar que seleccione más de un registro en el caso que exista
                                                                    Order by fec_modifica desc;--más de un cambio de serie en un día
                                                                  P_VALIDA_SUSPENSIONES (v_abonado,vp_indsusp); --ahott 18-12-2003

                                                                  dbms_output.put_line(' vp_indsusp = '||vp_indsusp);
                                                                  IF vp_indsusp=0 then --ahott 18-12-2003
                                                                     UPDATE ga_abocel
                                                                     set cod_situacion='AAA'
                                                                     WHERE num_abonado = v_abonado;
                                                                     dbms_output.put_line('Up ga_abocel');
                                                                     UPDATE ga_aboamist
                                                                     set cod_situacion='AAA'
                                                                     WHERE num_abonado = v_abonado;
                                                                     dbms_output.put_line('Up ga_aboamist');
                                                                  else
                                         ...
                                         AND ROWNUM=1;
                                                                     UPDATE ga_abocel
                                                                         ...
                                                                         WHERE num_abonado = v_abonado;
                                                                         dbms_output.put_line('Up ga_aboamist');
                                                                  end if; --ahott 18-12-2003
                                                          exception
                                                                  when no_data_found then
                                                                           dbms_output.put_line('No es restitucion');
                                                          end;
                              ...
                              AND COD_SERVICIO = v_cod_servicio;
                                                          --Fin ahott 23-12-2003 CH-021220031546
                              COMMIT;
                        ...
                        THEN --serie no encontro, genera movimiento
                           BEGIN
                              ...
                              THEN
                                 ...
                                 IF v_filas_ant_equ = 1 THEN
                                    SELECT num_serie,   tip_terminal,   num_imei, cod_tecnologia --COL-36436|04-01-2007|AHA
                                      INTO v_serie_ant, v_terminal_ant, v_imei_ant, v_tec_dma --COL-36436|04-01-2007|AHA
                                      FROM ga_equipaboser
                                     WHERE num_abonado = v_abonado
                                       AND fec_alta IN
                                                      (SELECT MAX (fec_alta)
                                 ...
                                 ELSE
                                    SELECT num_serie,   tip_terminal,   num_imei, cod_tecnologia --COL-36436|04-01-2007|AHA
                                      INTO v_serie_ant, v_terminal_ant, v_imei_ant, v_tec_gsm --COL-36436|04-01-2007|AHA
                                      FROM ga_equipaboser
                                     WHERE num_abonado = v_abonado
                                       AND fec_alta IN
                                                      (SELECT MAX (fec_alta)
                                 ...
                                 SELECT cod_tecnologia
                                  ...
                                  WHERE cod_central = v_codcentral_ori
                                    AND cod_producto = v_producto;
                                -- Hom. 05-05-2004 PAGC - Inc.: TM-200403150568
                                 /*SELECT count(*)
                                   INTO v_filas_nue_equ
                                   FROM ga_equipaboser
                                  WHERE num_abonado=v_abonado
                                  And fec_alta = v_fec_cambio;*/
                                --Fin Hom. TM-200403150568
                                 ...
                                 IF v_tecnologia_nue = v_tecnologia_ant AND
                                       ...
                                       --TMM-04026 Fin
                                        -- Hom. 05-05-2004 PAGC - Inc.: TM-200403150568
                                                SELECT decode(cod_tipmodi,'RE','CE',cod_tipmodi)
                                                INTO v_actabo
                                                from ga_modabocel
                                                where num_abonado =  v_abonado
                                                AND cod_tipmodi in ('CE', 'TG', 'GT', 'ET', 'EG','SA','RE')
                                                AND nvl(num_serie,NVL(v_serie_ant,'0')) = NVL(v_serie_ant,'0')
                                                AND nvl(num_imei, NVL(v_imei_ant,'0' )) = NVL(v_imei_ant, '0')
                                                AND TRUNC (fec_modifica) = TRUNC (v_fec_cambio)
                                                AND ROWNUM=1
                                                Order by fec_modifica desc;
                                         /* IF v_filas_nue_equ = 1 THEN
                                            v_actabo := 'SA';
                                         ELSE
                                            v_actabo := 'CE';
                                         END IF; */
                                        -- Fin Hom - Inc.: TM-200403150568
                                 --TMM-04026 Ini
                              ...
                              vp_act := 'S';
                                      ...
                                      SELECT cod_central
                                       ...
                                       WHERE num_abonado = v_abonado
                                         ...
                                         AND TRUNC (fec_modifica) = TRUNC (v_fec_cambio)
                                                                 ...
                                                                 --Insertar movimiento de desactivación de SS solo cuando
                                                                 -- exista un cambio de tecnología.
                                                                 if v_actabo='TG' or v_actabo='EG' or v_actabo='GT' or v_actabo='ET' then
                                                                        PV_RESTAURA_CAMBTEC_SS_PR(v_abonado,v_fec_cambio,'C',v_cadenaact,v_cadenades,v_error,v_sqlcode);
                                                                        if v_error='0' then
                                                                                        v_camb_tec_ss:=ge_fn_devvalparam('GA',1,'CAMBIO_TEC_SS');
                                                                                        if v_camb_tec_ss = '1' then

                                                                                                 SELECT cod_actcen INTO v_actcenSS
                                                                 FROM ga_actabo
                                                                 WHERE cod_producto   = 1
                                                         AND cod_modulo     = 'GA'
                                                         AND cod_actabo     = 'SS'
                                                         AND cod_tecnologia = v_tecnologia_ant;--v_tecnologia_nue;
                                                                                                ...
                                                                                                insert into
                                                                                                icc_movimiento(cod_estado,    num_movimiento,
                                                                                ...
                                                                                tip_terminal,
                                                                                                                                ind_bloqueo,
                                                                                tip_tecnologia,cod_servicios,Num_min)
                                                                                                           values
                                                                                                                         (1,            icc_seq_nummov.NEXTVAL,
                                                                         v_abonado,     'GA',
                                                                                                                         v_actcenSS,    'MIGRACION',
                                                                                                                         SYSDATE,       cod_central_modabo,
                                                                         v_celular,
                                                                                                                         --TMM-04026 Ini
                                                                         ...
                                                                         Decode(GA_APROVISIONAR_CENTRAL_PG.PV_GRUPO_TECNOLOGICO_FN(v_tecnologia_ant),GA_APROVISIONAR_CENTRAL_PG.PV_GRUPO_TECNOLOGICO_FN(v_tec_gsm),v_imsi_ant,NULL),
                                                                                                                         --Decode(v_tecnologia_nue,v_tecnologia_gsm,v_imsi_nue,NULL),
                                                                         Decode(GA_APROVISIONAR_CENTRAL_PG.PV_GRUPO_TECNOLOGICO_FN(v_tecnologia_ant),GA_APROVISIONAR_CENTRAL_PG.PV_GRUPO_TECNOLOGICO_FN(v_tec_gsm),v_imei_ant,NULL),
                                                                                                                         --Decode(v_tecnologia_nue,v_tecnologia_gsm,v_imei_nue,NULL),
                                                                                          --Decode(v_tecnologia_nue,v_tecnologia_gsm,Decode(v_actabo,'SA',NULL,v_imei_nue),NULL),
                                                                         Decode(GA_APROVISIONAR_CENTRAL_PG.PV_GRUPO_TECNOLOGICO_FN(v_tecnologia_ant),GA_APROVISIONAR_CENTRAL_PG.PV_GRUPO_TECNOLOGICO_FN(v_tec_gsm),v_icc_ant,NULL),
                                                                                                                         --Decode(v_tecnologia_nue,v_tecnologia_gsm,v_icc_nue,NULL),
                                                                                                                         'SS',
                                                                                                                         0,'PENDIENTE SS CAMBIO SERIE REST.',
                                                                     ...
                                                                     v_terminal_ant),
                                                                                         0,
                                                                                                                                         v_tecnologia,
                                                                                                                                         v_cadenades,v_NumMin
                                                                                                                         );
                                                                                         end if;--Control resultado parametro
                                                                        else
                                                                                vp_error:=v_error;
                                                                                raise error_proceso;
                                                                        end if; --Control de error
                                                                 end if;--Para cambio tecnologia
                                 ...
                                 INSERT INTO icc_movimiento
                                             (cod_estado,    num_movimiento,
                                              ...
                                              tip_terminal,
                                              ind_bloqueo,
                                              tip_tecnologia,Num_Min,cod_servicios)
                                 VALUES       (1,            icc_seq_nummov.NEXTVAL,
                                              ...
                                              v_celular,
                                                                                        --TMM-04026 Ini
                                                                                          Decode(v_actabo,'SA',v_serie_icc,Decode(GA_APROVISIONAR_CENTRAL_PG.PV_GRUPO_TECNOLOGICO_FN(v_tecnologia_ant),GA_APROVISIONAR_CENTRAL_PG.PV_GRUPO_TECNOLOGICO_FN(v_tec_dma),v_serie_icc,' ')),
                                              Decode(v_actabo,'SA',v_serie_nue_icc,Decode(GA_APROVISIONAR_CENTRAL_PG.PV_GRUPO_TECNOLOGICO_FN(v_tecnologia_nue),GA_APROVISIONAR_CENTRAL_PG.PV_GRUPO_TECNOLOGICO_FN(v_tec_dma),v_serie_nue_icc,NULL)),
                                              Decode(GA_APROVISIONAR_CENTRAL_PG.PV_GRUPO_TECNOLOGICO_FN(v_tecnologia_ant),GA_APROVISIONAR_CENTRAL_PG.PV_GRUPO_TECNOLOGICO_FN(v_tec_gsm),v_imsi_ant,NULL),
                                              ...
                                              Decode(GA_APROVISIONAR_CENTRAL_PG.PV_GRUPO_TECNOLOGICO_FN(v_tecnologia_ant),GA_APROVISIONAR_CENTRAL_PG.PV_GRUPO_TECNOLOGICO_FN(v_tec_gsm),v_imei_ant,NULL),
                                                                                          Decode(GA_APROVISIONAR_CENTRAL_PG.PV_GRUPO_TECNOLOGICO_FN(v_tecnologia_nue),GA_APROVISIONAR_CENTRAL_PG.PV_GRUPO_TECNOLOGICO_FN(v_tec_gsm),v_imei_nue,NULL),
                                                                                          --Decode(v_tecnologia_nue,v_tecnologia_gsm,Decode(v_actabo,'SA',NULL,v_imei_nue),NULL),
                                              Decode(GA_APROVISIONAR_CENTRAL_PG.PV_GRUPO_TECNOLOGICO_FN(v_tecnologia_ant),GA_APROVISIONAR_CENTRAL_PG.PV_GRUPO_TECNOLOGICO_FN(v_tec_gsm),v_icc_ant,NULL),
                                              ...
                                              0,             'PENDIENTE CAMBIO SERIE REST.',
                                              DECODE (v_terminal_ant,
                                                                     NULL, 'D',
                                                                     v_terminal_ant),
                                              0,
                                              v_tecnologia,v_NumMin,
                                                                                          DECODE(v_actabo,'CE',NULL,
                                                                                                                          'SA',NULL,
                                                                                                                                                v_clase_servicio));
                                 p_ins_audrest (
                                    v_producto,
                                    v_abonado,
                                    'P_DEL_CAMSERIE(PASA MOV.)',
                                    vp_error
                                 );
                                 ...
                                 END IF;
                                                                 ...
                                                                 --Inicio ahott 29-12-2003 CH-021220031546
                                                                 begin
                                                                        SELECT cod_tipmodi -- ahott 26-01-2004 CH-200401231637
                                                                        INTO v_cod_tipmodi -- ahott 26-01-2004 CH-200401231637
                                                                        FROM ga_modabocel
                                         ...
                                         -- Debemos finalizar la restitución
                                                                         begin
                                                 SELECT NUM_SINIESTRO,COD_CAUSA
                                                 into v_siniestro,v_causa_sinies
                                                 FROM GA_SINIESTROS
                                                ...
                                                --TMM-04026 Fin
                                                 ...
                                                 DELETE GA_SINIESTROS WHERE NUM_SINIESTRO=v_siniestro;
                                                                         exception
                                                                                when no_data_found then
                                                                                         dbms_output.put_line('Datos no encontrados de siniestros');
                                                                         end;
                                                                 exception
                                                                         when no_data_found then
                                                                                  dbms_output.put_line('Corresponde a cambio de serie no a restitucion');
                                                                 end;
                                                                 --Fin ahott 29-12-2003 CH-021220031546

-- ****************
                                 BEGIN
                                    IF v_ind_procequi = 'E'
                                    THEN
                                       SELECT distinct num_venta --ahott 20-11-2003 HB-311020030133_HB-311020030134
                                         INTO v_venta
                                         FROM ge_cargos
                                        WHERE cod_cliente = v_cliente
                                          AND num_abonado = v_abonado
                                          AND fec_alta > v_fec_cambio;
                                                                                  dbms_output.PUT_LINE('ge_cargos');
                                                                                  /*AND TRUNC (fec_alta) =
                                       ...
                                       SELECT num_venta
                                        ...
                                        WHERE cod_cliente = v_cliente
                                          ...
                                          AND num_serie IN (v_serie_nue,v_imei_nue)
                                                                                  AND fec_alta >= v_fec_cambio --ahott 22-12-2003 CH-021220031546
                                          AND ROWNUM = 1;
                                    END IF;
                                       ...
                                       v_venta := 0;
                                                                           ...
                                                                           IF v_imei_nue IS NOT NULL and v_actabo!='SA'  THEN --Tecnologia de serie nueva es GSM
                                                                                ...
                                                                                if vp_error='0' then
                                                                                                                                                                        ...
                                                                                                                                                                        begin --Inicio ahott 17-12-2003 CH-021220031546
                                                                                                                                              SELECT cod_tipmodi -- ahott 26-01-2004 CH-200401231637
                                                                                                                                                INTO v_cod_tipmodi -- ahott 26-01-2004 CH-200401231637
                                                                                                                                                FROM ga_modabocel
                                                                                                                                               WHERE num_abonado = v_abonado
                                                                                           ...
                                                                                           if vp_error<>'0' then
                                                                                                        rollback;
                                                                                                                                                                                                ...
                                                                                                                                                                                                begin
                                                                                                                                                                                                        ...
                                                                                                                                                                                                        dbms_output.put_line('v_siniestro = '||v_siniestro||' v_causa_sinies = '||v_causa_sinies);
                                                                                                                                        ...
                                                                                                                                        FEC_FORMALIZA, FEC_ANULA, NUM_CONSTPOL, NUM_SOLLIQ, NUM_SERIEREP,IND_SUSP,TIP_TERMINAL
                                                                                                                                        FROM GA_HSINIESTROS WHERE NUM_SINIESTRO = v_siniestro);
                                                                                                                                                                                                        dbms_output.put_line('Insert select ga_siniestros');
                                                                                                                                        DELETE GA_HSINIESTROS WHERE NUM_SINIESTRO=v_siniestro;
                                                                                                                                        ...
                                                                                                                                        OBS_DETALLE
                                                                                                                                        FROM GA_HDETSINIE WHERE NUM_SINIESTRO = v_siniestro);
                                                                                                                                                                                                        dbms_output.put_line('Insert select ga_detsinie');
                                                                                                                                        DELETE GA_HDETSINIE where NUM_SINIESTRO = v_siniestro;
                                                                                                                                                                                                exception
                                                                                                                                                                                                        when no_data_found then
                                                                                                                                                                                                                 dbms_output.put_line('Siniestro no encontrado en historico');
                                                                                                                                                                        ...
                                                                                                                                                                        AND COD_SERVICIO = v_cod_servicio;
                                                                                                                                                                                ...
                                                                                                                                                                                -- Fin ahott 17-12-2003 CH-021220031546
                                                                                                                                                                        exception --ahott 17-12-2003 CH-021220031546
                                                                                                                                                                            when no_data_found then --ahott 17-12-2003 CH-021220031546
                                                                                        ...
                                                                                        P_DESHACER_ABO (
                                                                                           ...
                                                                                           if vp_error<>'0' then
                                                                                                        rollback;
                                                                                                                                                                        ...
                                                                                                                                                                        end; --ahott 17-12-2003 CH-021220031546
                                                                                ELSE
                                                                                        rollback;
                                                                                end if;
                                 END;
-- ****************
      ...
      END IF; -- busca abonado CSP en Abocel, ya que no existe movimiento pendiente
          ELSE -- Ahott 19-02-2004 CH-200402161668
                  dbms_output.put_line('no se toman cambios de serie de servicio tecnico'); -- Ahott 19-02-2004 CH-200402161668
          END IF; -- Ahott 19-02-2004 CH-200402161668
   END LOOP;
         ...
         BEGIN
            ...
            THEN
               BEGIN
                  vp_tabla := 'AL_SERIES';
                  vp_act := 'S';
                  -- Se comprueba si la serie es Interna se debe encontrar en Almacin.
                  SELECT cod_estado
                    INTO v_estado
                    FROM al_series
                   WHERE num_serie = v_serie_nue;
               ...
               BEGIN
                  ...
                  SELECT COUNT(*) INTO v_filas_ant_equ
                   ...
                   WHERE num_abonado  = v_abonado AND
                         fec_alta     IN (SELECT MAX(fec_alta)
                                            FROM ga_equipaboser b
                                           WHERE  b.num_abonado = v_abonado );
                  If v_filas_ant_equ = 1 THEN
                        SELECT num_serie, tip_terminal, fec_alta,
                               ind_eqprestado, num_imei
                          INTO v_serie_nue, v_terminal_nue, v_fec_alta, v_imei_nue,
                               v_eqprestado
                  ...
                  ELSIF v_filas_ant_equ = 2 THEN
                        SELECT num_serie, tip_terminal, fec_alta,
                               ind_eqprestado,num_imei
                          INTO v_serie_nue, v_terminal_nue, v_fec_alta,
                               v_eqprestado, v_imei_nue
                          FROM ga_equipaboser a
                         WHERE num_abonado = v_abonado
                           AND fec_alta = (SELECT MAX (fec_alta)
                                             FROM ga_equipaboser b
                                            WHERE a.num_abonado = b.num_abonado)
                           AND num_imei IS NOT NULL;
                  ELSE  -- Esto porque si no encuentra registros entregra de todas maneras un cero y no gatillara
                        -- la excepcion que correepondma en la versisn anterior
                      RAISE NO_DATA_FOUND;
                  END IF;

                  If v_imei_nue IS NOT NULL THEN
                      v_serie_hex_ant := '0';  --este valor no tiene importancia cuando es GSM
                      --TMM-04026 Ini
                      v_tecnologia := v_tec_gsm;
                      --TMM-04026 Fin
                  ELSE
                  ...
                  UPDATE ga_abocel
                     SET num_serie        = v_serie_nue,
                         tip_terminal     = v_terminal_nue,
                         num_seriehex     = v_serie_hex_ant,
                         ind_eqprestado   = DECODE (v_eqprestado, NULL, 0, v_eqprestado),
                         fec_prorroga     = NULL,
                         cod_situacion    = 'AAA',
                         cod_tecnologia   = v_tecnologia,
                         num_imei         = v_imei_nue
                  ...
                  THEN
                     vp_tabla := 'FA_HISTDOCU';
                     vp_act := 'U';
                     DBMS_OUTPUT.put_line ('P_Histdocu ');
                     p_upd_histdocu (v_cliente, v_venta, vp_error);
