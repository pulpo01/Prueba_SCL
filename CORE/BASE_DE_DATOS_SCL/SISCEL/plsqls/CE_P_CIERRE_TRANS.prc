CREATE OR REPLACE PROCEDURE        CE_P_CIERRE_TRANS(
	num_atm	IN NUMBER,
	num_oper IN NUMBER,
	monto IN NUMBER,
...
IF cod_trans='NK' THEN
  ...
  ELSE
	...
	num_atm, num_oper, empresa, COD_SCRIPT, NUM_LOTE,
	FEC_ACTIVA, SYSDATE, mot_rechazo, NUM_OPACTIVA, FEC_TRANSACCION
	FROM CET_CLAVEDISP WHERE NUM_OPERACION=num_oper
	AND NUM_ATMOPER=num_atm;
...
IF cod_trans='OK' THEN
	pnum_celular:=to_char(to_number(num_celular));
	IF num_celular = NULL OR num_celular='' THEN
	   pnum_celular:='0';
	END IF;
	OPEN DATOS;
	marca :=1;
 ...
 IF resul='0000' THEN
	marca :=3;
	--Prepara el Insert1 si transaccisn es correcta, agrega la
	--clave a tabla que contiene las claves asignadas
	INSERT INTO CEH_CLAVEDISP
	(NUM_CLAVE,
	NUM_SERIE,
	FEC_EXPIRACION,
	...
	FEC_TRANSACCION)
	SELECT NUM_CLAVE, NUM_SERIE, FEC_EXPIRACION,
	COD_TIPCLAVE, MTO_CLAVE, 'V', num_atm, num_oper, empresa, COD_SCRIPT, NUM_LOTE,
	sysdate, '', '', 0, FEC_TRANSACCION
	FROM CET_CLAVEDISP WHERE NUM_OPERACION=num_oper
	AND NUM_ATMOPER=num_atm;
	...
	0,
	empresa_fin,
	empresa , --'REDBN' -- Esto se debe cambiar por empresa una vez que este definido el csdigo
