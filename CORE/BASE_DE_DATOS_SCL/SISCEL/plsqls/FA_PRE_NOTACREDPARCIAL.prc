CREATE OR REPLACE PROCEDURE Fa_Pre_Notacredparcial( vp_secuencia_ga IN VARCHAR2,
 vp_secuencia IN VARCHAR2,
 vp_origenejecuta IN VARCHAR2 default ' ')
IS
/*
<Documentación TipoDoc = "Procedimiento">
<Elemento Nombre = "Fa_Pre_Notacredparcial" Lenguaje="PL/SQL" Fecha="" Versión"1.0.0" Diseñador="" Programador="" Ambiente="BD">
<Retorno>NA</Retorno>
<Descripción>"Procedimiento encargado de generar la nota de crédito para una devolución y otros... " </Descripción>
<Parámetros>
<Entrada>
<param nom="vp_secuencia_ga" Tipo="varchar2">"Corresponde a la Secuencia GA.Abonados"</param>
<param nom="vp_secuencia" Tipo="varchar2">"Corresponde a la Secuencia N.C.Parcial "</param>
<param nom="vp_origenejecuta" Tipo="varchar2">"Corresponde a opción de ejecución del PL, 'NP' es de componente PryCreditoCliente "</param>
</Entrada>
<Salida>
</Salida>
</Parámetros>
...
<Descmod>"Incidencia: 64611, Se agrega tercer parametro en la lista de parametros, para determinar el origen de invocación del PL " </Descmod>
...
vp_error            NUMBER(2);
usuario             VARCHAR2(30);
gls_error           VARCHAR2(255); -- 15-04-2008 ZMH 64611
fecha               VARCHAR(10);
...
-- el ultimo fue Msj42,
manejo_nc_descuento NUMBER; -- Inc 45207 PPV 27/12/2007 Se incorpora parametro de manejo de descuentos al igual que la NC Total
...
BEGIN
    ...
    -- Inc 45207 PPV 27/12/2007 Se incorpora parametro de manejo de descuentos al igual que la NC Total
    BEGIN
      SELECT VAL_NUMERICO
...
--------------- Selecciona los Datos de tablas Nociclicas
        ...
        IF vp_tipo_tabla = 'N' THEN
           ...
           BEGIN
              -- Inc 45207 PPV 27/12/2007 Se agregan opciones de manejo de descuentos idem NC Total
              IF manejo_nc_descuento = 0 THEN
                      INSERT INTO FA_AJUSTECONC
                    (PREF_PLAZA, NUM_FOLIO, COD_CONCEPTO, COLUMNA, COD_PRODUCTO, FEC_EFECTIVIDAD, IMP_CONCEPTO,
                                         COD_MONEDA, IMP_FACTURABLE, COD_REGION, COD_PROVINCIA, COD_CIUDAD, COD_MODULO, NUM_UNIDADES,
                                         NUM_SERIEMEC, COD_TIPCONCE, FEC_VALOR, IMP_MONTOBASE, IND_FACTUR, IND_SUPERTEL, NUM_ABONADO,
                                         COD_PORTADOR, DES_CONCEPTO, SEQ_CUOTAS, NUM_CUOTAS, ORD_CUOTA, NUM_SERIELE, MES_GARANTIA, NUM_GUIA,
                                         IND_ALTA, NUM_PAQUETE, NUM_VENTA, NUM_TRANSACCION, FLAG_IMPUES, FLAG_DTO, COD_CLIENTE, COD_TIPDOCUM)
                     SELECT vp_pref_plaza, vp_num_folio, a.COD_CONCEPTO, a.COLUMNA, a.COD_PRODUCTO,    a.FEC_EFECTIVIDAD, 0, moneda,
                     a.IMP_FACTURABLE, a.COD_REGION, a.COD_PROVINCIA, a.COD_CIUDAD, b.COD_MODULO, a.NUM_UNIDADES,    a.NUM_SERIEMEC,
                     a.COD_TIPCONCE, a.FEC_VALOR, a.IMP_MONTOBASE, a.IND_FACTUR, c.ind_supertel,    a.NUM_ABONADO, a.COD_PORTADOR,
                      a.DES_CONCEPTO,    a.SEQ_CUOTAS, a.NUM_CUOTAS, a.ORD_CUOTA, a.NUM_SERIELE, a.MES_GARANTIA, a.NUM_GUIA,
                      a.IND_ALTA,    a.NUM_PAQUETE, c.NUM_VENTA, c.NUM_TRANSACCION,    a.FLAG_IMPUES, a.FLAG_DTO,    c.COD_CLIENTE,
                      C.COD_TIPDOCUM
                     FROM FA_CONCEPTOS              b,
                     ...
                     AND a.COD_TIPCONCE     <> 1
                       AND ( a.COD_TIPCONCE <> 2 OR ( a.COD_TIPCONCE=2 AND (COD_CONCEREL=0 OR COD_CONCEREL IS NULL))); --Inc 45207 PPV 27/12/2007
              ELSE
                   INSERT INTO FA_AJUSTECONC
                    (PREF_PLAZA, NUM_FOLIO, COD_CONCEPTO, COLUMNA, COD_PRODUCTO, FEC_EFECTIVIDAD, IMP_CONCEPTO,
                                         COD_MONEDA, IMP_FACTURABLE, COD_REGION, COD_PROVINCIA, COD_CIUDAD, COD_MODULO, NUM_UNIDADES,
        ...
        /* INC 64178 PPV 27/08/2008 Se quita actualizacion sopbre campo imp_concepto ya que este solo debe tener los ajustes realizados*/
...
--        --Fin PGG, RAO, LOD , FAR, JPO 12/08/2005
        ELSE
            ...
            ins_ajuste := ins_ajuste || '0,:moneda,';
            ...
            -- Inc 45207 PPV 27/12/2007 Se incorpora logica de la NC Total
             IF manejo_nc_descuento = 0 THEN
                BEGIN
                  ins_ajuste := '';
...
-------Grabar impuestos P-CSR-12019
         BEGIN
             ...
             IF cantidad = 0 THEN
                  ...
                  IF vp_tipo_tabla = 'N' THEN
                        ...
                        SELECT C.NUM_FOLIO, C.PREF_PLAZA, C.COD_CLIENTE, A.NUM_ABONADO, B.COD_TIPIMPUES, SUM(A.IMP_FACTURABLE),sum(e.IMP_FACTURABLE), SUM(a.IMP_MONTOBASE)
                        FROM fa_factconc_nociclo a, GE_IMPUESTOS B, FA_FACTDOCU_NOCICLO C, FA_GRPSERCONC D,   FA_AJUSTECONC e
                        ...
                        AND  A.COD_CONCEREL = E.COD_CONCEPTO
                        AND  A.COLUMNA_REL = E.COLUMNA
                       AND C.NUM_FOLIO = E.NUM_FOLIO
                       AND C.PREF_PLAZA = E.PREF_PLAZA
                  ...
                  ELSE
                      ...
                      lv_sql := lv_sql || 'SELECT C.NUM_FOLIO, C.PREF_PLAZA, C.COD_CLIENTE, A.NUM_ABONADO, B.COD_TIPIMPUES,  SUM(A.IMP_FACTURABLE), SUM(e.IMP_FACTURABLE), SUM(a.IMP_MONTOBASE) ';
                      lv_sql := lv_sql || 'FROM ' || TRIM(vp_tabla) || ' a, GE_IMPUESTOS B, FA_HISTDOCU C, FA_GRPSERCONC D ,  FA_AJUSTECONC e ';
                      ...
                      lv_sql := lv_sql || 'AND  A.COD_CONCEREL = E.COD_CONCEPTO ';
                      lv_sql := lv_sql || 'AND  A.COLUMNA_REL = E.COLUMNA ';
                      lv_sql := lv_sql || ' AND C.NUM_FOLIO = E.NUM_FOLIO ';
                      lv_sql := lv_sql || 'AND C.PREF_PLAZA = E.PREF_PLAZA ';
                      lv_sql := lv_sql || 'AND C.COD_CLIENTE = E.COD_CLIENTE ';
                      lv_sql := lv_sql || 'AND A.NUM_ABONADO= E.NUM_ABONADO ';
           ...
           EXCEPTION
                 WHEN OTHERS THEN
                          vp_error := 7;
                          gls_error := 'Error.No se puede actualizar tabla FA_DETIMPTOSTIPO_TO Msj36';
                          RAISE ERROR_PROCESO;
         END ;
          --FIN P-CSR-12019
    -- Inc 64178 Se reincorpora validacion PPV 15/04/2008
    ...
    IF vp_total > 0 THEN
            ...
            RAISE ERROR_PROCESO;
    END IF;
    -- Fin Inc 64178 Se reincorpora validacion PPV 15/04/2008
...
-- Incidencia: 64611, Fecha: 37-07-2006, Descripción: Cambio para obtener el usuario que aprobó la devolución. Responsable : Zenén Muñoz H.
    if vp_origenejecuta = 'NP' then
       sTabla := 'FA_NCPARCIAL';
       SELECT MIN(NOM_USUARIO), MIN(RTRIM(TO_CHAR(SYSDATE, sFormatoFecha))) INTO usuario, fecha
       FROM FA_NCPARCIAL
       WHERE NUM_SECUENCIA = vp_secuencia;
    else
       usuario := USER;
...
--------------- bInsertAplicadoNC
    /*    BEGIN
        ...
        EXCEPTION
            WHEN OTHERS THEN
                 vp_error := 3;
                 gls_error := 'Error. No pudo encontrar en la tabla FA_APLICADONC. Msj35';
                 RAISE ERROR_PROCESO;
        END; */     /* Se quita en incidencis RA-200603080881 PPV 21/03/2006 esta demas ya que vp_num_dias viene de antes*/
        ...
        BEGIN
             ...
             VALUES (    usuario,
                    ...
                    TO_DATE(TO_CHAR(SYSDATE + vp_num_dias ,'DD-MM-YYYY') || ' 23:59:59','DD-MM-YYYY HH24:MI:SS'),
                    vp_totparcial);
        EXCEPTION
                 WHEN DUP_VAL_ON_INDEX THEN
                        vp_error := 3;
...
------- bUpdSecuencia
    ...
    EXCEPTION
      WHEN ERROR_PROCESO THEN
           gls_error := gls_error || '. NC Parcial N° Proceso : ' || vp_num_proceso; -- 64611  15-04-2008 ZMH
           IF vp_error = 0 THEN
      ...
      WHEN DUP_VAL_ON_INDEX THEN
           INSERT INTO GA_TRANSACABO
                      (NUM_TRANSACCION, COD_RETORNO, DES_CADENA)
           VALUES (TO_NUMBER(vp_secuencia_ga), 1, 'Duplicado'|| sTabla || '. NC Parcial N° Proceso : ' || vp_num_proceso);  -- 64611  15-04-2008 ZMH
      WHEN NO_DATA_FOUND THEN
           INSERT INTO GA_TRANSACABO
                      (NUM_TRANSACCION, COD_RETORNO, DES_CADENA)
     ...
     WHEN OTHERS THEN
           ...
           VALUES (TO_NUMBER(vp_secuencia_ga), 1, 'Error desconocido ' || sTabla || '. NC Parcial N° Proceso : ' || vp_num_proceso);  -- 64611  15-04-2008 ZMH

END Fa_Pre_Notacredparcial;
/
