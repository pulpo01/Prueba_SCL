CREATE OR REPLACE FUNCTION        cr_fn_genera_arch_abocel_tmc RETURN VARCHAR2 IS
--
...
BEGIN
	...
	v_ssql:= 'SELECT B.COD_CLIENTE, B.NUM_EVENTO, c.cod_tipdocum, C.DES_TIPDOCUM, B.NUM_FOLIO, '||
			 ...
			 'E.NOM_CALLE , E.NUM_CALLE, E.NUM_PISO, E.NUM_CASILLA, E.OBS_DIRECCION, G.DES_COMUNA, H.DES_CIUDAD, I.NUM_CELULAR '||
...
EXCEPTION
   ...
   WHEN VALUE_ERROR THEN
   		RETURN 'ERROR 1 cr_fn_genera_arch_abocel_tmc, SQLCODE:'||TO_CHAR(SQLCODE)||' SQLERRM:'||SQLERRM;

   WHEN INVALID_NUMBER THEN
   		RETURN 'ERROR 2 cr_fn_genera_arch_abocel_tmc, SQLCODE:'||TO_CHAR(SQLCODE)||' SQLERRM:'||SQLERRM;

   WHEN OTHERS THEN
      RETURN 'ERROR 3 cr_fn_genera_arch_abocel_tmc, SQLCODE:'||TO_CHAR(SQLCODE)||' SQLERRM:'||SQLERRM;
END;
/
SHOW ERRORS
