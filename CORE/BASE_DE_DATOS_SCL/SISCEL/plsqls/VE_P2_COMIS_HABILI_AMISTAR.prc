CREATE OR REPLACE PROCEDURE          "VE_P2_COMIS_HABILI_AMISTAR" (
...
VE_P2_calcula_habili_amistar(vp_cod_vendedor,vp_num_liquidacion,vp_producto,vp_ctoliq,vp_cod_tip_cuad,vp_cat_vendedor,vp_tip_vendedor,vp_ind_nivel,vp_fecha_inicial,vp_fecha_final,vp_num_habi_amistar);
vp_num_total_ventas := vp_num_habi_amistar;
vp_importe_por_venta:=0;
vp_importe_total:=0;
vp_meta:=0;
IF vp_num_total_ventas >0 THEN
  IF vp_cod_tip_cuad = '1102' THEN  -- % DE META POR META
    ...
    WHERE VE_METAVEND.COD_TIPCOMIS= vp_tip_vendedor AND
     ...
     vp_fecha_final between VE_METAVEND.FEC_VALOR and nvl(VE_METAVEND.FEC_FINVALOR, to_date('99991231','YYYYMMDD')) AND
   ...
   VE_P2_frac_rango_porciento(vp_cod_cuadrante, vp_num_total_ventas,vp_meta,vp_importe_total);
  ELSE  -- 2202
   vp_tabla:='VE_CUADCOMIS';
...
END VE_P2_comis_habili_amistar;
/
SHOW ERRORS
