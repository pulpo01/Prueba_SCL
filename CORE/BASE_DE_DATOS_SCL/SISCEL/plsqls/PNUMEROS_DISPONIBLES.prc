CREATE OR REPLACE PROCEDURE PNUMEROS_DISPONIBLES(P_TRANSACCION in varchar2 ,p_num_desde in NUMBER, p_num_aper in NUMBER, p_nros_disp in NUMBER, p_cod_uso in NUMBER, p_cod_cat in NUMBER, p_cod_central in NUMBER, p_cod_subalm in varchar2, p_cod_producto in NUMBER) IS
...
BEGIN
	 ...
	 v_DiasHib:= v_DiasHib + 1 ; --Se agrega 1 para dejar disponible el mismo dia que se solicita
	 ...
	 if SQL%NOTFOUND then
       ...
       WHILE (v_desde < v_hasta) LOOP
	    ...
	    ---P-MIX-09003,24-02.2010, Se omite los dias de hibernacion segun lo solicitado 
        ----------------------------------------------------------------------------------
        --- VALUES(v_desde,p_cod_subalm,p_cod_producto,p_cod_central,p_cod_cat,p_cod_uso,(sysdate - v_DiasHib),1,p_cod_uso);
        VALUES(v_desde,p_cod_subalm,p_cod_producto,p_cod_central,p_cod_cat,p_cod_uso,sysdate,1,p_cod_uso);
...
EXCEPTION
  WHEN no_hay_datos THEN
    v_error:=1;
  WHEN	OTHERS THEN
    v_mensaje:=substr(SQLERRM,1,250); --cambio
    v_error:=1;
