...
CREATE OR REPLACE PROCEDURE pv_nue_rest_valida_vendedor_pr (
...
BEGIN
		 ...
		 IF (LN_ind_interno = 0) THEN
			LO_pv_comis_ooss.ind_interno := LN_ind_interno;
		 END IF;
        ...
        EXCEPTION
		 WHEN NO_DATA_FOUND THEN
             ...
             SN_num_evento := Ge_Errores_Pg.Grabarpl( SN_num_evento, 'VE', SN_cod_retorno||' -- '||SV_mens_retorno, '1.0', USER, 'pv_nue_rest_valida_vendedor_pr', sSql, SQLCODE, V_des_error );
             vMENSAJE := SUBSTR('Error en pv_nue_rest_valida_vendedor_pr: -'||SN_num_evento||'--' || SQLERRM, 1, 255);

         WHEN error_apl_vb THEN
                        bRESULTADO := 'TRUE';
         WHEN OTHERS THEN
            vMENSAJE := SUBSTR('Error en pv_nue_rest_valida_vendedor_pr: ' || SQLERRM, 1, 255);
END;
/
