CREATE OR REPLACE PROCEDURE Co_Imputacion_Depnt_Pr
...
/* Constantes */
c_nCodEvento CONSTANT SC_EVENTO.COD_EVENTO%TYPE := 78;
...
SELECT DISTINCT B.EMP_RECAUDADORA|| '/' ||A.NUM_EJERCICIO   ID_LOTE,
  ...
  FROM CO_ACUM_DEPRECAUDA A, CO_EMPRESAS_REX B
...
BEGIN
	...
	IF (  p_vCodOficina_Arg  IS NULL ) OR ( p_nCodCaja_Arg  IS NULL ) OR ( p_vNumEjercicio_Arg IS NULL )  THEN
		RAISE_APPLICATION_ERROR( -20101, 'CO_IMPUTACION_DEPNT_PR - No se aceptan valores en null');
	END IF;
	...
	v_vUbicError := 'CO_IMPUTACION_DEPNT_PR - Extrae Id Lote - ';
...
EXCEPTION
	WHEN OTHERS THEN
		 ...
		 RAISE_APPLICATION_ERROR( -20107, 'ERROR INESPERADO EN PL CO_IMPUTACION_DEPNT_PR : ORA-'||SQLERRM||'.', TRUE );

END Co_Imputacion_Depnt_Pr;
/
SHOW ERRORS
