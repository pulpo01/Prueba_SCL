CREATE OR REPLACE PACKAGE BODY PV_REORDENAMIENTO_PG AS
-- PL/SQL Specification
--
-- *************************************************************
-- * Paquete            : PV_REORDENAMIENTO_PG
-- * Descripcisn        : Procedimientos Almacenados para procesar Comercialemente Reordenamiento de Cuenta Sin Cambio de Plan
-- * Fecha de creacisn  : 13-01-2003 13:00
-- * Responsable        : Area Postventa
-- *************************************************************

--PV_REORDENAMIENTO_PG v1.0 -- COL-61078|11-01-2008|GEZ
--PV_REORDENAMIENTO_PG v1.1 -- COL-72325|09-12-2008|GEZ
--PV_REORDENAMIENTO_PG v1.2 -- COL 77474|16-02-2009|SAQL
   ...
   IF serror = 'NO' THEN
      ...
      IF serror = 'NO' THEN
         BEGIN
            ...
            INTO v_cod_cargobasico,v_tip_plantarif
            FROM ta_plantarif
...
IF serror = 'NO' THEN
    IF iTipTraspaso = 'IE' OR iTipTraspaso = 'EE' THEN
        ...
        -- Incrementa el n: de abonados de la empresa del nuevo cliente
                BEGIN
    ...
    END IF;
       ...
       IF iTipTraspaso = 'EI' OR iTipTraspaso = 'EE' THEN
           ...
           -- Decrementa el n: de abonados de la empresa del nuevo cliente
           ...
           IF serror = 'NO' THEN
          -- Decrementa el n: de abonados del antiguo cliente
          BEGIN
