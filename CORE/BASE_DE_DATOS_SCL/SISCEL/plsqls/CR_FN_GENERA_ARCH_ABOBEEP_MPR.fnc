CREATE OR REPLACE FUNCTION        cr_fn_genera_arch_abobeep_mpr RETURN VARCHAR2 IS
--
...
BEGIN
	...
	v_ssql:= 'SELECT B.COD_CLIENTE, B.NUM_EVENTO, c.cod_tipdocum, C.DES_TIPDOCUM, B.NUM_FOLIO '||
			 ...
			 ', E.DES_DIREC1 , E.DES_DIREC2, G.DES_COMUNA '||
			 ', H.DES_CIUDAD,E.ZIP, D.TEF_CLIENTE1, D.TEF_CLIENTE2, I.NUM_BEEPER '||
             'FROM  CRH_EVENTOS BB, '||
			 'CRT_DETALLES B,GE_TIPDOCUMEN C,GE_CLIENTES D, GE_DIRECCIONES E, ' ||
             'GA_DIRECCLI F, GE_COMUNAS G, GE_CIUDADES H, GA_ABOBEEP I,fa_histdocu J, CRH_DEVOLUCIONES A ' ||
...
EXCEPTION
   ...
   WHEN VALUE_ERROR THEN
   		RETURN 'ERROR 1 cr_fn_genera_arch_abobeep_mpr, SQLCODE:'||TO_CHAR(SQLCODE)||' SQLERRM:'||SQLERRM;

   WHEN INVALID_NUMBER THEN
   		RETURN 'ERROR 2 cr_fn_genera_arch_abobeep_mpr, SQLCODE:'||TO_CHAR(SQLCODE)||' SQLERRM:'||SQLERRM;

   WHEN OTHERS THEN
      RETURN 'ERROR 3 cr_fn_genera_arch_abobeep_mpr, SQLCODE:'||TO_CHAR(SQLCODE)||' SQLERRM:'||SQLERRM;
END;
/
SHOW ERRORS
