CREATE OR REPLACE PROCEDURE        P_ALTA_TREK_OFI(
...
-- de la red Trek al activarles desde la venta en oficina.
--
   V_PROCED VARCHAR2(25) := NULL;
BEGIN
   V_PROCED := 'P_ALTA_TREK_OFI';
   UPDATE GA_ABOTREK
...
EXCEPTION
   WHEN OTHERS THEN
        RAISE_APPLICATION_ERROR (-20230,V_PROCED||' '||SQLERRM);
END;
/
SHOW ERRORS
