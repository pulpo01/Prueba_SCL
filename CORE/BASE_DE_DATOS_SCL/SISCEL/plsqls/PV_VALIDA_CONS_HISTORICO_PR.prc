CREATE OR REPLACE PROCEDURE PV_VALIDA_CONS_HISTORICO_PR(v_param_entrada IN  VARCHAR2,
...
BEGIN
         ...
         --Ge_Pac_Arreglopr.GE_PR_RetornaArreglo(v_param_entrada, string);
     	 nABONADO := TO_NUMBER(string(5));
			 ...
			 IF sTecnologia <> sTecnologia_GSM AND sTecnologia <> sTecnologia_TDMA THEN
				...
				IF sTecnologia <> sTecnologia_GSM AND sTecnologia <> sTecnologia_TDMA THEN
         	 	   bRESULTADO := 'FALSE';
             	   vMENSAJE   := 'ABONADO NO POSEE LA TECNOLOGIA PARA REALIZAR ESTA CONSULTA (solo GSM o CDMA)';
				END IF;
			 END IF;

			 IF sTecnologia = sTecnologia_GSM THEN
			 	SELECT val_parametro INTO val_param_tecnologia FROM ged_parametros WHERE nom_parametro = 'COD_HISTORICO_GSM';
				--Solo con uno esta apto para esta consulta 0 = false 1 = true
				IF val_param_tecnologia <> 1 THEN
         	 	   bRESULTADO := 'FALSE';
             	   vMENSAJE   := 'LA CONSULTA NO SE ENCUENTRA DISPONIBLE PARA LA TECNOLOGIA DEL CLIENTE - GSM';
				END IF;
			 END IF;
			 IF sTecnologia = sTecnologia_TDMA THEN
			 	SELECT val_parametro INTO val_param_tecnologia FROM ged_parametros WHERE nom_parametro = 'COD_HISTORICO_CDMA';
				--Solo con uno esta apto para esta consulta 0 = false 1 = true
				IF val_param_tecnologia <> 1 THEN
         	 	   bRESULTADO := 'FALSE';
             	   vMENSAJE   := 'LA CONSULTA NO SE ENCUENTRA DISPONIBLE PARA LA TECNOLOGIA DEL CLIENTE - CDMA';
				END IF;
			 END IF;
     ...
     WHEN OTHERS THEN
          bRESULTADO := 'FALSE';
          vMENSAJE   := 'ERROR EN PV_VALIDA_CONS_HISTORICO_PR: NO SE PUEDE REALIZAR ESTA CONSULTA.';
