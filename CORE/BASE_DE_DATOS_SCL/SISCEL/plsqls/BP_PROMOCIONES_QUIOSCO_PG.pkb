...
--
   ...
   WHEN ERROR_PROCESO THEN
     ...
     'Campaña='||p_vcod_campana||',cod_plan'||v_vcod_plan||', Abonado='||v_nnum_abonado, 'S', v_nerror,v_verror);
...
BEGIN
    ...
    --Aplicar Valor con impuesto según zona
           ...
           ELSIF LTRIM(RTRIM(p_vcod_tiplan)) = '2' THEN --PLAN POSTPAGO
              IF LTRIM(RTRIM(p_vcod_tiplan)) <> LTRIM(RTRIM(v_vcod_tiplan)) THEN --Cliente vs PLAN
                 ...
                 IF LTRIM(RTRIM(v_vcod_tiplan)) = '3' THEN --CLIENTE ES HIBRIDO
                     --5 - Plan Pospago ML/Cliente Híbrido o Si ES DF es 0
...
IS
    ...
    EXCEPTION
        ...
        SN_cod_retorno:= 1;
        SV_mens_retorno:= 'Error obteniendo campañias';
        SN_num_evento:= 0; 
