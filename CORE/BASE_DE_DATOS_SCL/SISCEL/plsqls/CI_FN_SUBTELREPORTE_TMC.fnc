CREATE OR REPLACE FUNCTION ci_fn_subtelreporte_tmc (v_fec_inicio varchar2, v_fec_fin varchar2) RETURN VARCHAR2 IS
--
...
EXCEPTION
   ...
   WHEN VALUE_ERROR THEN
                RETURN 'ERROR 1 ci_fn_subtelreporte_tmc, SQLCODE:'||TO_CHAR(SQLCODE)||' SQLERRM:'||SQLERRM;

   WHEN INVALID_NUMBER THEN
                RETURN 'ERROR 2 ci_fn_subtelreporte_tmc, SQLCODE:'||TO_CHAR(SQLCODE)||' SQLERRM:'||SQLERRM;

   WHEN OTHERS THEN
      RETURN 'ERROR 3 ci_fn_subtelreporte_tmc, SQLCODE:'||TO_CHAR(SQLCODE)||' SQLERRM:'||SQLERRM;
END;
/
SHOW ERRORS
