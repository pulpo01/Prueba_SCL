CREATE OR REPLACE PROCEDURE        Z_PROD (p_orden AL_CAB_NUMERACION.NUM_NUMERACION%TYPE)
AS
...
v_ind             TMP_NUM_PROD.ind_serie%type;
...
v_err    NUMBER;
v_cant   NUMBER;
V_INICIAL VARCHAR(50);
V_FINAL VARCHAR(50);

BEGIN
	 SELECT TO_CHAR(SYSDATE, 'dd/mm/yyyy hh24:mm:ss') INTO V_INICIAL FROM DUAL;

     DBMS_OUTPUT.PUT_LINE('INICIO: ' || V_INICIAL);
...
-- AMS 				  p_asigna_numero_ordenes(p_orden,1,v_subalm,v_central,v_uso,v_cant,v_cat,v_err);
