CREATE OR REPLACE procedure PV_REORDENAMIENTOINTAR_PR
...
--
-- *************************************************************
-- * procedimiento      : PV_REORDENAMIENTOINFAC_PR
...
--
   ...
   SELECT ROWID,COD_CLIENTE,NUM_ABONADO,IND_NUMERO,
		  ...
		  --COD_CARGOBASICO,COD_CICLO,COD_PLANCOM,COD_PLANSERV, --inc. 72325-13/12/2008-jjr
            COD_CARGOBASICO,COD_CICLO,COD_PLANCOM,--COD_PLANSERV, --inc. 72325-13/12/2008-jjr
   ...
   CONT_CICLO   NUMBER; -- INI CSR_200605|28-11-2013|JHA

BEGIN
   VP_PROC := 'PV_REORDENAMIENTOINTAR_PR';
   VP_ERROR := '0';
   ...
   IF VP_PRODUCTO = 1 THEN
	  ...
	  IF VP_ABONADO <> 0 THEN
        ...
        --SELECT NUM_SERIE, COD_TECNOLOGIA   			 	--inc. 72325-13/12/2008-jjr
        --INTO  V_NUM_SERIE, V_TECNOLOGIA	  				--inc. 72325-13/12/2008-jjr
		SELECT NUM_SERIE, COD_TECNOLOGIA, COD_PLANSERV		--inc. 72325-13/12/2008-jjr
        INTO  V_NUM_SERIE, V_TECNOLOGIA, V_PLANSERV 		--inc. 72325-13/12/2008-jjr
		FROM  GA_ABOCEL
	  ...
	  -- FIN.MAM 27/05/2003
          ...
          BEGIN
          	 /*HM-200503030010: German Espinoza Z; 03/03/2005. hom: TM-200410261020
             ...
             COD_LIMCONSUMO, COD_CICLO, COD_HOLDING, COD_EMPRESA,COD_USO
                 ...
                 */
		 		...
		 		SELECT  COD_CLIENTE,TIP_PLANTARIF, COD_PLANTARIF, COD_CARGOBASICO,
                COD_LIMCONSUMO, COD_HOLDING, COD_EMPRESA,COD_USO
                INTO  VP_CLIENEW,V_TIPPLANTARIF, V_PLANTARIF, V_CARGOBASICO,
                V_LIMCONSUMO,  V_HOLDING, V_EMPRESA,V_USO
                FROM GA_ABOCEL
                WHERE COD_CLIENTE = VP_CLIENTED
                AND ROWNUM = 1;
                ...
                V_TABORIGEN:='ABOCEL';
		 		...
		 		BEGIN
		      		SELECT cod_ciclo into V_CICLOD
		      		FROM ge_clientes
		      		WHERE cod_cliente= VP_CLIENTED;
		 		EXCEPTION
		      		WHEN OTHERS THEN
          ...
          EXCEPTION
            ...
            WHEN OTHERS THEN
                 ...
                 VP_ERROR := '4';
          END;
	 	  /*FIN - HM-200503030010: German Espinoza Z; 03/03/2005. hom: TM-200410261020*/
      ...
      LOOP
         ...
         VP_ACT := 'F';
                 ...
                 FETCH C1 INTO V_ROWID,V_CLIENTE,V_ABONADO,V_NUMERO,
                       ...
                       --V_CICLO,V_PLANCOM,V_PLANSERV, --inc. 72325-13/12/2008-jjr
					   V_CICLO,V_PLANCOM,--V_PLANSERV, --inc. 72325-13/12/2008-jjr
					   V_GRPSERV,V_GRUPO,V_PORTADOR,V_USO, V_IMPLIMCONSUMO;
         ...
         WHERE ROWID = V_ROWID;
                 ...
                 IF V_TABORIGEN = 'ABOCEL' THEN
                    -- INI CSR_200605|28-11-2013|JHA
                    SELECT COUNT(1)  
                    INTO CONT_CICLO
                    FROM GA_INTARCEL A, FA_CICLFACT B
                    WHERE A.NUM_ABONADO = VP_ABONADO
                    AND A.COD_CLIENTE = VP_CLIENTE
                    AND A.FEC_DESDE > SYSDATE
                    AND A.COD_CICLO = B.COD_CICLO
                    AND SYSDATE BETWEEN B.FEC_DESDEOCARGOS AND B.FEC_HASTAOCARGOS
                    AND (SELECT MAX(FECHA) FROM CI_ORSERV
                            WHERE TIP_INTER = 8
                            AND COD_INTER = VP_CLIENTE
                            AND COD_OS = '10025')  BETWEEN B.FEC_DESDEOCARGOS AND B.FEC_HASTAOCARGOS;
                         

                    IF  CONT_CICLO > 0 THEN
                        ...
                        WHERE COD_CLIENTE = VP_CLIENTE
                        AND NUM_ABONADO = VP_ABONADO
                        AND FEC_DESDE > V_FECHASTA;
                        
                        V_FECHASTA := TO_DATE('31-12-3000', 'DD-MM-YYYY');
                        
                    END IF;
                    -- FIN CSR_200605|28-11-2013|JHA
