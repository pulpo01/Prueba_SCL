CREATE OR REPLACE FUNCTION GE_FN_EJECUTA_RUTINA_BIND (v_modulo VARCHAR2, v_correlativo NUMBER, v_param_entrada VARCHAR2) RETURN VARCHAR2
IS
-- Funcion que llama a otras rutinas en forma dinamica
-- Creacion : 11 de marzo de 2002
-- Modificacion : 23 de mayo de 2002
-- Autor Doris Soto A.
...
-- Selecciona rutina a ejecutar y valida su existencia
   ...
   IF SQLCODE <> 0 THEN
      v_param_salida := 'ERROR GE_FN_EJECUTA_RUTINA_BIND: ged_plantillarutinas';
      RAISE error_proceso;
   END IF;
...
--    Llama a la otra rutina diferenciando Funcion, PL sin parametros y PL con parametros
        ...
        BEGIN
            ...
            ELSE
        	   LOOP
        			...
        			IF contador > 10 THEN
      				   v_param_salida := 'ERROR GE_FN_EJECUTA_RUTINA_BIND: ged_plantillarutinas no acepta mas de 10 PAR';
      				   RAISE error_proceso;
        			   EXIT;
        			END IF;
              END LOOP;
      ...
      ELSIF v_cod_rutina = 'FC' THEN
		...
		-- 	05-04-2004
	  ELSE
            v_param_salida := 'ERROR GE_FN_EJECUTA_RUTINA_BIND: No esta habilitado para la ejecucion de ProC';
            RAISE error_proceso;
	  END IF;
   END IF;
...
-- Control de errores
   ...
   WHEN NO_DATA_FOUND THEN
      RETURN 'ERROR GE_FN_EJECUTA_RUTINA_BIND: No existe esta combinacion de datos';
   WHEN OTHERS THEN
      RETURN 'ERROR GE_FN_EJECUTA_RUTINA_BIND, SQLERRM : ' || SQLERRM;
END;
/
SHOW ERRORS
