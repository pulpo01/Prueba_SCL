CREATE OR REPLACE PROCEDURE CO_IMPUTACION_PAGOS_NA_PR
...
/* Variables */
v_nCodCaja		CO_HISTPAGOS_NA.COD_CAJA%TYPE;
...
BEGIN
       IF GE_FN_DEVVALPARAM('CO', 1, 'SE_CONTABILIZA') = 'S' THEN	/*Soporte RyC CGLagos 01/09/2005 XO-200508300523. */
		...
		/*Obtiene Evento*/
	       ...
	       INTO v_nCodEvento
	       FROM SC_EVENTO
	       WHERE DES_EVENTO = 'PAGOS NO ASOCIADOS';
		...
		(
	       ...
	       SELECT v_nCodEvento									COD_EVENTO,
		   	v_vIdLote										ID_LOTE,
		   	Sc_Conceptogrp_Fn( v_nCodEvento, 'CO_MOTIVOPAGO_NA', COD_MOTIVO_NA, 'GE_OFICINAS', p_vCodOficina_Arg )	COD_CONCEPTO,
		   	SUM(IMP_PAGO)									IMP_CONCEPTO,
		   	COD_OPERADORA_SCL								COD_OPERADORA_SCL
	       FROM CO_HISTPAGOS_NA A
	       WHERE COD_OFICINA = p_vCodOficina_Arg
	       AND COD_CAJA = v_nCodCaja
	       AND NUM_EJERCICIO = p_vNumEjercicio_Arg
	       AND COD_OPERADORA_SCL IS NOT NULL
	       GROUP BY COD_MOTIVO_NA, COD_OPERADORA_SCL;
