CREATE OR REPLACE PROCEDURE CO_P_APLICA_CAMBIO(
...
/*  Parametros de entrada:
			    vp_Secuencia_Co     ->Numero de Secuencia del Documento
...
BEGIN
	...
	vp_gls_error := 'Select Co_interfaz_caja';
        ...
        IF ltrim(vp_Tipo_Movimiento)='SCD' THEN
			vp_ruteo := 16;
...
/**************************************************************************/
			    ...
			    INSERT INTO CO_DOCUMENTOS (
			       ...
			       LOOP
    				  ...
    				  vp_ruteo := 12;
    				  vp_gls_error:='Insercion en la tabla co_det_documentos';
    	...
    	/**		22/01/2002   RRI
			   ...
			   SELECT CO_SEQ_PAGOCHEQUE.NEXTVAL INTO cart_secuencia FROM DUAL;
			       vp_ruteo := 23;
			       vp_gls_error:='Insercion en la tabla co_cancelados';
			       INSERT INTO CO_CANCELADOS (COD_CLIENTE,
					  NUM_SECUENCI,
					  COD_TIPDOCUM,
					  COD_VENDEDOR_AGENTE,
				   ...
				   WHERE
					 ...
					 SEC_CUOTA           = CODOCUMENTOS.NUM_SEC_CUOTA;
	       			vp_ruteo := 25;
	       			vp_gls_error:='Insercion en la tabla CO_PAGOSCONC';
	       		        INSERT INTO CO_PAGOSCONC (
		  	  		COD_TIPDOCUM,
		  	  		COD_CENTREMI,
		  	  		NUM_SECUENCI,
...
/*			     end if; 			 */
			    vp_ruteo := 25;
			    vp_gls_error:='actualizacion de la tabla co_documentos';
			    UPDATE CO_DOCUMENTOS
			    SET COD_ESTADO='4'
			    WHERE
				 NUM_SECUENCI  = codetsolicitud.num_secuenci_doc AND
			...
			END LOOP;    /*  ciclo principal    */
			vp_ruteo := 26;
			vp_gls_error:='Actualizacion de tabla co_solicitud';
			UPDATE CO_SOLICITUD
			       SET   ESTADO_SOLICITUD = 5
			       WHERE
				     NUM_SOLICITUD  = TO_NUMBER(vp_Num_Solicitud);
