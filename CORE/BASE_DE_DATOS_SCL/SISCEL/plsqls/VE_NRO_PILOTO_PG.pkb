CREATE OR REPLACE PACKAGE BODY ve_nro_piloto_pg
IS
  ...
  PROCEDURE pv_borra_nropiloto_pr(SN_num_piloto      IN  ga_nro_piloto_rango_to.num_piloto%TYPE,
        ...
        EXCEPTION
            WHEN OTHERS THEN
              SN_cod_retorno:=156;
                ...
                END IF;
                V_des_error := 'pv_borra_nropiloto_pr('||SN_num_piloto||'); - ' || SQLERRM;
                SN_num_evento := Ge_Errores_Pg.Grabarpl( SN_num_evento, 'GA', SV_mens_retorno, '1.0', USER, 've_nro_piloto_pg.pv_borra_nropiloto_pr', sSql, SN_cod_retorno, V_des_error );
   END;
        ...
        EXCEPTION
            WHEN OTHERS THEN
              SN_cod_retorno:=156;
                ...
                SN_num_evento := Ge_Errores_Pg.Grabarpl( SN_num_evento, 'GA', SV_mens_retorno, '1.0', USER, 've_nro_piloto_pg.pv_baja_nropiloto_pr', sSql, SN_cod_retorno, V_des_error );
    END;   
   ...
   PROCEDURE ve_enlace_vpn_rango_pr(SN_cod_retorno     OUT NOCOPY ge_errores_td.cod_msgerror%TYPE,
    ...
    IS
        ...
        LN_num_venta        GA_NRO_PILOTO_ABO_TO.NUM_VENTA%TYPE;
        ...
        CURSOR C_NRO_PILOTO IS
            ...
            WHERE ra.ESTADO NOT IN('TERMINADO','ERRONEO') AND ra.NUM_VENTA IS NOT NULL ORDER BY ra.FECHA_REGISTRO ASC; --REVISAR
        ...
        OPEN C_NRO_PILOTO;
            ...
            LN_num_venta:= cAbo.NUM_VENTA;
            LN_contador := LN_contador +1;
            LV_estado   := 'TERMINADO';
            dbms_output.PUT_LINE('operacion '||LV_operacion||' contador '||LN_contador||' num_venta '||LN_num_venta);
         
            SELECT count(1) INTO LN_contAbo FROM ga_abocel a WHERE a.num_venta = LN_num_venta AND a.cod_situacion NOT IN ('AAA');
         
            -- Si abonado piloto está en AAA se crean los abonados.
            -- Si no, no se hace nada y se espera la nueva pasada del job.
            
            IF LN_contAbo = 0 THEN 

                IF LV_operacion = 'CREAR' then --num_piloto
                    ve_numero_piloto_pr(LN_num_venta,SN_cod_retorno,SV_mens_retorno,SN_num_evento);
                    V_des_error := 've_numero_piloto_pr(' || LN_num_venta;
            
                    -- se dejará acá por ahora, si se deben realizar otras operaciones se podría dejar fuera 
                    ...
                    WHERE ra.NUM_VENTA = LN_num_venta AND ra.OPERACION = 'CREAR';
            ...
            WHEN OTHERS THEN
              SN_cod_retorno:=156;
                IF NOT Ge_Errores_Pg.MENSAJEERROR(SN_cod_retorno,SV_mens_retorno) THEN
                 SV_mens_retorno := CV_error_no_clasif;
                END IF;
                V_des_error := 've_enlace_vpn_rango_pr(); - ' || SQLERRM;
                SN_num_evento := Ge_Errores_Pg.Grabarpl( SN_num_evento, 'GA', SV_mens_retorno, '1.0', USER, 've_nro_piloto_pg.ve_enlace_vpn_rango_pr', sSql, SN_cod_retorno, V_des_error );
    END;   
   ...
   BEGIN
     ...
     BEGIN
      ...
      END;
             ...
             ve_abocel_pr(p_num_abonado,SN_cod_retorno,SV_mens_retorno,SN_num_evento);
             V_des_error := 've_abocel_pr(' || p_num_abonado;
             
             if SN_cod_retorno = 0 then 
               ve_servsuplabo_pr(p_num_abonado,SN_cod_retorno,SV_mens_retorno,SN_num_evento);
               V_des_error := 've_servsuplabo_pr(' || p_num_abonado;
             end if;  
             if SN_cod_retorno = 0 then 
               ve_infaccel_pr(p_num_abonado,SN_cod_retorno,SV_mens_retorno,SN_num_evento);
               V_des_error := 've_infaccel_pr(' || p_num_abonado;
             end if;  
             if SN_cod_retorno = 0 then 
               ve_intarcel_pr(p_num_abonado,SN_cod_retorno,SV_mens_retorno,SN_num_evento);
               V_des_error := 've_intarcel_pr(' || p_num_abonado;
             end if;  
             if SN_cod_retorno = 0 then 
               ve_intarcel_acciones_pr(p_num_abonado,SN_cod_retorno,SV_mens_retorno,SN_num_evento);
               V_des_error := 've_intarcel_acciones_pr(' || p_num_abonado;
             end if;  
             if SN_cod_retorno = 0 then 
               ve_limite_cliabo_pr(p_num_abonado,SN_cod_retorno,SV_mens_retorno,SN_num_evento);
               V_des_error := 've_limite_cliabo_pr(' || p_num_abonado;
             end if;  
             if SN_cod_retorno = 0 then 
               ve_rangos_numericos_pr(p_num_abonado,SN_cod_retorno,SV_mens_retorno,SN_num_evento); 
               V_des_error := 've_rangos_numericos_pr(' || p_num_abonado;
             end if;
             if SN_cod_retorno = 0 then 
               ve_ciclocli_pr(p_num_abonado,SN_cod_retorno,SV_mens_retorno,SN_num_evento); 
               V_des_error := 've_ciclocli_pr(' || p_num_abonado;
             end if;            
        ...
        EXCEPTION
            ...
            WHEN E_EXCEPTION THEN
              ...
              IF NOT Ge_Errores_Pg.MENSAJEERROR(SN_cod_retorno,SV_mens_retorno) THEN
                SV_mens_retorno := CV_error_no_clasif;
              END IF;
              V_des_error := 'pv_enlace_vpn_pr('||SN_num_piloto||'); - ' || SQLERRM;
              SN_num_evento := Ge_Errores_Pg.Grabarpl( SN_num_evento, 'GA', SV_mens_retorno, '1.0', USER, 've_nro_piloto_pg.pv_enlace_vpn_pr', sSql, SN_cod_retorno, V_des_error );
    ...
    FETCH C_NRO_PILOTO INTO cnro_piloto;
      EXIT WHEN C_NRO_PILOTO%NOTFOUND; 
             ...
             ve_abocel_pr(cnro_piloto.num_abonado,SN_cod_retorno,SV_mens_retorno,SN_num_evento);
             V_des_error := 've_abocel_pr(' || cnro_piloto.num_abonado;
             
             if SN_cod_retorno = 0 then 
               ve_servsuplabo_pr(cnro_piloto.num_abonado,SN_cod_retorno,SV_mens_retorno,SN_num_evento);
               V_des_error := 've_servsuplabo_pr(' || cnro_piloto.num_abonado;
             end if;  
             if SN_cod_retorno = 0 then 
               ve_infaccel_pr(cnro_piloto.num_abonado,SN_cod_retorno,SV_mens_retorno,SN_num_evento);
               V_des_error := 've_infaccel_pr(' || cnro_piloto.num_abonado;
             end if;  
             if SN_cod_retorno = 0 then 
               ve_intarcel_pr(cnro_piloto.num_abonado,SN_cod_retorno,SV_mens_retorno,SN_num_evento);
               V_des_error := 've_intarcel_pr(' || cnro_piloto.num_abonado;
             end if;  
             if SN_cod_retorno = 0 then 
               ve_intarcel_acciones_pr(cnro_piloto.num_abonado,SN_cod_retorno,SV_mens_retorno,SN_num_evento);
               V_des_error := 've_intarcel_acciones_pr(' || cnro_piloto.num_abonado;
             end if;  
             if SN_cod_retorno = 0 then 
               ve_limite_cliabo_pr(cnro_piloto.num_abonado,SN_cod_retorno,SV_mens_retorno,SN_num_evento);
               V_des_error := 've_limite_cliabo_pr(' || cnro_piloto.num_abonado;
             end if;  
             if SN_cod_retorno = 0 then 
               ve_rangos_numericos_pr(cnro_piloto.num_abonado,SN_cod_retorno,SV_mens_retorno,SN_num_evento); 
               V_des_error := 've_rangos_numericos_pr(' || cnro_piloto.num_abonado;
             end if;
             if SN_cod_retorno = 0 then 
               ve_ciclocli_pr(cnro_piloto.num_abonado,SN_cod_retorno,SV_mens_retorno,SN_num_evento); 
               V_des_error := 've_ciclocli_pr(' || cnro_piloto.num_abonado;
             end if;             
        ...
        EXCEPTION
            WHEN E_EXCEPTION THEN
              ...
              IF NOT Ge_Errores_Pg.MENSAJEERROR(SN_cod_retorno,SV_mens_retorno) THEN
                SV_mens_retorno := CV_error_no_clasif;
              END IF;
              V_des_error := 've_numero_piloto_pr('||SN_num_venta||'); - ' || SQLERRM;
              SN_num_evento := Ge_Errores_Pg.Grabarpl( SN_num_evento, 'GA', SV_mens_retorno, '1.0', USER, 've_nro_piloto_pg.ve_numero_piloto_pr', sSql, SN_cod_retorno, V_des_error );
            ...
            WHEN OTHERS THEN
              SN_cod_retorno:=156;
                IF NOT Ge_Errores_Pg.MENSAJEERROR(SN_cod_retorno,SV_mens_retorno) THEN
                 SV_mens_retorno := CV_error_no_clasif;
                END IF;
                V_des_error := 've_rangos_numericos_pr('||SN_num_abonado||'); - ' || SQLERRM;
                SN_num_evento := Ge_Errores_Pg.Grabarpl( SN_num_evento, 'GA', SV_mens_retorno, '1.0', USER, 've_nro_piloto_pg.ve_rangos_numericos_pr', sSql, SN_cod_retorno, V_des_error );
    END;   
            ...
            WHEN OTHERS THEN
              SN_cod_retorno:=156;
                IF NOT Ge_Errores_Pg.MENSAJEERROR(SN_cod_retorno,SV_mens_retorno) THEN
                 SV_mens_retorno := CV_error_no_clasif;
                END IF;
                V_des_error := 've_numero_piloto_pr('||SN_num_abonado||'); - ' || SQLERRM;
                SN_num_evento := Ge_Errores_Pg.Grabarpl( SN_num_evento, 'GA', SV_mens_retorno, '1.0', USER, 've_nro_piloto_pg.ve_abocel_pr', sSql, SN_cod_retorno, V_des_error );
    END;
     ...
     BEGIN
        ...
        EXCEPTION
            WHEN OTHERS THEN
              ...
              END IF;
              V_des_error := 've_numero_piloto_pr('||SN_num_abonado||'); - ' || SQLERRM;
              SN_num_evento := Ge_Errores_Pg.Grabarpl( SN_num_evento, 'GA', SV_mens_retorno, '1.0', USER, 've_nro_piloto_pg.ve_equipaboser_pr', sSql, SN_cod_retorno, V_des_error );
    END;
    ...
    BEGIN
        ...
        EXCEPTION
            WHEN OTHERS THEN
              ...
              END IF;
              V_des_error := 've_numero_piloto_pr('||SN_num_abonado||'); - ' || SQLERRM;
              SN_num_evento := Ge_Errores_Pg.Grabarpl( SN_num_evento, 'GA', SV_mens_retorno, '1.0', USER, 've_nro_piloto_pg.ve_servsuplabo_pr', sSql, SN_cod_retorno, V_des_error );
    END;
        ...
        CLOSE CUR_INF;  
         ...
         EXCEPTION
            WHEN OTHERS THEN
              ...
              END IF;
              V_des_error := 've_numero_piloto_pr('||SN_num_abonado||'); - ' || SQLERRM;
              SN_num_evento := Ge_Errores_Pg.Grabarpl( SN_num_evento, 'GA', SV_mens_retorno, '1.0', USER, 've_nro_piloto_pg.ve_infaccel_pr', sSql, SN_cod_retorno, V_des_error );
   END;                
    ...
    FETCH CUR_INT INTO cInt;
        ...
        EXCEPTION
            WHEN OTHERS THEN
              ...
              END IF;
              V_des_error := 've_numero_piloto_pr('||SN_num_abonado||'); - ' || SQLERRM;
              SN_num_evento := Ge_Errores_Pg.Grabarpl( SN_num_evento, 'GA', SV_mens_retorno, '1.0', USER, 've_nro_piloto_pg.ve_intarcel_pr', sSql, SN_cod_retorno, V_des_error );
    END;                
       ...
       CLOSE CUR_INT;  
        ...
        EXCEPTION
            WHEN OTHERS THEN
              ...
              END IF;
              V_des_error := 've_numero_piloto_pr('||SN_num_abonado||'); - ' || SQLERRM;
              SN_num_evento := Ge_Errores_Pg.Grabarpl( SN_num_evento, 'GA', SV_mens_retorno, '1.0', USER, 've_nro_piloto_pg.ve_intarcel_acciones_pr', sSql, SN_cod_retorno, V_des_error );
    END;                
      ...
      FOR LN_indice in 0 .. LN_record.count - 1 
        ...
        EXCEPTION
            WHEN OTHERS THEN
              ...
              END IF;
              V_des_error := 've_numero_piloto_pr('||SN_num_abonado||'); - ' || SQLERRM;
              SN_num_evento := Ge_Errores_Pg.Grabarpl( SN_num_evento, 'GA', SV_mens_retorno, '1.0', USER, 've_nro_piloto_pg.ve_limite_cliabo_pr', sSql, SN_cod_retorno, V_des_error );
    END;
    ...
    BEGIN
        ...
        EXCEPTION
            WHEN OTHERS THEN
              ...
              END IF;
              V_des_error := 've_numero_piloto_pr('||SN_num_abonado||'); - ' || SQLERRM;
              SN_num_evento := Ge_Errores_Pg.Grabarpl( SN_num_evento, 'GA', SV_mens_retorno, '1.0', USER, 've_nro_piloto_pg.ve_ciclocli_pr', sSql, SN_cod_retorno, V_des_error );
    END;    
