CREATE OR REPLACE PROCEDURE        p_a_num_o_ams(v_numeracion IN al_ser_numeracion.num_numeracion%TYPE ,
...
BEGIN
    ...
    v_rownum       := 11;
    BEGIN
    al_pac_numeracion.p_select_celular (v_producto, v_error);
    EXCEPTION
        WHEN OTHERS THEN
            v_error := 1;
            v_error_msg := 'p_select_celular()';
    END;
    ...
    EXCEPTION
        WHEN OTHERS THEN
            v_error := 1;
            v_error_msg := 'ge_fn_dias_hibernacion()';
    END;
    BEGIN
    ...
    IF v_error = 0 THEN
        OPEN c_sernumera(v_numeracion, v_subalm, v_central, v_cat);
        LOOP
...
EXCEPTION
    WHEN OTHERS THEN
        ...
        ELSE
            v_error := 1;
            v_error_msg := v_error_msg || SQLCODE;
        END IF;
END;
/
SHOW ERRORS
