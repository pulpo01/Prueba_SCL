CREATE OR REPLACE TRIGGER SC_BEDEL_COTIPAJUSTEND
BEFORE  DELETE
ON CO_TIPAJUSTEND
FOR EACH ROW
...
BEGIN
	...
	IF v_ind_grupo = 0 THEN  -- Si el concepto es simple
          ...
          WHERE  A.COD_CONCEPTO = SUBSTR(TO_CHAR(:OLD.COD_TIPAJUSTEND,'099999'),2,6)
          AND A.NUM_LOTE = B.NUM_LOTE
	      AND B.COD_EVENTO=C.COD_EVENTO
	      AND C.COD_DOMINIO_CTO=(SELECT COD_DOMINIO_CTO  FROM SC_DOMINIO_CTO WHERE DES_DOMINIO_CTO = 'CO_TIPAJUSTEND');
    ...
    --Abrir Cursor
	  FOR RegBuscaGrupo IN BuscaDominio LOOP
	   ...
	   AND   D.COD_DOMINIO_CTO=(SELECT COD_DOMINIO_CTO  FROM SC_DOMINIO_CTO WHERE DES_DOMINIO_CTO = 'CO_TIPAJUSTEND');
	...
	IF v_cantidad = 0 AND v_eliminar = TRUE THEN
	  ...
	  WHERE COD_CONCEPTO = SUBSTR(TO_CHAR(:OLD.COD_TIPAJUSTEND,'099999'),2,6);
    ELSE
      RAISE ELIMINA;
    END IF;
...
EXCEPTION
       WHEN  ELIMINA THEN
	   RAISE_APPLICATION_ERROR(-20003,'EL TIPO AJUSTE ND A ELIMINAR TIENE ASOCIADO CONCEPTOS CONTABLES POR LO QUE NO SE PUEDE ELIMINAR.');
