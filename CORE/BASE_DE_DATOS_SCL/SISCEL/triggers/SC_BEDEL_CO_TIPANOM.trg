CREATE OR REPLACE TRIGGER SC_BEDEL_CO_TIPANOM
BEFORE  DELETE
ON CO_TIPANOM
FOR EACH ROW
...
BEGIN
	...
	IF v_ind_grupo=0 THEN  -- Si el concepto es simple
          ...
          WHERE  A.COD_CONCEPTO = LPAD(:OLD.COD_ANOMALIA,6,'0')
          AND A.NUM_LOTE = B.NUM_LOTE
	      AND B.COD_EVENTO=C.COD_EVENTO
	      AND C.COD_DOMINIO_CTO =(SELECT COD_DOMINIO_CTO  FROM SC_DOMINIO_CTO WHERE DES_DOMINIO_CTO = 'CO_TIPANOM');
    ...
    ELSE -- si el concepto es de grupo
	  ...
	  FOR RegBuscaGrupo IN BuscaDominio LOOP
	   ...
	   AND   D.COD_DOMINIO_CTO=(SELECT COD_DOMINIO_CTO  FROM SC_DOMINIO_CTO WHERE DES_DOMINIO_CTO = 'CO_TIPANOM');
	...
	IF v_cantidad = 0  AND v_eliminar = TRUE THEN
	  ...
	  WHERE COD_CONCEPTO = LPAD(:OLD.COD_ANOMALIA,6,'0');
   	ELSE
   	   RAISE ELIMINA;
   	END IF;
...
EXCEPTION
       WHEN  ELIMINA THEN
	   RAISE_APPLICATION_ERROR(-20003,'EL TIPO DE ANOMALIA A ELIMINAR TIENE ASOCIADO CONCEPTOS CONTABLES POR LO QUE NO SE PUEDE ELIMINAR.');
