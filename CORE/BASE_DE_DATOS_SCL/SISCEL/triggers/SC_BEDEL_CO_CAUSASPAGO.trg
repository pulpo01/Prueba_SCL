CREATE OR REPLACE TRIGGER SC_BEDEL_CO_CAUSASPAGO
BEFORE DELETE
ON CO_CAUSASPAGO
FOR EACH ROW
...
BEGIN
	...
	IF v_ind_grupo=0 THEN  -- Si el concepto es simple
		...
		SELECT COUNT(1) INTO v_cantidad
         ...
         WHERE  A.COD_CONCEPTO = LPAD(:OLD.COD_CAUPAGO,6,'0')
         AND A.NUM_LOTE = B.NUM_LOTE
	     AND B.COD_EVENTO=C.COD_EVENTO
	     AND C.COD_DOMINIO_CTO=(SELECT COD_DOMINIO_CTO  FROM SC_DOMINIO_CTO WHERE DES_DOMINIO_CTO = 'CO_CAUSASPAGO');
    ...
    ELSE -- si el concepto es de grupo
	  ...
	  v_cod_dominio_cto:= RegBuscaGrupo.COD_DOMINIO_CTO;
	   ...
	   AND   D.COD_DOMINIO_CTO=(SELECT COD_DOMINIO_CTO  FROM SC_DOMINIO_CTO WHERE DES_DOMINIO_CTO = 'CO_CAUSASPAGO');
	...
	IF v_cantidad = 0 and v_eliminar = TRUE THEN
	  ...
	  WHERE COD_CONCEPTO = LPAD(:OLD.COD_CAUPAGO,6,'0');
    ELSE
      RAISE ELIMINA;
    END IF;
...
EXCEPTION
       WHEN  ELIMINA THEN
	   RAISE_APPLICATION_ERROR(-20003,'LA CTA CTE A ELIMINAR TIENE ASOCIADO CONCEPTOS CONTABLES POR LO QUE NO SE PUEDE ELIMINAR.');
