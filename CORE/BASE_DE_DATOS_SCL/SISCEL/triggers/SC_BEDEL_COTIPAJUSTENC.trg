CREATE OR REPLACE TRIGGER SC_BEDEL_COTIPAJUSTENC
BEFORE DELETE
ON CO_TIPAJUSTENC
FOR EACH ROW
...
BEGIN
	...
	IF v_ind_grupo=0 THEN  -- Si el concepto es simple
        ...
        WHERE  A.COD_CONCEPTO = SUBSTR(TO_CHAR(:OLD.COD_TIPAJUSTENC,'099999'),2,6)
        AND    A.NUM_LOTE = B.NUM_LOTE
	    AND B.COD_EVENTO=C.COD_EVENTO
	    AND C.COD_DOMINIO_CTO=(SELECT COD_DOMINIO_CTO  FROM SC_DOMINIO_CTO WHERE DES_DOMINIO_CTO = 'CO_TIPAJUSTENC');
    ...
    --Abrir Cursor
	  FOR RegBuscaGrupo IN BuscaDominio LOOP
	   ...
	   AND   D.COD_DOMINIO_CTO=(SELECT COD_DOMINIO_CTO  FROM SC_DOMINIO_CTO WHERE DES_DOMINIO_CTO = 'CO_TIPAJUSTENC');
	...
	IF v_cantidad = 0 AND v_eliminar = TRUE THEN
		...
		WHERE COD_CONCEPTO = SUBSTR(TO_CHAR(:OLD.COD_TIPAJUSTENC,'099999'),2,6);
	ELSE
		RAISE ELIMINA;
    END IF;
...
EXCEPTION
       WHEN  ELIMINA THEN
	   RAISE_APPLICATION_ERROR(-20003,'EL TIPO DE AJUSTE NC A ELIMINAR TIENE ASOCIADO CONCEPTOS CONTABLES POR LO QUE NO SE PUEDE ELIMINAR.');
