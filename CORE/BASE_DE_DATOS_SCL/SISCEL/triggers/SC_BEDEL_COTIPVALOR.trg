CREATE OR REPLACE TRIGGER SC_BEDEL_COTIPVALOR
BEFORE  DELETE
ON CO_TIPVALOR
FOR EACH ROW
DECLARE
...
BEGIN
	...
	IF v_ind_grupo = 0 THEN  -- Si el concepto es simple
            ...
            WHERE  A.COD_CONCEPTO = LPAD(:OLD.TIP_VALOR,v_largo,'0')
			...
			IF v_cantidad = 0 THEN
			   ...
			   FROM SC_CTO_CTA
			   WHERE COD_DOMINIO_CTO = v_cod_dominio_cto
			   AND COD_CONCEPTO = LPAD(:OLD.TIP_VALOR,v_largo,'0')
			   AND ROWNUM = 1;
    ...
    ELSE -- si el concepto es de grupo
	  ...
	  ELSE
	  	 --Verifica si se asoció a alguna cuenta contable
		 FOR RegBuscaConceptoGrp IN BuscaConceptoGrp(v_cod_grp_dominio, v_cod_dominio_cto, :OLD.TIP_VALOR, v_largo) LOOP
...
EXCEPTION
	   ...
	   RAISE_APPLICATION_ERROR(-20004,'EL TIPO DE VALOR A ELIMINAR TIENE ASOCIADA UNA CUENTA CONTABLE POR LO QUE NO SE PUEDE ELIMINAR.');

       WHEN  ELIMINA THEN
	   RAISE_APPLICATION_ERROR(-20003,'EL TIPO DE VALOR A ELIMINAR TIENE CONCEPTOS CONTABILIZADOS POR LO QUE NO SE PUEDE ELIMINAR.');
