CREATE OR REPLACE TRIGGER icm_bein_histmov
BEFORE INSERT
...
BEGIN
  ...
  V_TABLA := 'ICM_HISTMOV' || TO_CHAR(:NEW.FEC_HISTORICO,'MMYYYY');
  V_SENTENCIA := 'INSERT INTO ' || V_TABLA ||
                 ' (NUM_MOVIMIENTO,FEC_HISTORICO,NUM_ABONADO,COD_ACTUACION,
                    ...
                    COD_CENTRAL,COD_CENTRAL_NUE,
                    NUM_MAN,NUM_SERIE,COD_MODELO,DIR_X25,NUM_MAN_NUE,
                    NUM_SERIE_NUE,COD_MODELO_NUE,DIR_X25_NUE,
  ...
  dbms_sql.bind_variable(cursor_name,'x15',:NEW.NUM_MAN);
  dbms_sql.bind_variable(cursor_name,'x16',:NEW.NUM_SERIE);
  dbms_sql.bind_variable(cursor_name,'x17',:NEW.COD_MODELO);
  dbms_sql.bind_variable(cursor_name,'x18',:NEW.DIR_X25);
  dbms_sql.bind_variable(cursor_name,'x19',:NEW.NUM_MAN);
  dbms_sql.bind_variable(cursor_name,'x20',:NEW.NUM_SERIE);
  dbms_sql.bind_variable(cursor_name,'x21',:NEW.COD_MODELO_NUE);
  dbms_sql.bind_variable(cursor_name,'x22',:NEW.DIR_X25_NUE);
  dbms_sql.bind_variable(cursor_name,'x23',:NEW.COD_SERVICIOS);
  dbms_sql.bind_variable(cursor_name,'x24',:NEW.COD_SUSPREHA);
  dbms_sql.bind_variable(cursor_name,'x25',:NEW.NUM_MOVPOS);
  dbms_sql.bind_variable(cursor_name,'x26',:NEW.COD_ACTABO);
  dbms_sql.bind_variable(cursor_name,'x27',:NEW.TIP_TERMINAL);
  ...
  EXCEPTION WHEN OTHERS THEN
   ...
   (-20006,'ERROR TRIGGER ICM_BEIN_HISTMOV: '||
