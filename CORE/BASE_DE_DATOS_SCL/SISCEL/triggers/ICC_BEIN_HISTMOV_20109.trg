...
BEGIN
  ...
  V_SENTENCIA := 'INSERT INTO ' || V_TABLA ||
    ...
    :NEW.cod_act := '';
		...
		dbms_sql.bind_variable(cursor_name,'x57',:NEW.FEC_ACTIVACION);
		dbms_sql.bind_variable(cursor_name,'x58',:NEW.COD_ESPEC_PROV);
		dbms_sql.bind_variable(cursor_name,'x59',:NEW.COD_PROD_CONTRATADO);
		dbms_sql.bind_variable(cursor_name,'x60',:NEW.IND_BAJATRANS);		
