CREATE OR REPLACE PROCEDURE            P_INSERT_COURRIER_PED(
vp_cod_mensaje				out	npt_mensaje.cod_mensaje%TYPE,
vp_cod_courrier 			in	npt_courrier_pedido.cod_courrier%TYPE,
vp_cod_pedido					in	npt_courrier_pedido.cod_pedido%TYPE,
...
BEGIN
	...
	where cod_funcion = vp_cod_funcion
	  and orden_fun_est_flu_esc = 1;
	select cod_usuario into v_cod_usuario_des
	from npd_courrier
	where cod_courrier = vp_cod_courrier;
	insert into npt_courrier_pedido (cod_courrier, cod_pedido)
	values (vp_cod_courrier, vp_cod_pedido);
