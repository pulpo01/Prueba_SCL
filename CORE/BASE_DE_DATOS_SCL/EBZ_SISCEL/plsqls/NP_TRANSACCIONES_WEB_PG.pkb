CREATE OR REPLACE PACKAGE BODY NP_TRANSACCIONES_WEB_PG IS
...
BEGIN
      Rollback;
      update NP_VALIDACION_SERIES_TO set estado='ERROR', cod_error=cError
      ...
      OPEN c_series;
         LOOP
           ...
           BEGIN
            ...
            BEGIN
         	   select count(1) into v_serie_temp from NP_VALIDACION_SERIES_TO where num_serie=v_serie;
...
--  fin P-MIX-09003-Guatemala-Salvador
			  ...
			  EXCEPTION
                       WHEN e_BUSCA_ERROR THEN
                    	  ...
                    	  msgError := ERRORNEG;
              		  	  GRABAERROR_VALIDASERIES( cError, pCodPedido,  v_serie);
