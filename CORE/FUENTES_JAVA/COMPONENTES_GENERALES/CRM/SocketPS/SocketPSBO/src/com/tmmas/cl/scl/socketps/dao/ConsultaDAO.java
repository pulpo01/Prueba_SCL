...
public class ConsultaDAO implements ConsultaDAOIT {
	...
	public SaldoDTO consultaBolsas(ConsultaBolsasDTO consultaBolsas) throws SocketPSException {
		...
		PrintWriter os = null;
		Socket sock = null;
		HeaderPSDTO header = consultaBolsas.getHeader();
		String orden = null;
		SaldoDTO saldo = new SaldoDTO();
		...
		if (header.getOrden() == null) {
			// Seteo la orden a la consulta de plan
			orden = global.getValor("param.consultaBolsas");
			header.setOrden(orden);
		}
		logger.debug("orden[" + orden + "]");
		...
		if (!parametrosdeCliente) {
			...
			// que corresponden al primer nivel, es decir, sin el objeto
			// HeaderPSDTO.
			armarParametrosBolsas(consultaBolsas);
		}
		...
		try {
			...
			logger.debug("recibio (Acknolegment de recepcion)  [" + reply + "]");

			//>\\KVESUBERROR=,STATUS=OK,OS=10000,TICKET=1000,DETAIL=NUM_BOLSAS\\=4\\,VALOR\\=|1|50.20|30|2|20.30|30|3|10.00|30|4|0.00|30|
			...
			//Obtengo la respuesta
			String paramValor = "";
			String regExpValorBolsas = global.getValor("reg.exp.valorBolsas");
			logger.debug("regExpValorBolsas[" + regExpValorBolsas + "]");		
			
			Pattern pattern = Pattern.compile(regExpValorBolsas);
			Matcher matchedPattern = pattern.matcher(reply);
			while (matchedPattern.find()) {
				String strMatch = reply.substring(matchedPattern.start(), reply.length());
				logger.debug("strMatch="+strMatch);
				paramValor = strMatch; //VALOR\=|1|50.20|30|2|20.30|30|3|10.00|30|4|0.00|30|
				break;
			}
			
			String[] cadena =  paramValor.split(global.getValor("param.separador.igual"));
			String valor = "";
			if (cadena.length==2){
				valor = cadena[1].trim();
			}else{
				logger.debug("Error al parsear cadena");
				throw new SocketPSException("Error al parsear cadena");
			}
			logger.debug("valor[" + valor + "]");
			saldo.setSaldo(valor);
	...
	private void armarParametrosBolsas(ConsultaBolsasDTO consultaBolsas) {
		logger.debug("armarParametrosBolsas():start");
		String listaParametros = global.getValor("lista.param.consultaBolsas");
		logger.debug("listaParametros[" + listaParametros + "]");
		...
		String paramValores[] = new String[3];
		paramValores[0] = consultaBolsas.getLin();
		paramValores[1] = consultaBolsas.getCodMoneda();
		paramValores[2] = consultaBolsas.getCantRegs();
		logger.debug("Lin[" + consultaBolsas.getLin() + "]");
		logger.debug("CodMoneda[" + consultaBolsas.getCodMoneda() + "]");
		logger.debug("CantRegs[" + consultaBolsas.getCantRegs() + "]");
		HeaderPSDTO header = consultaBolsas.getHeader();
		header.setParametrosNombre(paramNombre);
		header.setParametrosValor(paramValores);
		logger.debug("armarParametrosBolsas():end");
	}
