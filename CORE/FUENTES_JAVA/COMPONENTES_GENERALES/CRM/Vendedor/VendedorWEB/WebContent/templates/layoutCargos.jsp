...
function calcular(){
	...
	if(document.getElementById("tablaDatos").rows.length>3){
		...
		try{
			...
			for(i=2; i<document.getElementById("tablaDatos").rows.length-1;i++){
				tipo = getCellByRowCol(i,1,"tabladatos");

				try {
					if (inCheck[i-2].checked) chequeado=true;
				}
				catch (e)	{
					chequeado=false;
				}
	...
	//					cantDescAut = parseFloat (getCellByRowCol(i,6,"tablaDatos"));
				...
				if (calculo) setearTotal(i,total,minimo,maximo,cienPorciento,valTipDesc,descuentoManual);
...
function activarCheckCargo(){
	...
	var inCheck = eval("document.CargosForm.selectedValorCheck");
	var selectTipDesc = eval("document.CargosForm.tipoDescuentoManual");
	var textboxDesc = eval("document.CargosForm.descuentoUnitarioMan");
	...
	for(i=2; i<document.getElementById("tablaDatos").rows.length-1;i++){
			  var asd=getCellByRowCol(i,0,"tablaDatos");
				...
				if (tipoCargo=="M"){
					...
					// Tambien hay que deshabilitar el select y textbox asociados al mismo
					selectTipDesc[j].disabled=true;				
					textboxDesc[j].disabled=true;				
				}
				else{
					inCheck[j].checked=true;
					
					// Tambien hay que habilitar el select y textbox asociados al mismo
					selectTipDesc[j].disabled=false;				
					textboxDesc[j].disabled=false;				
				}
				j++;
		} // for
