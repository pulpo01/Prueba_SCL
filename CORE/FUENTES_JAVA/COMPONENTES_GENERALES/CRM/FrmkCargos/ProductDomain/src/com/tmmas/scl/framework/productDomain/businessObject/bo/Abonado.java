...
public class Abonado implements AbonadoIT {
	...
	public ParametrosObtencionCargosDTO verificaRenovacion(ParametrosObtencionCargosDTO abonado) throws ProductException {
		ParametrosObtencionCargosDTO respuesta = null;
		
		try {
			
			logger.debug("verificaRenovacion():start");
			respuesta = abonadoDAO.verificaRenovacion(abonado);
			logger.debug("verificaRenovacion():end");
		} catch (ProductException e) {
			logger.debug("ProductException[", e);
			throw e;
		}
