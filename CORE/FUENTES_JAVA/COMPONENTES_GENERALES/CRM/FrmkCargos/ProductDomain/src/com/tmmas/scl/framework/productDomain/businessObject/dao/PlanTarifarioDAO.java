...
public class PlanTarifarioDAO extends ConnectionDAO{
	...
	private String getSQLgetCodigoCategoria(){
		...
		call.append("");
		call.append("BEGIN "); 
		call.append("  PV_SERVICIOS_POSVENTA_PG.VE_CONSULTA_CATEG_PTARIF_PR (?, ?, ?, ?, ? ); ");
		call.append("END;");
	...
	public PlanTarifarioDTO getPlanTarifario(PlanTarifarioDTO planEntrada) throws ProductOfferingException{
		...
		try {
			//String call = getSQLDatosPlanTarifario("PV_SERVICIOS_POSVENTA_PG","VE_consulta_plan_tarifario_PR",20);
			String call = "{call PV_SERVICIOS_POSVENTA_PG.VE_consulta_plan_tarifario_PR(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)}";
			...
			cstmt.registerOutParameter(7, java.sql.Types.VARCHAR);      //SN_numdias        	 	  
			...
			cstmt.registerOutParameter(13, java.sql.Types.VARCHAR);     //SV_monedacargobasico        
			cstmt.registerOutParameter(14, java.sql.Types.VARCHAR);     //SV_codtiplan       	    
			cstmt.registerOutParameter(15, java.sql.Types.VARCHAR);     //SN_ind_familiar   
			cstmt.registerOutParameter(16, java.sql.Types.NUMERIC);     //SN_num_Abonado
			cstmt.registerOutParameter(17, java.sql.Types.VARCHAR);     //SN_cod_plan_converse
			...
			if (codError==0){
				...
				resultado.setLimiteConsumo(cstmt.getString(6));
				...
				resultado.setNumAbonado(cstmt.getString(16));
			}
			else{
				//TODO : implementar metodo de exception para recepcion de Datos codError,msgError,numEvento
	...
	public PrecioCargoDTO[] getCargoBasico(PlanTarifarioDTO entrada) throws ProductOfferingException{
		...
		try {
			String call = getSQLDatosPlanTarifario("PV_SERVICIOS_POSVENTA_PG","VE_precio_cargo_basico_PR",6);
			logger.debug("sql[" + call + "]");
	...
	public DescuentoDTO[] getDescuentoCargoBasicoArticulo(ParametrosDescuentoDTO entrada) throws ProductOfferingException{
		...
		try {
			String call = getSQLDatosPlanTarifario("PV_SERVICIOS_POSVENTA_PG","VE_obtiene_descuento_art_PR",14);
			logger.debug("sql[" + call + "]");
			conn = getConnectionFromWLSInitialContext(global.getCPUJndiForDataSource());
	...
	public DescuentoDTO[] getDescuentoCargoBasicoConcepto(ParametrosDescuentoDTO entrada) throws ProductOfferingException{
		...
		try {
			String call = getSQLDatosPlanTarifario("PV_SERVICIOS_POSVENTA_PG","VE_obtiene_descuento_con_PR",16);
			logger.debug("sql[" + call + "]");
			conn = getConnectionFromWLSInitialContext(global.getCPUJndiForDataSource());
	...
	public PlanTarifarioDTO getCategoriaPlanTarifario(PlanTarifarioDTO planEntrada) throws ProductOfferingException{
		...
		try {
			...
			logger.debug("Código Plan tarifario :: getCodPlanTarif       ["+planEntrada.getCodPlanTarif()+"]");
			logger.debug("Código Plan tarifario :: getCodigoPlanTarifario["+planEntrada.getCodigoPlanTarifario()+"]");
			String call = getSQLgetCodigoCategoria();
			logger.debug("sql[" + call + "]");
			conn = getConnectionFromWLSInitialContext(global.getCPUJndiForDataSource());

			cstmt = conn.prepareCall(call);
			logger.debug("Código Plan tarifario:mod 02022010 -->getCodigoPlanTarifario()!=null? codPlantarif:getCodPlanTarif()");
			//cstmt.setString(1,planEntrada.getCodPlanTarif());

            String codPlantarif = planEntrada.getCodigoPlanTarifario();
            codPlantarif= codPlantarif!=null? codPlantarif: planEntrada.getCodPlanTarif();
	...
	public DescuentoDTO getCodigoDescuentoManual(ParametrosDescuentoDTO entrada) throws ProductOfferingException{
		...
		try {
			String call = getSQLDatosPlanTarifario("PV_SERVICIOS_POSVENTA_PG","VE_consulta_cod_desc_manual_PR",6);
			logger.debug("sql[" + call + "]");
			conn = getConnectionFromWLSInitialContext(global.getCPUJndiForDataSource());
