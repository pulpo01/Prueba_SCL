...
public class ReglasTerminalPV extends ReglaListaPrecio{
	private final Logger logger = Logger.getLogger(ReglasTerminalPV.class);
	...
	public ReglasTerminalPV(ParametrosReglasTerminalDTO parametros) {
		super(parametros);
	...
	public PrecioDTO[] seleccionPrecios() throws FrameworkCargosException{
		...
		try {
			if (parametrosRegla.getIndicadorPrecioLista().equals(IndPrecioLista)){
				...
				terminal.setNumAbonado(Long.parseLong(parametrosRegla.getNumAbonado()));
				
				if (terminal.getParamRenova() != null && terminal.getParamRenova().equals("1")){
					logger.debug("CON RENOVACION");
					logger.debug("entrada.getParamRenova(): "+terminal.getParamRenova());
					arregloPreciosCargos = terminalBO.getPrecioCargoTerminal_NoPrecioLista(terminal);
		        }else if (terminal.getParamRenova() != null && terminal.getParamRenova().equals("0")) {
		        	logger.debug("SIN RENOVACION");   
		        	logger.debug("entrada.getParamRenova(): "+terminal.getParamRenova());
		        	                                                         //PV_PreCarTerminal_NoPreLis_PR 
					arregloPreciosCargos = terminalBO.getPrecioCargoTerminal_NoPrecioLista_PV(terminal);
				}																
			}
