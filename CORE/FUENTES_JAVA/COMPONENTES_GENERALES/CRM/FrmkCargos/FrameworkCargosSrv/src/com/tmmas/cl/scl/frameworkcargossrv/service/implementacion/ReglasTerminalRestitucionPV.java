...
public class ReglasTerminalRestitucionPV extends ReglaListaPrecio{
	private final Logger logger = Logger.getLogger(ReglasTerminalRestitucionPV.class);

	private ParametrosReglasTerminalRestitucionDTO parametrosRegla;
    ...
    private ParametrosCargoTerminalRestitucionDTO terminal;
    ...
    private static String IndPrecioDeducible = "TRUE"; 		//< TRUE: considera precio lista >
    private static String IndEquipo = "E";
    private static String CodigoUsoPrepago = "3";
    
    private PrecioDTO[] preciosTerminal;
    
	
	public ReglasTerminalRestitucionPV(ParametrosReglasTerminalRestitucionDTO parametros) {
		super(parametros);
	...
	public PrecioDTO[] seleccionPrecios() throws FrameworkCargosException{
		...
		terminal = new ParametrosCargoTerminalRestitucionDTO();
		...
		terminal.setCodigoCategoria(parametrosRegla.getCodigoCategoria()); //NU
		terminal.setModalidadVenta(parametrosRegla.getCodigoModalidadVenta());
		terminal.setTipoContrato(parametrosRegla.getTipoContrato()); //NU
		terminal.setCodigoUsoPrepago(CodigoUsoPrepago); //NU
		terminal.setPlanTarifario(parametrosRegla.getCodigoPlanTarifario()); //NU
		
		terminal.setParamRenova(parametrosRegla.getParamRenova());              // INI P-MIX-09003 OCB;
		terminal.setNumAbonado(Long.parseLong(parametrosRegla.getNumAbonado()));// INI P-MIX-09003 OCB;
		
		terminal.setNumeroSerie(parametrosRegla.getNumeroSerie()); //CSR11003
		
		if(parametrosRegla.getCodigoAntiguedad() != null){
			terminal.setCodigoAntiguedad(Integer.valueOf(parametrosRegla.getCodigoAntiguedad())); //CSR11003
		}else{
			terminal.setCodigoAntiguedad(null); //CSR11003
		}
		
		if(parametrosRegla.getNumMesesProrroga() != null){
			terminal.setNumMeses(Integer.valueOf(parametrosRegla.getNumMesesProrroga()));//CSR11003
		}else{
			terminal.setNumMeses(null);//CSR11003
		}
		
		terminal.setNombreUsuario(parametrosRegla.getNombre()); //CSR11003
		
		if(parametrosRegla.getPromoCelular() != null){
			terminal.setPromoCelular(Integer.valueOf(parametrosRegla.getPromoCelular())); //CSR11003
		}else{
			terminal.setPromoCelular(null); //CSR11003
		}
		
		logger.debug("PARAMETROS PARA LA OBTENCION DE CARGOS POR EQUIPO INI --------->");
		logger.debug("terminal.getCodigoArticulo() ["+terminal.getCodigoArticulo()+"]");
		logger.debug("terminal.getTipoStock()      ["+terminal.getTipoStock()+"]");
		logger.debug("terminal.getCodigoUso()      ["+terminal.getCodigoUso()+"]");
		logger.debug("terminal.getEstado()         ["+terminal.getEstado()+"]");
		logger.debug("terminal.getCodigoCategoria()["+terminal.getCodigoCategoria()+"]");
		logger.debug("terminal.getModalidadVenta() ["+terminal.getModalidadVenta()+"]");
		logger.debug("terminal.getTipoContrato()   ["+terminal.getTipoContrato()+"]");
		logger.debug("terminal.getCodigoUsoPrepag()["+terminal.getCodigoUsoPrepago()+"]");
		logger.debug("terminal.getPlanTarifario()  ["+terminal.getPlanTarifario()+"]");
		logger.debug("terminal.getParamRenova()    ["+terminal.getParamRenova()+"]");
		logger.debug("terminal.getNumAbonado()     ["+terminal.getNumAbonado()+"]");
		logger.debug("terminal.getNumeroSerie()     ["+terminal.getNumeroSerie()+"]"); 
		logger.debug("terminal.getCodigoAntiguedad()["+terminal.getCodigoAntiguedad()+"]");
		logger.debug("terminal.getNumMeses()["+terminal.getNumMeses()+"]");
		logger.debug("terminal.getNombreUsuario()["+terminal.getNombreUsuario()+"]"); 
		logger.debug("terminal.getPromoCelular()["+terminal.getPromoCelular()+"]");
		logger.debug("PARAMETROS PARA LA OBTENCION DE CARGOS POR EQUIPO FIN --------->");
		...
		try {
			...
			if (parametrosRegla.getIndicadorPrecioDeducible().equals(IndPrecioDeducible)){
			
				logger.debug("PRECIO DEDUCIBLE");
				
				arregloPreciosCargos = terminalBO.getPrecioCargoTerminal_Deducible_Rest(terminal);
			
			}else{
				
				if (parametrosRegla.getIndicadorPrecioLista().equals(IndPrecioLista)){
					
					logger.debug("PRECIO LISTA");
					
					arregloPreciosCargos = terminalBO.getPrecioCargoTerminal_PrecioLista_Rest(terminal);
					
				}else{
					
					logger.debug("PRECIO NO LISTA");
					
					if(parametrosRegla.getIndicadorRecambio() != null && "0".equals(parametrosRegla.getIndicadorRecambio())){

						logger.debug("RECAMBIO");
						
						//Con Recambio
						arregloPreciosCargos = terminalBO.getPrecioCargoTerminal_NoPrecioLista_Rest(terminal);

					}else{
						logger.debug("SIN RECAMBIO");
						
						//Sin Recambio
						arregloPreciosCargos = terminalBO.getPrecioCargoTerminal_NoPrecioLista_Rest_PV(terminal);
					}
		...
		this. preciosTerminal = arregloPrecios;
	...
	public DescuentoDTO[] seleccionDescuentos() throws FrameworkCargosException {
		...
		try {
			...
			//SE ASIGNA PARAMETRO DE RENOVACION 15-03-2010
			if (parametrosRegla.getParamRenova().equals("")) {
				parametrosDescto.setParamRenova("0");  
			}else{
				parametrosDescto.setParamRenova(parametrosRegla.getParamRenova());
			}
			
			parametrosDescto.setNumAbonado(Long.parseLong(parametrosRegla.getNumAbonado()));
			logger.info("cod concepto: "+this.preciosTerminal[0].getCodigoConcepto());
			parametrosDescto.setCodigoConcepto(this.preciosTerminal[0].getCodigoConcepto());
...
*/
	...
	public ParametrosReglaDTO getAtributos() throws FrameworkCargosException {
		...
		atributos.setNumContrato(parametrosRegla.getNumeroContrato());
		atributos.setCodTecnologia(parametrosRegla.getCodigoTecnologia());
