...
public class ReglasSimcardRestitucion extends ReglaListaPrecio{

	private final Logger logger = Logger.getLogger(ReglasSimcardRestitucion.class);
	private ParametrosReglasSimcardRestitucionDTO parametrosRegla;
	...
	private ParametrosCargoSimcardRestitucionDTO simcard;
	...
	public ReglasSimcardRestitucion(ParametrosReglasSimcardRestitucionDTO parametros) {
		super(parametros);
	...
	public PrecioDTO[] seleccionPrecios() throws FrameworkCargosException {
		...
		simcard = new ParametrosCargoSimcardRestitucionDTO();
		...
		simcard.setNumAbonado(Long.parseLong(parametrosRegla.getNumAbonado()));
		simcard.setParamRenova(parametrosRegla.getParamRenova()); 
		...
		try {
			if (parametrosRegla.getIndicadorPrecioLista().equals(IndPrecioLista)){
				...
				arregloPreciosCargos = simcardBO.getPrecioCargoSimcard_PrecioLista_Rest(simcard);
			}else{
				simcard.setIndicadorEquipo(IndEquipo);
				simcard.setIndiceRecambio(parametrosRegla.getIndiceRecambioNoLista());
				simcard.setNumAbonado(Long.parseLong(parametrosRegla.getNumAbonado()));
				arregloPreciosCargos = simcardBO.getPrecioCargoSimcard_NoPrecioLista_Rest(simcard);				
			}
	...
	public DescuentoDTO[] seleccionDescuentos() {
		...
		try {
			...
			if (parametrosRegla.getParamRenova()!=null) {
				parametrosDescto.setParamRenova("1");  
			}
