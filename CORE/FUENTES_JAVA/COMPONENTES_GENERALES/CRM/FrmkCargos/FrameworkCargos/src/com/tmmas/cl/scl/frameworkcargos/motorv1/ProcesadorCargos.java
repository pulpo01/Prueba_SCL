...
import com.tmmas.scl.framework.customerDomain.businessObject.bo.ArticuloFCBOFactory;
...
//	TODO : Metodos de Acceso a BD a través de Factory
	...
	public void calcularCargos(ReglaListaPrecio[] reglasNegocio) throws FrameworkCargosException {
	...
	public void registrarCargos(TareasRegistroCargos[] tareas) throws FrameworkCargosException{
		...
		try{
			...
			cliente.setCodCliente(Long.parseLong(configuracion.getCodigoCliente()));
			...
			for (int i=0;i<cargosDTO.length;i++)
			{
				...
				//incidencia 010909 pv ini
				if("0".equals(registroCargos.getCodigoCicloFacturacion()))
				{
					registroCargos.setCodigoCicloFacturacion(null);
				}
				cat.debug("registroCargos.getCodigoCicloFacturacion["+registroCargos.getCodigoCicloFacturacion()+"]");
				//incidencia 010909 pv fin
				...
				if (cargosDTO[i].getDescuento()!=null)
					if (cargosDTO[i].getDescuento().length>0){
						for (int k=0; k<cargosDTO[i].getDescuento().length;k++){
							...
							/**
							 * @author rlozano 
							 * @description Se registra código de descuento manual en caso que no exista código de descuento asociado y el valor de descuento es distinto de 0
							 * 
							 * */
							registroCargos.setCodigoConceptoDescuento(cargosDTO[i].getDescuento()[k].getCodigoConcepto());
						}
				...
				registroCargos.setNumeroTerminal(atributo.getNumeroCelular());
