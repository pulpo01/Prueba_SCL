...
public class ServicioSuplementarioDAO extends ConnectionDAO implements ServicioSuplementarioDAOIT{
	...
	throws ProductException{
		...
		try {
			...
			String call = getSQLDatosServSupl("VE_serv_suplem_abo_PG","VE_ProcesoSSAbonado_PR",6);
			...
			logger.debug("Inicio:creaSSAbonado:execute");
			cstmt.execute();
			logger.debug("Fin:creaSSAbonado:execute");

			codError=cstmt.getInt(4);
			msgError = cstmt.getString(5);
			numEvento=cstmt.getInt(6);
			logger.debug("msgError[" + codError + "]");
			logger.debug("msgError[" + msgError + "]");			
			logger.debug("msgError[" + numEvento + "]");
			
		} catch (Exception e) {
			logger.error("Ocurrió un error al insertar al crear servicios suplemetarios del abonado",e);
	...
	throws ProductException{
		logger.debug("Inicio:getListaSSAbonadoParaCentral()");
		int codError = 0;
		String msgError = null;
		int numEvento = 0;
		ServicioSuplementarioDTO[] resultado = null;
		...
		logger.debug("Coneccion obtenida OK");
		try {
			String call = getSQLDatosServSupl("VE_serv_suplem_abo_PG","VE_obtiene_SSabo_paracent_PR",6);
			...
			logger.debug("Inicio:getListaSSAbonadoParaCentral:Execute");
			cstmt.execute();
			logger.debug("Fin:getListaSSAbonadoParaCentral:Execute");
		...
		logger.debug("Fin:getListaSSAbonadoParaCentral()");
