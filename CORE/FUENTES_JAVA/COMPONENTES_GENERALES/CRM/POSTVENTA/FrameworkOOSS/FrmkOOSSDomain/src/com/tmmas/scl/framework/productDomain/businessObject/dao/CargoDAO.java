...
import com.tmmas.scl.framework.productDomain.productOfferingABE.dataTransferObject.FormaPagoDTO;
...
public class CargoDAO extends ConnectionDAO implements CargoDAOIT {
	...
	private String getSQLactualizarCargoBolsaDinamica() {
		StringBuffer call = new StringBuffer();
		call.append(" DECLARE ");
		...
		call.append("   eo_bolsa.COD_PLANTARIF2 := ?;");
		call.append("   eo_bolsa.COD_CARGO := ?;");
		call.append("   eo_bolsa.IMP_CARGO := ?;");
		call.append("   eo_bolsa.IMP_MAXIMO:= ?;");
		call.append("   PV_CARGOS_PG.PV_ACTUA_CARGO_BOLSA_DINA_PR( eo_bolsa, ?, ?, ?);");
		call.append(" END;");
		return call.toString();		
	}	
	...
	private String getSQLobtenerFormasPago() {
		StringBuffer call = new StringBuffer();
		call.append(" DECLARE ");
		call.append("   so_param PV_BUSQ_FORMA_PAGO_QT := PV_INICIA_ESTRUCTURAS_PG.PV_BUSQ_FORMA_PAGO_QT_FN;");
		call.append(" BEGIN ");
		call.append("   so_param.PARAM_ORDEN_COMPRA_VAL := ?;");
		call.append("   so_param.EXISTE_PLAN_FREEDOM := ?;");
		call.append("   so_param.CATEG_TRIBUTARIA := ?;");
		call.append("   PV_CARGOS_PG.PV_OBTENER_FORMAS_PAGO_PR( so_param ,? ,? ,? ,? );");
		call.append(" END;");		
		return call.toString();		
	}
	
	private String getSQLobtenerTiposDocumentos() {
		StringBuffer call = new StringBuffer();
		call.append(" DECLARE ");
		call.append("   so_param PV_BUSQ_TIPO_DOC_QT := PV_INICIA_ESTRUCTURAS_PG.PV_BUSQ_TIPO_DOC_QT_FN;");
		call.append(" BEGIN ");
		call.append("   so_param.COD_CLIENTE := ?;");
		call.append("   so_param.IND_AGENTE := ?;");
		call.append("   so_param.IND_SITUACION := ?;");
		call.append("   so_param.COD_PRODUCTO := ?;");
		call.append("   so_param.COD_AMICICLO := ?;");
		call.append("   so_param.COD_MODULO := ?;");
		call.append("   PV_CARGOS_PG.PV_OBTENER_TIPOS_DOC_PR( so_param ,? ,? ,? ,? );");
		call.append(" END;");		
		return call.toString();		
	}	
	...
	private String getSQLEliminarPresupuesto() {
		...
		call.append("   fa_presupuesto.NUM_VENTA := ?;");
	...
	private String getSQLEliminarCargosGeCargos() {
		StringBuffer call = new StringBuffer();
		call.append(" DECLARE "); 
		call.append("   REGISTRO GE_CARGOS_QT;");
		call.append("   SN_COD_RETORNO NUMBER;");
		call.append("   SV_MENS_RETORNO VARCHAR2(200);");
		call.append("   SN_NUM_EVENTO NUMBER;");
		call.append(" BEGIN ");
		call.append("   REGISTRO := NULL;  Modify the code to initialize this parameter");
		call.append("   SN_COD_RETORNO := NULL;");
		call.append("   SV_MENS_RETORNO := NULL;");
		call.append("   SN_NUM_EVENTO := NULL;");
		call.append("   GE_CARGOS_SP_PG.GE_CARGOS_D_PR ( REGISTRO, SN_COD_RETORNO, SV_MENS_RETORNO, SN_NUM_EVENTO );");
		call.append(" END;"); 	
		return call.toString();		
	}
	...
	public RetornoDTO eliminarPresupuesto(RegistroFacturacionDTO registro)
		...
		try {
			...
			cstmt.setString(2, registro.getNumeroVenta());
	...
	public RetornoDTO actualizarCargoBolsaDinamica(BolsaDinamicaDTO bolsa)
		...
		String call = getSQLactualizarCargoBolsaDinamica();
		try {
		
			logger.debug("bolsa.getIdSecuencia()[" + bolsa.getIdSecuencia() + "]");
			logger.debug("bolsa.getCodCliente()[" + bolsa.getCodCliente() + "]");
			logger.debug("bolsa.getCodCiclo()[" + bolsa.getCodCiclo() + "]");
			logger.debug("bolsa.getCodPlanTarif()[" + bolsa.getCodPlanTarif() + "]");
			logger.debug("bolsa.getCodPlanTarifDestino()[" + bolsa.getCodPlanTarifDestino() + "]");
			logger.debug("bolsa.getCodCargoBasico()[" + bolsa.getCodCargoBasico() + "]");
			logger.debug("bolsa.getImpCargo()[" + bolsa.getImpCargo() + "]");
			logger.debug("bolsa.getImpMaximo()[" + bolsa.getImpMaximo() + "]");
			...
			cstmt.setString(5, bolsa.getCodPlanTarifDestino());
			cstmt.setString(6, bolsa.getCodCargoBasico());
			cstmt.setFloat(7, bolsa.getImpCargo());
			cstmt.setFloat(8, bolsa.getImpMaximo());
			...
			if (codError != 0) {
				logger.error(" Ocurrió un error al actualizar bolsa dinamica");
				throw new ProductOfferingPriceException(String.valueOf(codError),numEvento, msgError);
			}
	...
	public RetornoDTO aceptarPresupuesto(PresupuestoDTO presup) throws ProductOfferingPriceException{
		...
		try {
			...
			presup.setFechaVcto(new Date(sysdate));
			...
			cstmt.setTimestamp(4, new Timestamp(sysdate)); //FECHA_VCTO
			cstmt.registerOutParameter(5, java.sql.Types.NUMERIC);
	...
	public FormaPagoListDTO obtenerFormasPago(BusquedaFormasPagoDTO param) throws ProductOfferingPriceException{
		...
		String msgError = null;
		int numEvento = 0;
		FormaPagoListDTO formasPagoList = null;
		FormaPagoDTO[] formasPago = null;
		Connection conn = null;
		CallableStatement cstmt = null;
		
		String call = getSQLobtenerFormasPago();
		try {
			
			logger.debug("param.getParamOrdenCompraValor()[" + param.getParamOrdenCompraValor() + "]");
			logger.debug("param.getExistePlanFreedom()[" + param.getExistePlanFreedom() + "]");
			logger.debug("param.getCategoriaTributaria()[" + param.getCategoriaTributaria() + "]");
			
			conn = getConnectionFromWLSInitialContext(global.getJndiForSCLDataSource());
			
			cstmt = conn.prepareCall(call);
			cstmt.setString(1, param.getParamOrdenCompraValor()); //PARAM_ORDEN_COMPRA_VAL
			cstmt.setString(2, param.getExistePlanFreedom()); //EXISTE_PLAN_FREEDOM
			cstmt.setString(3, param.getCategoriaTributaria()); //CATEG_TRIBUTARIA
			...
			if (codError != 0) {
				logger.error(" Ocurrió un error al obtener lista de formas de pago");
				throw new ProductOfferingPriceException(String.valueOf(codError),numEvento, msgError);
			}
			...
			while (rs.next()) {
				...
				formaPago.setTipoValor(rs.getString(1));
				formaPago.setDescripcionTipoValor(rs.getString(2));
				
				logger.debug("TipoValor[" + formaPago.getTipoValor() + "]");
				logger.debug("DescripcionTipoValor[" + formaPago.getDescripcionTipoValor() + "]");
				
				lista.add(formaPago);
			}
			
			formasPago = (FormaPagoDTO[]) ArrayUtl.copiaArregloTipoEspecifico(	lista.toArray(), FormaPagoDTO.class);
			formasPagoList = new FormaPagoListDTO();
			formasPagoList.setFormasPago(formasPago);		
			...
			logger.error("Ocurrió un error general al obtener lista de formas de pago", e);
			throw new ProductOfferingPriceException("Ocurrió un error general al obtener lista de formas de pago",e);
		}
		finally {
			try {
				if (cstmt != null) {
	...
	/**
	 * Obtiene un listado de tipos de documentos que son aplicables a un cliente
	 * 
	 * @param  param
	...
	public DocumentoListDTO obtenerTiposDocumentos(BusquedaTiposDocumentoDTO param) throws ProductOfferingPriceException{
		...
		String msgError = null;
		int numEvento = 0;
		DocumentoListDTO documentosList = null;
		DocumentoDTO[] documentos = null;
		Connection conn = null;
		CallableStatement cstmt = null;
		
		String call = getSQLobtenerTiposDocumentos();
		try {
		  
			logger.debug("param.getCodCliente()[" + param.getCodCliente() + "]");
			logger.debug("param.getIndAgente()[" + param.getIndAgente() + "]");
			logger.debug("param.getIndSituacion()[" + param.getIndSituacion() + "]");
			logger.debug("param.getCodProducto()[" + param.getCodProducto() + "]");
			logger.debug("param.getCodAmiCiclo()[" + param.getCodAmiCiclo() + "]");
			...
			cstmt.setString(2, param.getIndAgente());   //IND_AGENTE
			cstmt.setString(3, param.getIndSituacion());//IND_SITUACION
			cstmt.setInt(4, param.getCodProducto()); //COD_PRODUCTO
			cstmt.setString(5, param.getCodAmiCiclo()); //COD_AMICICLO
			cstmt.setString(6, param.getCodModulo());   //COD_MODULO
			...
			if (codError != 0) {
				logger.error(" Ocurrió un error al obtener lista de tipos de documentos");
				throw new ProductOfferingPriceException(String.valueOf(codError),numEvento, msgError);
			}
			
			logger.debug("Recuperando cursor...");
			logger.debug("object="+cstmt.getObject(7));
			...
			while (rs.next()) {
				...
				logger.debug("Codigo[" + documento.getCodigo() + "]");
				logger.debug("Descripcion[" + documento.getDescripcion() + "]");
				logger.debug("CategoriaTributaria[" + documento.getCategoriaTributaria() + "]");
				logger.debug("TipoFoliacion[" + documento.getTipoFoliacion() + "]");

				lista.add(documento);
			}
			
			documentos = (DocumentoDTO[]) ArrayUtl.copiaArregloTipoEspecifico(	lista.toArray(), DocumentoDTO.class);
			documentosList = new DocumentoListDTO();
			documentosList.setDocumentos(documentos);		
			...
			logger.error("Ocurrió un error general al obtener lista de tipos de documentos", e);
			throw new ProductOfferingPriceException("Ocurrió un error general al obtener lista de tipos de documentos",e);
		}
		finally {
			try {
				if (cstmt != null) {
...
//				obtener info del cliente y completar respuesta
	...
	/*public RetornoDTO eliminarCargosGeCargos(GeCargoDTO productoContratado) throws ServiceException
	{
