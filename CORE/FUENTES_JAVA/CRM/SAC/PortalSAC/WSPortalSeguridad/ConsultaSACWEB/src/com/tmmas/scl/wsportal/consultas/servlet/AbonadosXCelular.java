...
public class AbonadosXCelular extends ConsultasBaseServlet implements javax.servlet.Servlet
{

	private static final String NOMBRE_PARAMETRO = "numCelular";

	private static final long serialVersionUID = -3757700981805324457L;

	private static Logger logger = Logger.getLogger(AbonadosXCelular.class);

	/* (non-Java-doc)
	 * @see javax.servlet.http.HttpServlet#HttpServlet()
	 */
	public AbonadosXCelular()
	{
		super();
	}
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		...
		try
		{
			...
			if (!existe)
			{
				throw new PortalSACException(config.getString("COD.ERR_SAC.5008"),
						config.getString("DES.ERR_SAC.5008"), 0);
			}

			WSPortalSrv srv = new WSPortalSrv();
			Long numCelular = new Long(request.getParameter(NOMBRE_PARAMETRO));
			logger.debug(NOMBRE_PARAMETRO + " [" + numCelular + "]");
			r = srv.abonadosXCelular(numCelular);
