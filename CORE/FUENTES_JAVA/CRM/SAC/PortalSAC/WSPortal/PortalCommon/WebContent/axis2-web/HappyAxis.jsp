...
<%!/*
	 ...
	 * @throws IOException
	 */
	int probeClass(JspWriter out, String category, String classname,
			String jarFile, String axisOperation, String errorText,
		...
		try {
			...
			if (clazz == null) {
				...
				out.write("<p>" + category + ": could not find class "
						+ classname + " from file <b>" + jarFile + "</b><br>  "
						+ errorText + url + "<p>");
				return 1;
			} else {
			...
			out.write("<p>" + category + ": could not find a dependency"
					+ " of class " + classname + " from file <b>" + jarFile
					+ "</b><br> " + errorText + url
	 ...
	 */
	int needClass(JspWriter out, String classname, String jarFile,
			String axisOperation, String errorText, String homePage)
			throws IOException {
		return probeClass(out, "<b>Error</b>", classname, jarFile,
				axisOperation, errorText, homePage);
	}
	 ...
	 * @return the number of missing libraries (0 or 1)
	 */
	int wantClass(JspWriter out, String classname, String jarFile,
			String axisOperation, String errorText, String homePage)
			throws IOException {
		return probeClass(out, "<b>Warning</b>", classname, jarFile,
				axisOperation, errorText, homePage);
	}
	 ...
	 * @throws Exception
	 */
	int wantResource(JspWriter out, String resource, String errorText)
			throws Exception {
	...
	private OMElement createEnvelope() {
		...
		OMNamespace omNs = fac.createOMNamespace(
				"http://axisversion.sample/xsd", "ns1");
...
<%
	...
	needed = needClass(out,
			...
			"axis2-1.0.jar", "Apache-Axis", "Axis2 will not work",
	...
	needed += needClass(out, "javax.xml.stream.XMLStreamReader",
			"stax-api-1.0.1.jar", "Streaming API for XML",
			"Axis2 will not work",
...
%> <%
 	...
 	if (needed == 0) {
 		//yes, be happy
 		out
 				.write("<p><font color='green'><strong>The core axis2 libraries are present.</strong></font></p>");
 	} else {
 		//no, be very unhappy
 		response
 				.setStatus(HttpServletResponse.SC_INTERNAL_SERVER_ERROR);
 		out.write("<font color='red'><i>" + needed
 				+ " core axis2 librar"
 ...
 %>
<p><B><I>Note:</I></B> Even if everything this page probes for is
...
tests are <i>necessary</i> but not <i>sufficient</i></p>
</blockquote>
...
<%
	...
	if (serviceStatus) {
%>
<blockquote><font color="green"><strong>
Found Axis2 default Version service and Axis2 is working properly.</strong></font>
...
%>
<p><font color="brown"> There was a problem in Axis2 version
service , may be the service not available or some thing has gone wrong.
But this does not mean system is not working ! Try to upload some other
service and check to see whether it is working. <br>
</font></p>
...
<%
	...
	if (e != null) {
		out.write("<pre>");
		out
				.write("<table cellpadding='5px' cellspacing='0px' style='border: .5px blue solid;'>");
		for (; e.hasMoreElements();) {
			...
			out.write("<th style='border: .5px #A3BBFF solid;'>" + key
					+ "</th>");
			out
					.write("<td style='border: .5px #A3BBFF solid;'>"
							+ getFormatedSystemProperty(System
									.getProperty(key)) + "&nbsp;</td>");
			out.write("<tr>");
		}
		out.write("</table>");
