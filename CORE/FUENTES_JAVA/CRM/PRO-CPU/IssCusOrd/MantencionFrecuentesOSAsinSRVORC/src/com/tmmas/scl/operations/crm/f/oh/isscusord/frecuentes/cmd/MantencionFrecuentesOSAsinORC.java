package com.tmmas.scl.operations.crm.f.oh.isscusord.frecuentes.cmd;
...
import com.tmmas.scl.operations.crm.f.oh.isscusord.frecuentes.cmd.helper.Global;
...
public class MantencionFrecuentesOSAsinORC extends OrdenServicioAsincronoCmdORC {


	private static final long serialVersionUID = 1L;

	transient static Logger cat = Logger.getLogger(MantencionFrecuentesOSAsinORC.class);
	transient private AsyncProcessParameterObject parametros = null;
	Map datosExtra = new HashMap();

	public MantencionFrecuentesOSAsinORC() {
		// TODO Auto-generated constructor stub
	}

	public MantencionFrecuentesOSAsinORC(AsyncProcessParameterObject value) {
		super(value);
		// TODO Auto-generated constructor stub
	}
	...
	throws IssCusOrdException {
		...
		try{
			registroOrden((ParamMantencionFrecuentesDTO) objeto);
	...
	public void registroOrden(ParamMantencionFrecuentesDTO objeto) throws InstantiationException, IllegalAccessException, ClassNotFoundException, JDOMException, IOException, SecurityException, IllegalArgumentException, InvocationTargetException, NoSuchFieldException, NoSuchMethodException, IssCusOrdException
	{
		NumeroListDTO numeroListAdd = objeto.getNumeroListAdd();//Números nuevos a agregar
		NumeroListDTO numeroListDel = objeto.getNumeroListDel();//Números antiguos a eliminar
		
		ejecutarRegistroOrden(objeto);
		
		if(numeroListAdd!=null){
	        	ejecutarAgregarFrecuentes(numeroListAdd);
	    }
	    if(numeroListDel!=null){
	        	ejecutarEliminarFrecuentes(numeroListDel);
	    }
	    //TODO Revisar si esto está funcionando 110809 pv
	    /*
	    boolean insertaEvento = verificarInsertEvento(numeroListAdd,numeroListDel);
	...
	private void ejecutarAgregarFrecuentes(NumeroListDTO numeroListAdd)//	throws NegSalException 
	{
		...
		{		
			...
			NumeroDTO[] numDtoarr = numeroListAdd.getNumerosDTO();
			if(numDtoarr != null)
			{
				cat.debug("NUMEROS A AGREGAR POR ENCOLAMIENTO "+numDtoarr.length);
				for(int i=0;i<numDtoarr.length;i++)
				{
					cat.debug("numDtoarr[i].getNro() "+numDtoarr[i].getNro());
				}
			}
			retorno = getManCusInvFacade().crearListaNumeros(numeroListAdd);
			//agregar rollback
		}catch(Exception e)
		{
			String loge = StackTraceUtl.getStackTrace(e);
	...
	private void ejecutarEliminarFrecuentes(NumeroListDTO numeroListDel)//	throws NegSalException 
	{
		...
		{		
			...
			//retorno = this.getIssCusOrdORC().descontratarOfertaComercial(productoContratadoList);
			//debo llar a fachada con control del rollback o sea como en negsal
			NumeroDTO[] numDtoarr = numeroListDel.getNumerosDTO();
			if(numDtoarr != null)
			{
				cat.debug("NUMEROS A ELIMINAR POR ENCOLAMIENTO "+numDtoarr.length);
				for(int i=0;i<numDtoarr.length;i++)
				{
					cat.debug("numDtoarr[i].getNro() "+numDtoarr[i].getNro());
				}
	...
	private void ejecutarRegistroOrden(ParamMantencionFrecuentesDTO param){
	...
	private boolean verificarInsertEvento(NumeroListDTO numeroListAdd, NumeroListDTO numeroListDel) {
		//Para la mantencion de Productos se debe verificar si elimina lista de numeros
		if(numeroListAdd != null && numeroListAdd.getNumerosDTO() != null && numeroListAdd.getNumerosDTO().length >0)
		{
			cat.debug("add getNumerosDTO()[0].getNro()[" + numeroListAdd.getNumerosDTO()[0].getNro() + "]");
			return true;
		}
		//Para la mantencion de Productos esto no se debe preguntar ya que se crean listas vacias
		if(numeroListDel != null && numeroListDel.getNumerosDTO() != null && numeroListDel.getNumerosDTO().length >0)
		{
			cat.debug("del getNumerosDTO()[0].getNro()[" + numeroListDel.getNumerosDTO()[0].getNro() + "]");
			return true;
		}
		return false;
	}
	
	private void insertarEventoNumFrec(ParamMantencionFrecuentesDTO param) throws IssCusOrdException {
		...
		{		
			...
			cat.debug("eventoNumFrecDTO.getNumAbonado["+eventoNumFrecDTO.getNumAbonado()+"]");
			cat.debug("eventoNumFrecDTO.getCodCliente["+eventoNumFrecDTO.getCodCliente()+"]");
			cat.debug("eventoNumFrecDTO.getCodOOSS   ["+eventoNumFrecDTO.getCodOOSS()+"]");
			cat.debug("eventoNumFrecDTO.getNumOOSS   ["+eventoNumFrecDTO.getNumOOSS()+"]");
			
			cat.debug("insertarEventoNumFrec() :inicio");
			getCloCusOrdFacade().insertarEventoNumFrec(eventoNumFrecDTO);
	...
	private ManCusInvFacade getManCusInvFacade() throws ManCusInvException
	{
		...
		ManCusInvFacade manCusBilFacade = null;

		try 
		{
			manCusBilFacade = manCusInvFacadeHome.create();
		}
