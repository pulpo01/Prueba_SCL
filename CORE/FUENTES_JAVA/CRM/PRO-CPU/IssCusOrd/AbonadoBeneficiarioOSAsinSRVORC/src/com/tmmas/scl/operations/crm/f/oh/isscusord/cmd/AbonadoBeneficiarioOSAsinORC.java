...
import com.tmmas.scl.operations.crm.f.oh.isscusord.cmd.helper.GlobalAB;




public class AbonadoBeneficiarioOSAsinORC extends OrdenServicioAsincronoCmdORC {


	private static final long serialVersionUID = 1L;

	transient static Logger cat = Logger.getLogger(AbonadoBeneficiarioOSAsinORC.class);
	transient private AsyncProcessParameterObject parametros = null;
	Map datosExtra = new HashMap();

	public AbonadoBeneficiarioOSAsinORC() {
		// TODO Auto-generated constructor stub
	}

	public AbonadoBeneficiarioOSAsinORC(AsyncProcessParameterObject value) {
		super(value);
		// TODO Auto-generated constructor stub
	}
	...
	throws IssCusOrdException {
		...
		try{
			registroOrden((ParamAbonBenefRegOSDTO) objeto);
	...
	public void registroOrden(ParamAbonBenefRegOSDTO objeto) throws InstantiationException, IllegalAccessException, ClassNotFoundException, JDOMException, IOException, SecurityException, IllegalArgumentException, InvocationTargetException, NoSuchFieldException, NoSuchMethodException, IssCusOrdException
	{
		AbonadoBeneficiarioListDTO abonadosAgregar = objeto.getAbonadosBenefiariosNuevos();//Números nuevos a agregar
		AbonadoBeneficiarioListDTO abonadosActualizar = objeto.getAbonadosBenefiariosEliminar();//Números antiguos a eliminar
		
		 ejecutarRegistroOrden(objeto);
		
		if (abonadosAgregar!=null&&abonadosAgregar.getAbonadoBeneficiarioList()!=null&&abonadosAgregar.getAbonadoBeneficiarioList().length>0){
			ejecutarAgregarAbonadosBeneficiarios(abonadosAgregar);
		}
		if (abonadosActualizar!=null&&abonadosActualizar.getAbonadoBeneficiarioList()!=null&&abonadosActualizar.getAbonadoBeneficiarioList().length>0){
			ejecutarEliminarAbonadosBeneficiarios(abonadosActualizar);
		}
	...
	private void ejecutarAgregarAbonadosBeneficiarios(AbonadoBeneficiarioListDTO abonadoBeneficiarioListDTO)//	throws NegSalException 
	{
		...
		{		
			...
			retorno = getManConFacade().agregaAbonadosBeneficiarios(abonadoBeneficiarioListDTO);
			//agregar rollback
		}catch(Exception e)
		{
			String loge = StackTraceUtl.getStackTrace(e);
	...
	private void ejecutarEliminarAbonadosBeneficiarios(AbonadoBeneficiarioListDTO abonadoBeneficiarioListDTO)//	throws NegSalException 
	{
		...
		{		
			...
			retorno = getManConFacade().eliminaAbonadoBeneficiario(abonadoBeneficiarioListDTO);
			//agregar rollback
		}catch(Exception e)
		{
			String loge = StackTraceUtl.getStackTrace(e);
	...
	private void ejecutarRegistroOrden(ParamAbonBenefRegOSDTO param){
	...
	cat.debug("start():start");
		...
		try {
			...
			throw new GeneralException("A ocurrido un error al eliminar Abonados Beneficiarios", e); 
		}
		cat.debug("start():end");
		return null;
	}
	...
	{
		GlobalAB global = GlobalAB.getInstance();		
