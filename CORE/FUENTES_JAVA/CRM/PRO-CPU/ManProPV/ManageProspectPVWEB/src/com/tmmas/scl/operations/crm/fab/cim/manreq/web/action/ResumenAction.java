...
public class ResumenAction extends Action {
	...
	throws Exception {
		...
		/**
		 * @author rlozano
		 * @description se realiza validacion para las ordenes de servicio de producto,
		 * no aplicar para :
		 * OOSS 41001 : Contratacion y descontratacion de productos
		 * OOSS 41004 : Abonados Beneficiarios
		 * OOSS 41005 : Abonados Vetados
		 * OOSS 41006 : Mantención de Límite de Consumo Plan Adicional
		 * OOSS 41007 : Abono a Límite de Consumo	
		 * @date 17-11-2008
		 */
		boolean aplicaOOSSRegPlan=sessionData.getCodOrdenServicio()==41001||sessionData.getCodOrdenServicio()==41004||
								  sessionData.getCodOrdenServicio()==41005||sessionData.getCodOrdenServicio()==41006||
								  sessionData.getCodOrdenServicio()==41007?false:true;
		if(sessionData.getCodOrdenServicio()!=40009&&aplicaOOSSRegPlan){
		...
		long codOOSS=sessionData.getCodOrdenServicio();
		
		if(codOOSS==40009){
			/**
			 * @author rlozano
			 * @description Se modifica properties asi, como la asignacion para poder soportar ambas casuísticas.
			 * @date 22-10-2008
			 */
			codOSAnt = codOOSS==40009?global.getValor("codOS.anula.solicitud"):global.getValor("codOS.contratacion.producto");
			nombreOS = codOOSS==40009?global.getValor("nomOS.anula.solicitud"):global.getValor("nomOS.contratacion.producto");
			
		}else{
			if(!registroPlan.getParamRegistroPlan().getCombinatoria().equals("")){
		...
		*/
		...
		if(botonSeleccionado!=null&&botonSeleccionado.equalsIgnoreCase("anterior")){
			...
			if( (vaACargos!=null&&vaACargos.equalsIgnoreCase("SI"))||(sessionData.getObtenerCargos()!=null&& sessionData.getObtenerCargos().equalsIgnoreCase("NO"))
			   || (tablaCargos == null ||tablaCargos.length == 0) ){
				...
				cargosForm.setBotonSeleccionado("retrocedio");//anterior");
		...
		if(botonSeleccionado!=null&&botonSeleccionado.equals("visualizar")){
			...
			String rutaReporte = global.getPathInstancia() + global.getValor("ruta.reportes")+global.getValor("reporte.carta");
