...
public class CargosContDescPlanAction extends BaseAction{
	
	private final Logger logger = Logger.getLogger(CargosContDescPlanAction.class);
	...
	public ActionForward executeAction(ActionMapping mapping, ActionForm form,
		...
		logger.debug("No se cobran cargos de negocio para Contratar y Descontratar");
		...
		/*pv 300709 monto por evento de num frec*/
		EventoNumFrecDTO eventoNumeroFrec = new EventoNumFrecDTO();
		eventoNumeroFrec.setCodCliente(sessionData.getCliente().getCodCliente());//sessionData.getCodCliente()
		eventoNumeroFrec.setNumAbonado(sessionData.getNumAbonado());
		eventoNumeroFrec.setCodOOSS(sessionData.getCodOrdenServicio());
		logger.debug("cargos.length ["+cargos.length+"]");
		logger.debug("eventoNumeroFrec = delegate.obtenerMontoEvento(eventoNumeroFrec) antes");
		eventoNumeroFrec = delegate.obtenerMontoEvento(eventoNumeroFrec);
		if(eventoNumeroFrec.getMontoEvento() > 0)
		{
			CargosDTO cargoNumFrecDTO = obtenerCargoEvento(eventoNumeroFrec,sessionData);
			CargosDTO [] cargosConNF = new CargosDTO [cargos.length+1];
			for(int i=0;i<cargos.length;i++)
			{
				cargosConNF[i] = cargos[i];
			}
			cargosConNF[cargosConNF.length] = cargoNumFrecDTO;
			cargos = cargosConNF;
		}
		logger.debug("eventoNumeroFrec = delegate.obtenerMontoEvento(eventoNumeroFrec) despues");
		logger.debug("cargos.length ["+cargos.length+"]");
		/**/
	...
	private CargosDTO obtenerCargoEvento(EventoNumFrecDTO eventoNumeroFrec, ClienteOSSesionDTO sessionData) throws ManReqException {
		PrecioDTO precioDTO = new PrecioDTO();
		precioDTO.setMonto(Float.parseFloat(""+eventoNumeroFrec.getMontoEvento()));
		precioDTO.setCodigoConcepto(""+eventoNumeroFrec.getCodConcepto());
		//precioDTO.setDescripcionConcepto(cargoWeb.getDescripcion());
		precioDTO.setFechaAplicacion(null);
		precioDTO.setValorMaximo("0");
		precioDTO.setValorMinimo("0");
		precioDTO.setIndicadorAutMan("0");//TODO 0 es cargo automatico; 1 cargo manual
		
		MonedaDTO moneda = new MonedaDTO();
		moneda.setCodigo("001");//REVISAR QUE ESTA INFO LLEGUE POR PL 
		moneda.setDescripcion("QUETZAL");
		
		precioDTO.setUnidad(moneda);
		...
		logger.debug("CargosContDescPlanAction obtenerCargoEvento numAbonado: "+sessionData.getAbonados()[0].getNumAbonado());
		precioDTO.setDatosAnexos(atrMig);
		
		CargosDTO cargo = new CargosDTO();
		
		cargo.setPrecio(precioDTO);
		cargo.setCantidad(1);//Integer.parseInt(cargoWeb.getCantidad()));
