...
import com.tmmas.scl.operations.crm.fab.cim.manreq.web.form.ManLcPlanForm;
...
public class MantencionLcPlanAdicionalAction extends Action{
	
	
	private final Logger logger = Logger.getLogger(MantencionLcPlanAdicionalAction.class);
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form,
	    ...
	    ManLcPlanForm manLcPlanForm=(ManLcPlanForm)form;
		boolean condicionesCK = manLcPlanForm.isCondicionesCK();
		...
		session.setAttribute("ManLcPlanForm",manLcPlanForm);
        ...
        /*if (!"OK".equals(retorno.getDescripcion())){
			logger.debug("hubo error al aplicarRestricciones["+retorno.getDescripcion()+"]");
	...
	{
		logger.debug("RAV NavegacionWeb INICO 1");
	    ...
	    sessionData = (ClienteOSSesionDTO)session.getAttribute("ClienteOOSS");
	    //(-)
		logger.debug("RAV navegacion.creaProductoPorAbonado 1 ABONADO: "+ abonado.getNumAbonado());
		
		navegacion.creaProductoPorAbonado(abonado,clienteVtaPar,ordenServicio,sessionData.getNumAbonado(),sessionData);//EV
		
		logger.debug("RAV NavegacionWeb FIN 2");
