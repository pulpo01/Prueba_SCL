...
public class MantencionLcPlanAdicionalRecargaAction extends Action {
	
	private final Logger logger = Logger.getLogger(MantencionLcPlanAdicionalRecargaAction.class);
	...
	private NavegacionWeb obtenerNavegacion(HttpServletRequest request,AbonadoDTO abonado,ClienteDTO clienteVtaPar,OrdenServicioDTO ordenServicio) throws Exception
	{
		...
		logger.debug("RAV NavegacionWeb 0 INICIO");
		...
		logger.debug("RAV navegacion.creaProductoPorAbonado 0 ABONADO: "+ abonado.getNumAbonado());
		
		navegacion.creaProductoPorAbonado(abonado,clienteVtaPar,ordenServicio,sessionData.getNumAbonado(),sessionData);//EV
		
		logger.debug("RAV NavegacionWeb 0 FIN");
		return navegacion;
	}
