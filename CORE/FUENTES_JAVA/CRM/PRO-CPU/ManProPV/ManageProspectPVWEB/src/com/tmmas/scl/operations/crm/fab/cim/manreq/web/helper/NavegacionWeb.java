...
public class NavegacionWeb {
	...
	private String[] listaAbonosBita; //Bitacora de Abonos
	private String[] listaAbonosHabilitados; //Bitacora de Abonos Habilitados
	
    public String[] getListaAbonosHabilitados() {
		return listaAbonosHabilitados;
	}

	public void setListaAbonosHabilitados(String[] listaAbonosHabilitados) {
		this.listaAbonosHabilitados = listaAbonosHabilitados;
	}
	...
	private ArrayList bitacoraDef; //Bitacora de los Lìmites de consumo de productos contratados.
	private ArrayList bitacoraContIndexLcDef;
	private ArrayList bitacoraOferIndexLcDef;
	...
	public void setCantidadesPorProductoOfertado(String idAbonado,String cantidades[]){
		//, ProductoOfertadoListDTO productoOfertadoListCont
	...
	public void setLCPorProductoOfertado(String idAbonado,String limitesDeConsumo[], String[] montoslc){
		...
		ArrayList productoDisponibles = paqueteWeb.getProductosDisponible();
		ArrayList productoDisponibleAux = new ArrayList();
		LimiteConsumoPlanAdicionalListDTO listLC = null;
		for (int proCant=0;proCant<productoDisponibles.size();proCant++) {
			ProductoWeb productoX = (ProductoWeb) productoDisponibles.get(proCant);	
			
			if (productoX.getChequeado() && !productoX.getPaquete().equals("1")){
				listLC = productoX.getProductoDTO().getListaLimCons();
				if(listLC!=null && listLC.getLimitesDeConsumo()!=null && listLC.getLimitesDeConsumo().length >0)
				{
					...
					productoX.setMtoConsumido(""+montoslc[recorreCantidades]);//mix9003 260310 pv
					productoX.setIndexLimConsuInicial(String.valueOf(bitacoraOferIndexLcDef.get(recorreCantidades)));
					recorreCantidades++;
				}
			}
			else
	...
	public void setLCPorProductoContratado(String idAbonado, String[] montoslc_c){
		
		this.paqueteWeb = getPaqueteWeb(idAbonado);
		int recorreCantidades =0;
		ArrayList productoDefecto = paqueteWeb.getProductosContratados();
		ArrayList productoDefectoAux = new ArrayList();
		for (int proCant=0;proCant<productoDefecto.size();proCant++) {
			ProductoWeb productoX = (ProductoWeb) productoDefecto.get(proCant);	
			
			if (productoX.getChequeado() && !productoX.getPaquete().equals("1")){//revisar aca deschequeado
				//productoX.setCodLimConsuSeleccionado(String.valueOf(bitacoraDef.get(recorreCantidades)));		
				productoX.setMtoConsumido(""+montoslc_c[recorreCantidades]);//mix9003 260310 pv
				productoX.setIndexLimConsuInicial(String.valueOf(bitacoraContIndexLcDef.get(recorreCantidades)));
				//productoX.setMtoConsumoConfigurado(new Float()) mix9003
				recorreCantidades++;
			}
	...
	public void setAbonoLimiteConsumoProductos(String idAbonado){
			
			this.paqueteWeb = getPaqueteWeb(idAbonado);
			int recorreCantidades =0;
			ArrayList limiteCosnumoProductos = paqueteWeb.getLimiteConsumoProductos();
			ArrayList limiteCosnumoProductosAux = new ArrayList();
			
			for (int proCant=0;proCant<limiteCosnumoProductos.size();proCant++) {
				ProductoWeb productoX = (ProductoWeb) limiteCosnumoProductos.get(proCant);	
	...
	public void creaProductoPorAbonado(AbonadoDTO abonadoDto,ClienteDTO cliente,OrdenServicioDTO ordenServicio, long numAbonado, ClienteOSSesionDTO sessionData) throws Exception{
		PaqueteWeb paqueteWeb= new PaqueteWeb(abonadoDto,cliente,ordenServicio, numAbonado, sessionData);
		historial.put(String.valueOf(abonadoDto.getNumAbonado()), paqueteWeb);
	}
	
	public void creaLimiteConsuProductoPorAbonado(AbonadoDTO abonadoDto,ClienteDTO cliente,OrdenServicioDTO ordenServicio, long numAbonado, ClienteOSSesionDTO sessionData) throws Exception{
		PaqueteWeb paqueteWeb= new PaqueteWeb(abonadoDto,cliente,ordenServicio, numAbonado, sessionData);
		paqueteWeb.obtenerLimiteConsumoProducto(numAbonado,cliente.getCodCliente());
		historial.put(String.valueOf(abonadoDto.getNumAbonado()), paqueteWeb);
	}
	...
	public void setBitacoraDef(String bitacoraLcPDef) {
		...
		if (bitacoraLcPDef!=null){// se personalizo un producto contratado??				
	...
	public void setBitacoraContIndexLcDef(String bitacoraLcPDef) {
		...
		this.bitacoraContIndexLcDef = limConsu;
	
	}
	
	public void setBitacoraOferIndexLcDef(String bitacoraLcPDef) {
		...
		this.bitacoraOferIndexLcDef = limConsu;
	}
...
//		Obtencion de numeros no frecuentes, a nivel de abonado para validacion
   ...
   public AbonadoWeb getAbonadoWeb() {
	...
	public String[] getListaAbonosBita() {
		return listaAbonosBita;
	}

	public void setListaAbonosBita(String[] listaAbonosBita) {
		this.listaAbonosBita = listaAbonosBita;
	}
