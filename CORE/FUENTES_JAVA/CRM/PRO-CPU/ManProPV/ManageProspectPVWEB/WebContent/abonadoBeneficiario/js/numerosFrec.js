...
{
	document.getElementById('taberr').style.display="none";
}
function muestraMensajeErr(msgErr)
{
	...
	document.getElementById('taberr').style.display="block";
}
...
function validaNumero(numero)
{
	try
	{
		...
		if(!IsNumeric(numero))
		{
			muestraMensajeErr("El número no debe tener letras");
			return false;
		}
	}
	catch(e)
...
function verificaTipoNumRespuesta(tipoEval)
{
  	try
	{
	  	if(tipoEval != null && tipoEval.length > 0)
		{
			...
			{
				muestraMensajeErr("El producto no puede aceptar numeros de tipo "+tipoEval);
			}
		}
	}
	catch(e)
...
function agregarNumeroFrec(tipo)
{
	try
	{
		var numero = document.getElementById('nuevoNumFrec').value;
