...
public class ClienteFrmkDTO {
	...
	public ClienteFrmkDTO(ClienteDTO clienteDTO, PrestacionDTO[] prestaciones) {
		...
		{				
			//(+)obtiene glosa prestacion
			String glsPrestacion = "";
			for(int j = 0; j < prestaciones.length; j++){
				if (abonados[i].getCodPrestacion()!=null && 
						abonados[i].getCodPrestacion().equals(prestaciones[j].getCodPrestacion())){
					glsPrestacion = prestaciones[j].getDesPrestacion();break;
				}
			}
			//(-)
