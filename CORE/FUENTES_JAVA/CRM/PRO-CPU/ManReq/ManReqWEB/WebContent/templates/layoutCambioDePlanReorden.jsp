...
<c:set var="fecDesdeLlam" value="${sessionScope.fecDesdeLlam}"></c:set>
<c:set var="consultaER" value="${sessionScope.consultaEvalRiesgoReo}"></c:set>
...
<script type='text/javascript' src='/ManReqWEB/dwr/interface/JCondicionesOOSSReorden.js'></script>
...
var optClienteAux = document.createElement('option'); //GS 19-05
var consultaEvalRiesgo = false;

<c:if test="${!empty consultaER}">
  <c:if test="${consultaER == true}"> 
  	consultaEvalRiesgo =  true;
  	//alert ("inicio consultaEvalRiesgo:"+consultaEvalRiesgo);
  </c:if>
</c:if>
...
function cargaInicial(){
	//alert ("En cargaInicial layoutCambioReorden:"+consultaEvalRiesgo);
	...
	JCondicionesOOSSReorden.cargaInicial(retornoCargaInicial);
}
...
function retornoCargaInicial(prd){
	...
	// Si el origen en un cliente empresa, solo aplica pospagopospago
...
//valida seleccion de opcion, si no es una opcion valida se debe volver al ultimo estado 
function validarOpcion(){
	...
	if (totalAboSel > 0){
	 ...
	 //validaOpcion
	 JCondicionesOOSSReorden.validaOpcion(parametrosCondicionesOS, arrAbonados, retornoValidaOpcion);

}
//retorno de funcion JCondicionesOOSSReorden.validaOpcion()
...
//valida plan tarifario seleccionado  
 /*function validarPlanTarif(){
	...
	if (totalAboSel > 0){
  	 ...
  	 //validaPlanTarif
 	 JCondicionesOOSSReorden.validaPlanTarif(parametrosCondicionesOS, arrAbonados, retornoValidaPlanTarif);
} */
/*
//retorno de funcion JCondicionesOOSSReorden.validaPlanTarif()
function retornoValidaPlanTarif(data)
{
      if(data != null){
	      var valoresRetorno=data;
	      msg=valoresRetorno['mensaje'];
	      codActAboAux1=valoresRetorno['codActAboAux'];
		  ...
		  $("codActAboAux").value = codActAboAux1;
...
function obtenerPlanesEvaluacionRiesgo()  {
    ...
    JCondicionesOOSSReorden.obtenerPlanesEvaluacion(tipoListaPlan,cliDestinoTipPlanTarif,numIdentCliOrigen,tipIdentCliOrigen,retornoObtenerPlanesEvaluacion);  
}
...
function retornoObtenerPlanesEvaluacion(data){
    ...
    if (data==null){
       mostrarMensajeError("Error al consultar evaluacion de riesgo");
       return;
    }
	...
	var indiceElemEncontrado = null; 
	var planSeleccionado = null;
		
	if (codPlanSeleccionado!=null && listaActualizada!=null ) {
		for (index = 0; index< listaActualizada.length; index++) {
		    if (codPlanSeleccionado == listaActualizada[index].codigoPlanTarifario){
		    	indiceElemEncontrado = index;
    ...
    // Si cliente destino es empresa
    if ($("flgClienteDestinoEmpresa").value == 'S' ){
		if (indiceElemEncontrado!=null){
			// Actualiza la lista en pantalla y selecciona el plan encontrado
			...
			comboPlan.selectedIndex = parseInt(indiceElemEncontrado);
		}
		else {
		   mostrarMensajeError("No existe evaluación de riesgo para el cliente seleccionado");	
		   return;
		}
    }
    else {
	    // Cliente destino individual
	    DWRUtil.removeAllOptions(tipoComboPlan);
	    DWRUtil.addOptions(tipoComboPlan,listaActualizada,"codigoPlanTarifario","descripcionPlanTarifario");
        comboPlan.selectedIndex = parseInt(indiceElemEncontrado);
...
function esCombinatoriaparaVendedor() 
{
	...
	//Busco la configuracion de la combinatoria para la orden de servicio
 	JCondicionesOOSSReorden.recuperarConfiguracionVendedorCPU(configuracionVendedorCPU, {callback:callbackRecuperarConfiguracionVendedorCPU, errorHandler:errorRecuperarConfiguracionVendedorCPU})
	...
	if (xmlVendedorComisionable == "SI" && CombinatoriaConfigurable == 1) {
		...
		JCondicionesOOSSReorden.obtenerInformacionUsuarioVendedor(usuarioSistema, {callback:callbackObtenerInformacionUsuarioVendedor, errorHandler:errorInformacionUsuarioVendedor});
		...
		JCondicionesOOSSReorden.usuarioesdeGrupoComisionable(usuario, {callback:callbackUsuarioesdeGrupoComisionable, errorHandler:errorUsuarioesdeGrupoComisionable} )
		...
		if (grupoVendedorComisionable == "S") {
			...
			JCondicionesOOSSReorden.setUsuarioVendedorenSesion();
	  ...
	  JCondicionesOOSSReorden.removerUsuarioVendedordeSesion();
	}
...
function estaVendedorComisionableConfigurado() {
	return JCondicionesOOSSReorden.estaVendedorComisionableConfigurado(callbackEstaVendedorComisionableConfigurado);
}
...
function validacionCampos()
  {
    ...
    if (opcionComboPlanTarif ==""){
       if (document.getElementById("combinatoria").value != "POSPAGOPOSPAGO" && document.getElementById("combinatoria").value != "HIBRIDOHIBRIDO") {
       		desactivaBotonSiguiente(); 
    ...
    if(document.getElementById("combinatoria").value == "POSPAGOPREPAGO" || document.getElementById("combinatoria").value == "HIBRIDOPREPAGO"){
...
function valoresDefaultCarga()
{
    ...
    // Si el origen es POSPAGO y el origen es empresa, deshabilita PREPAGO	
// PF-PV-182    
//	if (origenTipoPlan=="POSPAGO" && codTipoPlanTarif == "E" ) {
...
{	  
	  ...
	  //llenaClienteDestino();//carga informacion de clientes destino, evera 19/07/08, se comenta llamada
	  generarCombinatoria();
	  activaOpcionCambioPlan(); 
	  var combinatoria = document.getElementById("combinatoria").value;
	  ...
	  //deshabilita buscador de clientes
  	  document.getElementById("buscadorClientesTr").style["display"]="none"; //EV, 19/07/08
	  ...
	  if(combinatoria=="HIBRIDOHIBRIDO" || combinatoria=="POSPAGOHIBRIDO"){
	  ...
	  if(combinatoria=="POSPAGOHIBRIDO" ||combinatoria=="HIBRIDOPOSPAGO" ||combinatoria=="POSPAGOPOSPAGO"){
	  ...
	  if(combinatoria=="POSPAGOPREPAGO" || combinatoria=="HIBRIDOPREPAGO") {
		     ...
		     document.getElementById("causaBajaCB").disabled = false;
	  }

 
	   //activa ventana popup
	  if (indEqPrestado != null && indEqPrestado!="" ){                             
	  		if ( (combinatoria=="POSPAGOPREPAGO" && indEqPrestado=="1" ) ||
	        (combinatoria=="HIBRIDOPREPAGO" && indEqPrestado=="1" )
	     															){
	 ...
	 //(+) evera 19/07/08, se obtiene info desde lista de sesion, se pasa codigo a retornoClienteListaActivarCombosDependDelPlan
      var tipoPlan = 0;
	  if(comboAPrender=="combopre")			tipoPlan = 1;
	  else if(comboAPrender=="combopos")	tipoPlan = 2;
	  else if(comboAPrender=="combohib")  	tipoPlan = 3;
	  
  	  //atributosClienteDestino(codCliente);
	  //buscaClienteSeleccionado(codCliente,tipoPlan);
	  
	  //debe cargar lista, o habilitar buscador de clientes
	  var tipoFiltro = 2;
	  var maxClientesPermitidos = document.getElementById("numMaxClientesLista").value;
	  desactivarTodosControles(true);
	  JCondicionesOOSSReorden.buscarClientes(maxClientesPermitidos, tipoPlan, 0, tipoFiltro, retornoClienteListaActivarCombosDependDelPlan);
	  
	  /*
	  var  cliDestinoTipPlanTarif = "I";
	  var  cliDestinoCodPlanTarif = "";
	  var  cliDestinoCodEmpresa = 0;
		...
		if (cliDestinoCodCliente != null){
			for(var i=0; i<cliDestinoCodCliente.length; i++){
					...
					if (codCliente == cliDestinoCodClienteAux){
						...
						cliDestinoCodEmpresa   = listaCliDestinoCodEmpresa[i];					
						break;
					}
			}//fin for
	  ...
	  if (cliDestinoTipPlanTarif == 'E'){ //Cliente destino es empresa	  
		  ...
		  JCondicionesOOSSReorden.cargarPlanTarifarioEmpresa(cliDestinoCodPlanTarif, codCliente,retornoCargarPlanTarifarioEmpresa);
	      document.getElementById("codPlanTarifSelec").value = cliDestinoCodPlanTarif;
	      document.getElementById("codEmpresaDestino").value = cliDestinoCodEmpresa;
...
//(-)
	if (combinatoria !="POSPAGOPOSPAGO" &&   combinatoria!="PREPAGOPOSPAGO" ){
	  ...
	  //validar opcion pospagohibrido
	  /*if(combinatoria=="POSPAGOHIBRIDO" || combinatoria=="HIBRIDOPOSPAGO" ){
	  	validarOpcion();
	  }*/
} 
...
function retornoClienteListaActivarCombosDependDelPlan(dataCliente){
   ...
   // alert ("mtigua sal retornoClienteListaActivarCombosDependDelPlan");
		 ...
		 if(codMsgError =="0"){
				...
				if (largo > 0){
					...
					for(var i=0;i <largo;i++){
					    ...
					    var codEmpresa = clientesFiltrados[i].codEmpresa;  // EV 02/09/08
				        ...
				        if (codClienteSel!='' && codClienteSel == codigo) {
				            ...
				            codCliente1=codigo;
							codCiclo1=ciclo;
							codEmpresa1=codEmpresa; // EV 02/09/08
					 ...
					 document.getElementById("codEmpresaDestino").value = codEmpresa1;  //EV 02/09/08
					...
					if (consultaEvalRiesgo){
					    DWREngine.setAsync(false);
					    JCondicionesOOSSReorden.obtenerPlanesEvalSesion(retornoObtenerPlanesEvalSesion);
					    DWREngine.setAsync(true);
					}
		  			...
		  			if(comboTipoPlan=="combopos" && (tipPlanTarif1 == 'E')){//Cliente destino es empresa	  
						  //Obtener plan tarifario
						  document.getElementById("flgClienteDestinoEmpresa").value ="S";
						  JCondicionesOOSSReorden.cargarPlanTarifarioEmpresa(codPlanTarif1, codCliente1,retornoCargarPlanTarifarioEmpresa);
...
function retornoQuitarPlanTarifarioEmpresa(listaActualizada){
     ...
     /* 040608
     if(document.getElementById("combopos").disabled==false){
     	document.getElementById("combopos").disabled=true;  // despues de cargar el combo con el plan individual, debe quedar deshabilitado
     }*/
...
function activaOpcionCambioPlan(){
    generarCombinatoria();
	combinatoria=document.getElementById("combinatoria").value;
	if(combinatoria=="POSPAGOPREPAGO" || combinatoria=="HIBRIDOPREPAGO"){
...
function validaActivaNumFrecuentes(){
	...
	//(+) evera 19/07/08, se obtiene informacion desde la lista de sesion, se traspasa codigo a retornoClienteValidaActivaNumFrecuentes     	
	tipoPlan = 1; // es valido solo para pospagos
	var codigoClienteDest = document.getElementById("codClienteDestSelec").value;
	
   	JCondicionesOOSSReorden.buscaClienteSeleccionado (codigoClienteDest, tipoPlan, retornoClienteValidaActivaNumFrecuentes);
...
function validaComboClienteDestino(opcionClienteDestino)
  {
	...
	//(+) evera 16/07/08, se obtiene info de la lista de sesion, y se traspasa codigo a retornoClienteValidaComboClienteDestino
   	JCondicionesOOSSReorden.buscaClienteSeleccionado (opcionClienteDestino.value, tipoPlan, retornoClienteValidaComboClienteDestino);
	...
	if (cliDestinoTipPlanTarif == 'E'){ //Cliente destino es empresa	 
			document.getElementById("flgClienteDestinoEmpresa").value ="S";
			//obtener plan tarifario
			JCondicionesOOSSReorden.cargarPlanTarifarioEmpresa(cliDestinoCodPlanTarif, opcionClienteDestino.value, retornoCargarPlanTarifarioEmpresa);
		    document.getElementById("codPlanTarifSelec").value = cliDestinoCodPlanTarif;
   		    document.getElementById("codEmpresaDestino").value = cliDestinoCodEmpresa;
		    //carga numero de abonados del cliente
		    JCondicionesOOSSReorden.obtenerNumAbonadosCliente(opcionClienteDestino.value, 0, retornoObtenerNumAbonadosCliente);
		...
		if(document.getElementById("combinatoria").value == "POSPAGOPOSPAGO" || document.getElementById("combinatoria").value == "HIBRIDOHIBRIDO"){
			// 040608
			//if(codTipoPlanTarif=="I"){ // tipPlanTarif ORIGEN
				//mensajes.innerHTML="El Abonado Conservará su Plan Origen";
				
				//if(document.getElementById("combinatoria").value == "POSPAGOPOSPAGO"){
					//document.getElementById("combopos").disabled=true;
				//}
				//if(document.getElementById("combinatoria").value == "HIBRIDOHIBRIDO"){
					//document.getElementById("combohib").disabled=true;
				//}
							
				//document.getElementById("codPlanTarifSelec").value = codTipoPlanTarifOrigen; // Asignar el Plan Tarif. Origen en el valor que almacena el plan seleccionado
				//alert("codTipoPlanTarifOrigen : " + document.getElementById("codPlanTarifSelec").value);
		...
		if (flgClienteDestinoEmpresa == "S"){ //Como el anterior cliente seleccionado era empresa, y ahora se selecciona 
	        ...
	        document.getElementById("flgClienteDestinoEmpresa").value = "N";
   		    document.getElementById("codEmpresaDestino").value = 0;
			document.getElementById("combopos").disabled=false;
			JCondicionesOOSSReorden.quitarPlanTarifarioEmpresa(retornoQuitarPlanTarifarioEmpresa);
 ...
 function retornoClienteValidaComboClienteDestino(dataCliente){
	...
	if(dataCliente != null){
		 ...
		 //actualiza empresa
		 document.getElementById("codEmpresaDestino").value = dataCliente['codEmpresa'];  //EV 04/09/08
	...
	if (cliDestinoTipPlanTarif == 'E'){ //Cliente destino es empresa
		document.getElementById("flgClienteDestinoEmpresa").value ="S";
		//obtener plan tarifario
		JCondicionesOOSSReorden.cargarPlanTarifarioEmpresa(cliDestinoCodPlanTarif, codClienteDestino, retornoCargarPlanTarifarioEmpresa);
	    document.getElementById("codPlanTarifSelec").value = cliDestinoCodPlanTarif;
	    //carga numero de abonados del cliente
	    JCondicionesOOSSReorden.obtenerNumAbonadosCliente(codClienteDestino, 0, retornoObtenerNumAbonadosCliente);
...
// (+)evera 19/07/08, se comenta ya que las listas se manejaran en sesion
...
/*function llenaClienteDestino(){
    ...
    if (radio=="combopre"){
		        ...
		        listaCliDestinoCicloFacturacion[i] = "<bean:write name='listaClientesDestino' property='codCiclo'/>";
   		        ...
   		        listaCliDestinoCodEmpresa[i]  = "<bean:write name='listaClientesDestino' property='codEmpresa'/>";   		        
		        i++;
	        </logic:iterate>
    }
    else if(radio=="combopos"){
...
function atributosClienteDestino(codCliente){
    //(+) evera 19/07/08, se obtiene ciclo desde lista en sesion    
    /*
    var codCicloCliente=document.getElementById("codCicloCliente");
	...
	if (radio.value == "combopre")	tipoPlan = 1; 
   	...
   	JCondicionesOOSSReorden.buscaClienteSeleccionado (codCliente,tipoPlan, retornoAtributosClienteDestino);
   	//(-)
...
function evaluarCondicionesAjaxOOSS()
  {
  	  ...
  	  if (document.getElementById("causaExcepcionCB").style.display=="none"){
		document.getElementById("causaExcepcionCB").value = "-1";
      }
  	  parametrosCondicionesOS['codCausaExcepcion']=document.getElementById("causaExcepcionCB").value;
	...
	if (totalAboSel > 0){
 	  ...
 	  JCondicionesOOSSReorden.validaCondicionesInicio(parametrosCondicionesOS, arrAbonados, retornoConsideracionesAjaxReturn);//evera 27/08/07 
...
function evaluarCondicionesAjaxOOSScontinuar(codigoMsg, respuestaMsg)
  {
	  ...
	  JCondicionesOOSSReorden.validaCondicionesContinuar(parametrosCondicionesOS, codigoMsg, respuestaMsg, retornoConsideracionesAjaxReturn);
  }  
//(-) evera 28/08/07  
...
function retornoConsideracionesAjaxReturn(data)
	      ...
	      msg=valoresRetorno['mensaje'];
	  	  ...
	  	  if (msg == null){
			   ...
			   document.getElementById("codPlanServNuevo").value = valoresRetorno['codPlanServNuevo']; 
   			   ...
   			   document.getElementById("codActAboAux").value = valoresRetorno['codActAboAux'];  
		       ...
		       document.forms[0].submit();
				...
				if (codTipMensaje == CODIGOMSGCONFIRMAR){ //CODIGOMSGCONFIRMAR es 500
					...
					if (codMensaje == 'c.004'){ //Activacion de linea 
						//alert ('en tratar causa');
						//tratarCausaExcepcion(respuesta);
						if (respuesta == 'S' ) {
							alert ("Debe seleccionar Causa de excepcion");
						    document.getElementById("causaExcepcionLB").style["display"]="";
						    document.getElementById("punto").style["display"]="";	  
						    document.getElementById("causaExcepcionCB").style["display"]="";
						    activaBotonSiguiente();								
						}
					}
					else {
  ...
  function actualizaListaCliente(tipoCliente) {
     JCondicionesOOSSReorden.actualizaListaCliente(tipoCliente, retornoActualizarCliente);
  }*/
  
  function retornoActualizarCliente(){
  	validaComboClienteDestino(optClienteAux);
  } 
  

  /*function buscaClienteSeleccionado(codCliente,tipoPlan) {
  	 JCondicionesOOSSReorden.buscaClienteSeleccionado(codCliente,tipoPlan,actualizaDatosCliente);
  }*/
  ...
  function actualizaListaPlanTarifario(tipoPlan,codigoPlanTarifario,descripcionPlanTarifario,tipoPlanTarifario,codigoCargoBasico,descripcionCargoBasico,importeCargoBasico,importeLimite,importeFinal,codigoLimiteConsumo,descripcionLimiteConsumo,numDias) {
    JCondicionesOOSSReorden.actualizaListaPlanTarifario(tipoPlan,codigoPlanTarifario,descripcionPlanTarifario,tipoPlanTarifario,codigoCargoBasico,descripcionCargoBasico,importeCargoBasico,importeLimite,importeFinal,codigoLimiteConsumo,descripcionLimiteConsumo,numDias);
  }  
  
  function buscaPlanTarifSeleccionado (codPlanTarif,tipoPlan){
	JCondicionesOOSSReorden.buscaPlanTarifSeleccionado(codPlanTarif,tipoPlan,actualizaAtributosPlan);	
  }
  ...
  function actualizaAtributosPlan(planTarif) {
  	if (planTarif!=null){
		...
		var clienteDestino = document.getElementById("codClienteDestSelec").value;
     		JCondicionesOOSSReorden.obtenerLimitesConsumo(codPlanTarif, clienteDestino, fncResultadoObtenerLimitesConsumo);
	    	//(-) EV 07/12/09 carga combo de limites de consumo
...
function evaluarCondicionesCuentaPersonalAjaxOOSS()
  {
	  ...
	  //alert ("Codigo causa:"+document.getElementById("causaExcepcionCB").value);
	...
	if (totalAboSel > 0){
	   ...
	   var tipoComboPlan = $("radioTipoPlan").value;
	   JCondicionesOOSSReorden.ValidaCuentaPersonal(parametrosCondicionesOS, arrAbonados,tipoComboPlan ,retornoConsideracionesAjaxReturn2); 
 	}
...
function validarOpcionesCuentaPersonal(){
	...
	JCondicionesOOSSReorden.ValidaAltaCuentaPersonal(celPers.value , retornoValidaciones);
}
...
function f_buscarClientes(tipoFiltro){
    ...
    var tipoCliente = 0;
  	...
  	if (codClienteFiltrado > 0){
	    ...
	    JCondicionesOOSSReorden.buscarClientes(maxClientesPermitidos, tipoCliente, codClienteFiltrado, tipoFiltro, retornoBuscarClientes);
	}else{
	...
	if(data != null){         
		  ...
		  if(codMsgError =="0"){
			    ...
			    if (largo>0){
						...
						var codEmpresa = clientesFiltrados[0].codEmpresa;
				        ...
				        $(strComboCliente).add(optCliente);
						 ...
						 document.getElementById("codEmpresaDestino").value = codEmpresa;
						 ...
						 if (tipPlanTarif == "E"){//Cliente destino es empresa
							...
							document.getElementById("flgClienteDestinoEmpresa").value ="S";
							//obtener plan tarifario
							JCondicionesOOSSReorden.cargarPlanTarifarioEmpresa(codPlanTarif, codigo, retornoCargarPlanTarifarioEmpresa);
						    document.getElementById("codPlanTarifSelec").value = codPlanTarif;
						    //carga numero de abonados del cliente
						    JCondicionesOOSSReorden.obtenerNumAbonadosCliente(codigo, 0, retornoObtenerNumAbonadosCliente);
						 }
						 else{ 
							var flgClienteDestinoEmpresa = document.getElementById("flgClienteDestinoEmpresa").value;
							if (flgClienteDestinoEmpresa == "S"){ //Como el anterior cliente seleccionado era empresa, y ahora se selecciona 
