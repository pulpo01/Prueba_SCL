...
function enviarFormulario(){
	...
	document.forms[0].action="<%=request.getContextPath()%>/productoFrec/FlujoProdContratadosFrec.do";
    document.forms[0].submit();
}


function guardarParametros(idProd, codProd, nomProd){
	...
	document.forms[0].action="<%=request.getContextPath()%>/productoFrec/NumerosFrecuentesAction.do";
    document.forms[0].submit();
}
...
function sobreElBoton(id,imagen){
	try{
		...
		alert(e.name + " - "+e.message+" En sobreElBoton() id "+id+" imagen "+imagen);
	}
}
...
<html:form  method="POST" action="/ListaProdContratadosFrec.do">
