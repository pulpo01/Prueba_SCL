...
import com.tmmas.scl.operations.crm.fab.cim.manreq.web.form.VendedorForm;
...
import com.tmmas.scl.vendedor.dto.UsuarioSistemaDTO; // RRG
public class VendedorAction extends Action {
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form,
		...
		if (session.getAttribute("flagVendedor") == null)	session.setAttribute("flagVendedor", "NO");
		
		
		try {
			logger.debug("recuperarInformacionVendedor():antes");
			resultado = delegate.recuperarInformacionVendedor(dto);
			session.setAttribute("flagVendedor", "SI");
			logger.debug("recuperarInformacionVendedor():despues");			
		} catch (Exception e) {
			 session.setAttribute("flagVendedor", "NO");
		...
		UsuarioVendedorDTO usuariovendedor = new UsuarioVendedorDTO();
		//El usuario no se obtiene de la sesion, sino de la pantalla del vendedor comisionable
		usuariovendedor.setVendedorenSesion(false);
		usuariovendedor.setVendedor(resultado);


		session.setAttribute("UsuarioVendedorComisionable", usuariovendedor);

		//logger.debug("[VendedorAction]setVendedor:"); // RRG
