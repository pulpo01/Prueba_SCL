...
public class ProductoAction extends Action
{
	...
	HttpServletResponse response) throws Exception
  {
		...
		System.out.println("ProductoAction bitacorad "+bitacorad);
		System.out.println("ProductoAction bitacorac "+bitacorac);
		//bitacorad = bita;
		//bitacorac = bitac;
		String idControlCh = (String) request.getParameter("idControlCh");
		System.out.println("ProductoAction bitac "+bitac);
		System.out.println("ProductoAction bita "+bita);
		System.out.println("ProductoAction idControlCh id|esvisible|disabled|checkeado:"+idControlCh);//controlesId[0]+"|"+esvisible+"|"+disabled+"|"+checkeado;
		...
		if ((bitacorad!=null && bitacorad.trim().length()>0) || (bitacorac!=null && bitacorac.trim().length()>0)){
			...
			System.out.println("RECUPERANDO PRODUCTOS CONTRATADOS Y OFERTADOS SELECCIONADOS");
			...
			if(bitacorad!=null && bitacorad.trim().length()>0)
			{
				...
				if(cantidades == null)
				{
					...
					System.out.println("cantidadesP "+cantidadesP);
				}
	...
	private String[] obtieneCantidades(String cantidadesP) {
		...
		try
		{
			...
			System.out.println("cantidades = null "+e.getMessage());
		}
		return cantidades;
	}
