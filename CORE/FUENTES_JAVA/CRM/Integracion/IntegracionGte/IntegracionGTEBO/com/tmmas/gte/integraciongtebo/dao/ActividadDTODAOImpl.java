...
public class ActividadDTODAOImpl extends com.tmmas.cl.framework.base.ConnectionDAO implements ActividadDTODAO{
	private CompositeConfiguration config;
	private static Logger logger = Logger.getLogger(ActividadDTODAOImpl.class);
	private Global global = Global.getInstance();
	private ServiceLocator  serviceLocator = ServiceLocator.getInstance();

	public ActividadDTODAOImpl() {
	...
	/**
	* rescata la descripcion de la profesion.
	*/
	public com.tmmas.gte.integraciongtecommon.dto.ActividadResponseDTO consultarActividad(com.tmmas.gte.integraciongtecommon.dto.ActividadDTO inParam0)
		...
		logger.info("consultarActividad:start()");
		java.sql.Connection conn = null;
		java.sql.CallableStatement cstmt = null;
		logger.info("declara parametro de salida...");
		com.tmmas.gte.integraciongtecommon.dto.ActividadResponseDTO outParam0 = new com.tmmas.gte.integraciongtecommon.dto.ActividadResponseDTO();
		...
		try {
			String call = "{call GE_INTEGRACION_PG.GE_CONS_PROFESION_PR(?,?,?,?,?)}";
			logger.info("call[" + call + "]");

			cstmt = conn.prepareCall(call);

			logger.info("armar parametros de entrada...");
			cstmt.setLong(1,inParam0.getCodActividad());
			logger.debug("codActividad[" + inParam0.getCodActividad() + "]");
			...
			outParam0.setDesActividad(cstmt.getString(2));
			logger.debug("desActividad[" + outParam0.getDesActividad() + "]");
		...
		logger.info("consultarActividad:end()");
		return outParam0;
	}
	
	/**
	* rescata la descripcion de la ocupacion.
	*/
	public com.tmmas.gte.integraciongtecommon.dto.ActividadResponseDTO consultarOcupacion(com.tmmas.gte.integraciongtecommon.dto.ActividadDTO inParam0)
		...
		try {
			String call = "{call GE_INTEGRACION_PG.GE_CONS_OCUPACION_PR(?,?,?,?,?)}";
			logger.info("call[" + call + "]");

			cstmt = conn.prepareCall(call);

			logger.info("armar parametros de entrada...");
			cstmt.setString(1,inParam0.getCodOcupacion());
			logger.debug("codOcupacion[" + inParam0.getCodOcupacion() + "]");
