...
public class PagoDTODAOImpl extends com.tmmas.cl.framework.base.ConnectionDAO implements PagoDTODAO{
	private CompositeConfiguration config;
	private static Logger logger = Logger.getLogger(PagoDTODAOImpl.class);
	private Global global = Global.getInstance();
	private ServiceLocator  serviceLocator = ServiceLocator.getInstance();

	public PagoDTODAOImpl() {
	...
	*/
	public com.tmmas.gte.integraciongtecommon.dto.PagoResponseDTO consultaPagosRealizados(com.tmmas.gte.integraciongtecommon.dto.PagoInDTO inParam0)
		...
		try {
			String call = "{call GE_INT_JUAN_PG.GE_CONS_PAGOS_REALIZADO_PR(?,?,?,?,?)}";
			...
			while (rs.next()) {
				...
				dto2.setImpPago(rs.getLong("imp_pago"));
				...
				dto2.setFecEfectividad(new java.util.Date(rs.getDate("fec_efectividad").getTime()));
				logger.debug("fecEfectividad[" + dto2.getFecEfectividad() + "]");

				dto2.setImporteTotDoc(rs.getLong("importe_tot_doc"));
				...
				dto2.setTotFactura(rs.getLong("tot_factura"));
				logger.debug("totFactura[" + dto2.getTotFactura() + "]");

				dto2.setTotPagar(rs.getLong("tot_pagar"));
				logger.debug("totPagar[" + dto2.getTotPagar() + "]");

				dto2.setFechaHora(rs.getString("fecha_hora"));
	...
	*/
	public com.tmmas.gte.integraciongtecommon.dto.PagoRecaudadoraOutDTO consultarRecaudadora(com.tmmas.gte.integraciongtecommon.dto.PagoDTO inParam0)
		...
		try {
			String call = "{call GE_INT_JUAN_PG.GE_CONS_RECAUDADORA_PAGO_PR(?,?,?,?,?,?,?,?)}";
			logger.info("call[" + call + "]");
