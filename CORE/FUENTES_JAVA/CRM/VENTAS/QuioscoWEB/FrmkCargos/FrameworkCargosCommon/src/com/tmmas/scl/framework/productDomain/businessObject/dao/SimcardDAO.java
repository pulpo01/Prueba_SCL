...
public class SimcardDAO extends ConnectionDAO implements SimcardDAOIT{
	private final Logger logger = Logger.getLogger(SimcardDAO.class);
		...
		throws GeneralException{
			...
			try {
				//CSR_11002
//				String call = getSQLDatosSimcard("VE_servicios_venta_PG","VE_PrecCargoSimcard_PreLis_PR",10);
		...
		throws ProductException{
			...
			try {
				conn = getConnectionFromWLSInitialContext(global.getCPUJndiForDataSource());
				//CSR-11002
//				String call = getSQLDatosSimcard("VE_servicios_venta_PG","VE_PreCarSimcard_NoPreLis_PR",15);
		...
		public DescuentoDTO[] getDescuentoCargoArticulo(ParametrosDescuentoDTO entrada) throws ProductException{
			...
			try {
				...
				logger.debug("[getDescuentoCargoBasicoArticulo] Nombre usuario: " + entrada.getNombreUsuario());
	    			...
	    			//CSR-11002		
//					call = getSQLDatosSimcard("VE_servicios_venta_PG","VE_obtiene_descuento_art_PR",14);
				    call = getSQLDatosSimcard("VE_servicios_venta_Quiosco_PG","VE_obtiene_descuento_art_PR",14);
					logger.debug("sql[" + call + "]");
					conn = getConnectionFromWLSInitialContext(global.getCPUJndiForDataSource());
					cstmt = conn.prepareCall(call);
...
//						    "VE_servicios_venta_PG.VE_OBTIENE_DESCUENTO_CON_PR ( ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ? ); "+
					"VE_servicios_venta_Quiosco_PG.VE_OBTIENE_DESCUENTO_CON_PR ( ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ? ); "+
						" END;");
		...
		public DescuentoDTO[] getDescuentoCargoConceptoCol(ParametrosDescuentoDTO entrada) throws ProductException{
			...
			try {
				...
				if (entrada.getTipoPlanTarifario()!=null){
					call =  getSQLDatosSimcard("PV_CAMBIO_SIMCARD_SB_PG", "PV_obtiene_descuento_art_PR", 14);
					logger.debug("sql[" + call + "]");
					conn = getConnectionFromWLSInitialContext(global.getCPUJndiForDataSource());	
					cstmt = conn.prepareCall(call);
					cstmt.setString(1,entrada.getCodigoOperacion());					
					...
					call = getSQLDatosSimcard("VE_servicios_venta_PG","VE_obtiene_descuento_con_PR",16);
					logger.debug("sql[" + call + "]");
					conn = getConnectionFromWLSInitialContext(global.getCPUJndiForDataSource());	
					cstmt = conn.prepareCall(call);
