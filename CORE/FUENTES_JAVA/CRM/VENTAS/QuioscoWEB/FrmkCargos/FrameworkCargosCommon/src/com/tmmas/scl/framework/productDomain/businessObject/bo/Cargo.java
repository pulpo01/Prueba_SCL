...
public class Cargo implements CargoIT  {
	...
	public RetornoDTO eliminarFactura(RegistroFacturacionDTO registro) throws ProductOfferingPriceException{
		RetornoDTO retorno = null;
		try {
			logger.debug("eliminarFactura():start");
			retorno = cargoDAO.eliminarFactura(registro);
			logger.debug("eliminarFactura():end");
		} catch (ProductOfferingPriceException e) {
			logger.debug("ProductOfferingPriceException[", e);
			throw e;
		}
