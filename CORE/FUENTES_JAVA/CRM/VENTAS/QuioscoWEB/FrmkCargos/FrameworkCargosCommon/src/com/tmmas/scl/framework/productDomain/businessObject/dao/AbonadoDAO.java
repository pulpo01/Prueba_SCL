...
public class AbonadoDAO extends ConnectionDAO implements AbonadoDAOIT {
	...
	private String getSQLUpdCodSituaAbonado(){
		...
		call.append("	DECLARE	"); 
		call.append("	 BEGIN	");
		call.append("	   VE_SERVICIOS_VENTA_PG.VE_UPDABOCELCODSITUAC_PR ( ?,?,?,?,? );");
		call.append("	 END; 	");
	...
	private String getSQLgetListaAbonadosVenta(){
		StringBuffer call = new StringBuffer();
		call.append(" BEGIN "+
				  		"VE_SERVICIOS_VENTA_PG.VE_OBTIENE_ABONADOS_VENTA_PR ( ?, ?, ?, ?, ? );"+
				  	"  END;");
		return call.toString();
	}
	
	private String getSQLgetListaAbonadosVentaNoKit(){
		StringBuffer call = new StringBuffer();
		call.append(" BEGIN "+
				  		"ve_cargos_pg.ve_obtiene_abo_ven_nokit_pr ( ?, ?, ?, ?, ? );"+
				  	"  END;");
		return call.toString();
	}
	
	private String getSQLgetListaAbonadosVentaAmistar(){
		StringBuffer call = new StringBuffer();
		call.append(" BEGIN "+
				  		"VE_SERVICIOS_VENTA_PG.VE_OBTIENE_ABONADOS_VENTAAM_PR ( ?, ?, ?, ?, ? );"+
				  	"  END;");
		return call.toString();
	}
	...
	public AbonadoDTOInt[] getListaAbonadosVenta(RegistroVentaDTO registroventa) throws ProductException{
		...
		String msgError = null;
		int numEvento = 0;
		AbonadoDTOInt[] resultado = null;
		Connection conn = null;
		logger.debug("Coneccion obtenida OK");
		CallableStatement cstmt = null;
		try {
			conn = getConnectionFromWLSInitialContext(global.getCPUJndiForDataSource());
			String call = getSQLgetListaAbonadosVenta();
			logger.debug("sql[" + call + "]");
			cstmt = conn.prepareCall(call);
			...
			if (codError != 0) {
				...
				ResultSet rs = (ResultSet) cstmt.getObject(2);
				while (rs.next()) {
					AbonadoDTOInt abonadoDTO = new AbonadoDTOInt();
				...
				resultado =(AbonadoDTOInt[]) ArrayUtl.copiaArregloTipoEspecifico(
					lista.toArray(), AbonadoDTOInt.class);
				rs.close();
	...
	public AbonadoDTOInt[] getListaAbonadosVentaNoKit(RegistroVentaDTO registroventa) throws ProductException{
		...
		try {
			conn = getConnectionFromWLSInitialContext(global.getCPUJndiForDataSource());
			String call = getSQLgetListaAbonadosVentaNoKit();
			logger.debug("sql[" + call + "]");
			cstmt = conn.prepareCall(call);
			cstmt.setLong(1,registroventa.getNumeroVenta());
			cstmt.registerOutParameter(2,OracleTypes.CURSOR);
	...
	private String getSQLgetListaAbonadosVentaKit(){
		StringBuffer call = new StringBuffer();
		call.append(" BEGIN "+
				  		"VE_CARGOS_PG.VE_OBTIENE_VENTA_KIT_PR( ?, ?, ?, ?, ? );"+
				  	"  END;");
		return call.toString();
	}
	...
	public AbonadoDTOInt[] getListaAbonadosVentaKit(RegistroVentaDTO registroventa) throws ProductException{
		logger.debug("getListaAbonadosVentaKit:start");
		int codError = 0;
		String msgError = null;
		int numEvento = 0;
		AbonadoDTOInt[] resultado = null;
		...
		try {
			conn = getConnectionFromWLSInitialContext(global.getCPUJndiForDataSource());
			String call = getSQLgetListaAbonadosVentaKit();
			logger.debug("sql[" + call + "]");
			cstmt = conn.prepareCall(call);
			...
			cstmt.registerOutParameter(4, java.sql.Types.VARCHAR);
			cstmt.registerOutParameter(5, java.sql.Types.NUMERIC);
			logger.debug("Inicio:getListaAbonadosVentaKit:Execute");
			cstmt.execute();
			logger.debug("Fin:getListaAbonadosVentaKit:Execute");
			codError = cstmt.getInt(3);
			msgError = cstmt.getString(4);
			numEvento = cstmt.getInt(5);
			...
			if (codError != 0) {
				...
				while (rs.next()) {
					...
					AbonadoDTOInt abonadoDTO = new AbonadoDTOInt();
					abonadoDTO.setCodigoArticulo(rs.getString(1));
					logger.debug("abonadoDTO.setCodigoArticulo() [" + abonadoDTO.getCodigoArticulo() + "]");
					abonadoDTO.setTipoStock(rs.getLong(2));
					logger.debug("abonadoDTO.setTipoStock() [" + abonadoDTO.getTipoStock() + "]");
					abonadoDTO.setCodPlanTarif(rs.getString(3));
					logger.debug("abonadoDTO.setCodPlanTarif() [" + abonadoDTO.getCodPlanTarif() + "]");
					abonadoDTO.setCodCargoBasico(rs.getString(4));
					logger.debug("abonadoDTO.setCodCargoBasico() [" + abonadoDTO.getCodCargoBasico() + "]");
					abonadoDTO.setCodPlanServ(rs.getString(5));
					logger.debug("abonadoDTO.setCodPlanServ() [" + abonadoDTO.getCodPlanServ() + "]");
					abonadoDTO.setCodCliente(rs.getLong(6));					
					logger.debug("abonadoDTO.setCodCliente() [" + abonadoDTO.getCodCliente() + "]");
					abonadoDTO.setCodVendedor(rs.getLong(7));
					logger.debug("abonadoDTO.setCodVendedor() [" + abonadoDTO.getCodVendedor() + "]");
					abonadoDTO.setCodTipContrato(rs.getString(8));
					logger.debug("abonadoDTO.setCodTipContrato() [" + abonadoDTO.getCodTipContrato() + "]");
					abonadoDTO.setCodCausaVenta(rs.getString(9));
					logger.debug("abonadoDTO.setCodCausaVenta() [" + abonadoDTO.getCodCausaVenta() + "]");
					abonadoDTO.setCodModVenta(rs.getLong(10));
					logger.debug("abonadoDTO.setCodModVenta() [" + abonadoDTO.getCodModVenta() + "]");
					abonadoDTO.setTipPlantarif(rs.getString(11));
					logger.debug("abonadoDTO.setTipPlantarif() [" + abonadoDTO.getTipPlantarif() + "]");
					abonadoDTO.setNumContrato(rs.getString(12));
					logger.debug("abonadoDTO.setNumContrato() [" + abonadoDTO.getNumContrato() + "]");
					abonadoDTO.setCodUso(rs.getString(13));
					logger.debug("abonadoDTO.setCodUso() [" + abonadoDTO.getCodUso() + "]");
					abonadoDTO.setNumMin(rs.getString(14));
					logger.debug("abonadoDTO.setNumMin() [" + abonadoDTO.getNumMin() + "]");
					abonadoDTO.setCodCentral(rs.getInt(15));
					logger.debug("abonadoDTO.setCodCentral() [" + abonadoDTO.getCodCentral() + "]");
					abonadoDTO.setCodCuenta(rs.getLong(16));
					logger.debug("abonadoDTO.setCodCuenta() [" + abonadoDTO.getCodCuenta() + "]");
					abonadoDTO.setCodCiclo(rs.getInt(17));
					logger.debug("abonadoDTO.setCodCiclo() [" + abonadoDTO.getCodCiclo() + "]");
					abonadoDTO.setCodVendealer(rs.getInt(18));
					logger.debug("abonadoDTO.setCodVendealer() [" + abonadoDTO.getCodVendealer() + "]");
					abonadoDTO.setCodTecnologia(rs.getString(19));
					logger.debug("abonadoDTO.setCodTecnologia() [" + abonadoDTO.getCodTecnologia() + "]");
					abonadoDTO.setCodCelda(rs.getString(20));
					logger.debug("abonadoDTO.setCodCelda() [" + abonadoDTO.getCodCelda() + "]");
					abonadoDTO.setNumeroSerie(rs.getString(21));
					logger.debug("abonadoDTO.setNumeroSerie() [" + abonadoDTO.getNumeroSerie() + "]");
					abonadoDTO.setCodTipContrato(rs.getString(22));
					logger.debug("abonadoDTO.setCodTipContrato() [" + abonadoDTO.getCodTipContrato() + "]");
		...
		logger.debug("Fin:getListaAbonadosVentaKit()");
