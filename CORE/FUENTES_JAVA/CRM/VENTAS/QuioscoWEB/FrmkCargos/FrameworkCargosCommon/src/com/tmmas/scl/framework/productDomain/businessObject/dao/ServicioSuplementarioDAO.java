...
public class ServicioSuplementarioDAO extends ConnectionDAO implements ServicioSuplementarioDAOIT{
	...
	throws ProductException{
		...
		try {
			...
			logger.debug("entrada.getCodigoProducto() ["+entrada.getCodigoProducto()+"]");
			cstmt.setString(2,entrada.getCodigoServicio());
			logger.debug("entrada.getCodigoServicio() ["+entrada.getCodigoServicio()+"]");
			cstmt.setString(3,entrada.getCodigoPlanServicio());
			logger.debug("entrada.getCodigoPlanServicio() ["+entrada.getCodigoPlanServicio()+"]");
			cstmt.setString(4,entrada.getCodigoActuacion());
			logger.debug("entrada.getCodigoActuacion() ["+entrada.getCodigoActuacion()+"]");
			...
			if (codError != 0&&codError!=99) {
				...
				while (rs!=null&&rs.next()) {
					...
					logger.debug("precioDTO.setMonto ["+rs.getFloat(3)+"]");
					precioDTO.setCodigoMoneda(rs.getString(4));
					logger.debug("precioDTO.setCodigoMoneda ["+rs.getString(4)+"]");
					precioDTO.setDescripcionMoneda(rs.getString(5));
					logger.debug("precioDTO.setDescripcionMoneda ["+rs.getString(5)+"]");
					precioDTO.setIndicadorAutMan(rs.getString(6));
					logger.debug("precioDTO.setIndicadorAutMan ["+rs.getString(6)+"]");
					precioDTO.setNumeroUnidades(rs.getString(7));
					logger.debug("precioDTO.setNumeroUnidades ["+rs.getString(7)+"]");
					precioDTO.setIndicadorEquipo(rs.getString(8));
					logger.debug("precioDTO.setIndicadorEquipo ["+rs.getString(8)+"]");
					precioDTO.setIndicadorPaquete(rs.getString(9));
					logger.debug("precioDTO.setIndicadorPaquete ["+rs.getString(9)+"]");
					precioDTO.setMesGarantia(rs.getString(10));
					logger.debug("precioDTO.setMesGarantia ["+rs.getString(10)+"]");
					precioDTO.setIndicadorAccesorio(rs.getString(11));
					logger.debug("precioDTO.setIndicadorAccesorio ["+rs.getString(11)+"]");
					precioDTO.setCodigoArticulo(rs.getString(12));
					logger.debug("precioDTO.setCodigoArticulo ["+rs.getString(12)+"]");
					precioDTO.setCodigoStock(rs.getString(13));
					logger.debug("precioDTO.setCodigoStock ["+rs.getString(13)+"]");
					precioDTO.setCodigoEstado(rs.getString(14));
					logger.debug("precioDTO.setCodigoEstado ["+rs.getString(14)+"]");
	...
	public DescuentoDTO[] getDescuentoCargoConcepto(ParametrosDescuentoDTO entrada) throws ProductException{
		...
		conn = getConection();
		try {
			String call = getSQLDatosServSupl("VE_servicios_venta_PG","VE_obtiene_descuento_con_PR",16);
			logger.debug("sql[" + call + "]");
	...
	public ServicioSuplementarioDTO[] getSSAbonadoNoCentrales(ServicioSuplementarioDTO servicioSuplementario) throws ProductException{
		logger.debug("Inicio:getSSAbonadoNoCentrales()");
		int codError = 0;
		String msgError = null;
		int numEvento = 0;
		...
		try {
			...
			logger.debug("NumeroAbonado   " + servicioSuplementario.getNumeroAbonado());
			logger.debug("IndicadorEstado " + servicioSuplementario.getIndicadorEstado());
			logger.debug("CodigoProducto  " + servicioSuplementario.getCodigoProducto());
			logger.debug("CodigoModulo    " + servicioSuplementario.getCodigoModulo());
			logger.debug("CodigoSistema   " + servicioSuplementario.getCodigoSistema());
			logger.debug("CodigoActuacion " + servicioSuplementario.getCodigoActuacion());
			...
			logger.debug("Inicio:getSSAbonadoNoCentrales:Execute");
			cstmt.execute();
			logger.debug("Fin:getSSAbonadoNoCentrales:Execute");
			...
			if (codError != 0) {
				logger.error("Ocurrió un error al recuperar SS del abonado");
				throw new ProductException(
						"Ocurrió un error al recuperar SS del abonado", String
								.valueOf(codError), numEvento, msgError);
				
			}else{
				logger.debug("Recuperando SS del abonado");
				ArrayList lista = new ArrayList();
				...
				while (rs.next()) {
					...
					logger.debug("setCadSSNivel   " + servicioSuplementarioDTO.getCodigoConcepto());
				...
				logger.debug("Fin recuperacion de SS del abonado");
			}
			if (logger.isDebugEnabled()) {
				logger.debug(" Finalizo ejecución");
				logger.debug(" Recuperando salidas");
			...
			logger.error("Ocurrió un error al recuperar SS del abonado",e);
			if (logger.isDebugEnabled()) {
				logger.debug("Codigo de Error[" + codError + "]");
				logger.debug("Mensaje de Error[" + msgError + "]");
				logger.debug("Numero de Evento[" + numEvento + "]");
		...
		logger.debug("Fin:getSSAbonadoNoCentrales()");
	...
	throws ProductException{
		logger.debug("Inicio:getListaSSAbonado()");
		int codError = 0;
		String msgError = null;
		int numEvento = 0;
		ServicioSuplementarioDTO[] resultado = null;
		...
		try {
			...
			logger.debug("Inicio:getListaSSAbonado:Execute");
			cstmt.execute();
			logger.debug("Fin:getListaSSAbonado:Execute");
		...
		logger.debug("Fin:getListaSSAbonado()");
