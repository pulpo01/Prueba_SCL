...
public class EstadoDAO extends ConnectionDAO{
	private final Logger logger = Logger.getLogger(EstadoDAO.class);
	...
	public EstadoDireccionDTO getListadoEstados() throws GeneralException{
		...
		int numEvento = 0;
		EstadoDireccionDTO resultado = null;
		Connection conn = null;
		CallableStatement cstmt = null;
		conn = getConection();
		logger.debug("Coneccion obtenida OK");
		try {
			//INI-01 (AL) String call = getSQLPackage("VE_servicios_direcciones_PG","VE_getListEstados_PR",5);
			String call = getSQLPackage("VE_servicios_direc_Quiosco_PG","VE_getListEstados_PR",5);
			...
			if (codError != 0) {
				logger.error("Ocurrió un error al consultar las Estados");
				throw new GeneralException(
						"Ocurrió un error al consultar las Estados", String
								.valueOf(codError), numEvento, msgError);
				
			}else{
				logger.debug("Llenado Estados");
				...
				logger.debug("Fin Llenado Estados");
			}
			...
			logger.error("Ocurrió un error al consultar los Estados",e);
			if (logger.isDebugEnabled()) {
			...
			throw new GeneralException(
					"Ocurrió un error al consultar los Estados",e);
		} finally {
		 	if (logger.isDebugEnabled()) 
