	...
	try{
		...
		if(cantidadElementos < 2) 
		{
			...
			if(cantidadElementos == 1){
				...
				DistribucionBolsaForm.btnDistEqui.disabled = true;
			}
			muestraError(msgerr);
			bloquearControles();
			//return;
...
function verificarDistCompleta()
{
	try{
		if(distribucionCompleta == 'true'){
			DistribucionBolsaForm.btnDistCompleta.disabled = false;
		}
		else{
			DistribucionBolsaForm.btnDistCompleta.disabled = true;
		}
	}catch(e){
...
function recargarCantidades()
{
	try{
		...
		{
			DistribucionBolsaForm.minutesToAssign.value = freeUnits;
		...
		else
		{
			...
			{
				DistribucionBolsaForm.minutesToAssign[i].value = minutesToAssignArr[i];
...
function bloquearControles(){
	...
	try{
		...
		//DistribucionBolsaForm.minutesToAssign.disabled = true;//si el largo >1 se debe usar como unidadesA
		//DistribucionBolsaForm.unidadesA.disabled = true;
		...
		//DistribucionBolsaForm.disabled="disabled"
