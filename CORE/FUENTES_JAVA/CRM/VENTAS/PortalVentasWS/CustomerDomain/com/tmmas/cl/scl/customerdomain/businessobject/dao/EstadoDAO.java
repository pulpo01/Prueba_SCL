...
public class EstadoDAO extends ConnectionDAO{
	private static Category cat = Category.getInstance(EstadoDAO.class);
	...
	public EstadoDireccionDTO getListadoEstados() 
		throws CustomerDomainException
	{
		...
		try {
			String call = getSQLPackage("VE_servicios_direcciones_PG","VE_getListEstados_PR",5);
			cat.debug("sql[" + call + "]");
			...
			if (codError != 0) {
				cat.error("Ocurrió un error al consultar las Estados");
				throw new CustomerDomainException(
						"Ocurrió un error al consultar las Estados", String
								.valueOf(codError), numEvento, msgError);
				
			}else{
				cat.debug("Llenado Estados");
			...
			cat.error("Ocurrió un error al consultar los Estados",e);
			if (cat.isDebugEnabled()) {
