...
public class RegistroDetPreliquidacionDAO extends ConnectionDAO {
	...
	public void registraDetallePreliquidacion(RegistroDetPreliquidacionDTO detpreliquidacion,RegistroPreliquidacionDTO cabpreliquidacion)
	...
	{		
		...
		try {
			...
			String call = getSQLPackage("VE_servicios_venta_PG","VE_in_ga_det_preliq_PR",13);
			cat.debug("sql[" + call + "]");
	...
	public ArrayList getMercaderiaEnConsignacion(RegistroPreliquidacionDTO preliquidacion)
		throws CustomerDomainException
	{
		...
		try {
			String call  = getSQLPackage("VE_servicios_venta_PG","VE_obtiene_articulos_consig_PR",5);
			cat.debug("sql[" + call + "]");
			cstmt = conn.prepareCall(call);
