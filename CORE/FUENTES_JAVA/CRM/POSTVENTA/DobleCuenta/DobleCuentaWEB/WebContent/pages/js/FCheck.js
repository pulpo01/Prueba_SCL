...
// s tiene solo letras y punto.
function isAlphabetic2 (s)
{   var i;

    if (isEmpty(s)) 
       if (isAlphabetic2.arguments.length == 1) return defaultEmptyOK;
       else return (isAlphabetic2.arguments[1] == true);
    for (i = 0; i < s.length; i++)
    {   
        // Check that current character is letter.
        var c = s.charAt(i);

        if (!isLetter(c)|| isSimbol(c))
        return false;
    }
    return true;
}
