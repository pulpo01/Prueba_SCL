				...
				<table border="0" cellspacing="1" cellpadding="2" width="100%">
					<tr>                  
					    <td width="80%" colspan="8" align="left" class="amarillo" >
					      <img src="img/green_arrow.gif" width="15" height="15" hspace="2" align="left" /><strong>Modificación de Atributos de Servicios Suplementarios a Nivel de Cliente</strong>
					    </td>					
		...
		<tr>
			<td>
				<table border="0" cellspacing="1" cellpadding="2" width="100%">
					...
					<logic:iterate indexId="count" id="fila" name="UpdateSSClienteForm" property="listaProductos" type="com.tmmas.cl.scl.pv.customerorder.commonapp.dto.ProductDTO">
						<logic:equal name="UpdateSSClienteForm" property="editar" value="<%=String.valueOf(count.intValue())%>">
							<%String colorFila = null;
							...
							<tr class="<%=colorFila%>">
								<td>
									<html:multibox value="<%=String.valueOf(count.intValue())%>" property="numProduct" />
								</td>
								<td>
								...
								<td>
									<div align="center"><html:text property="priority" styleClass="<%=colorFila%>" tabindex="1" /></div>
								</td>
								<td>
									<div align="center"><html:select styleClass="normal" property="exceedId">
										<logic:iterate indexId="i" id="desborde" name="listaDesbordes" scope="session" type="com.tmmas.cl.scl.pv.customerorder.commonapp.dto.DesbordeDTO">
											<html:option value="<%=String.valueOf(desborde.getCodigoDesborde())%>"> <%=String.valueOf(desborde.getDescripcionDesborde())%> </html:option>
										</logic:iterate>
										</html:select>
									</div>
								</td>
								<td>
									<div align="center">
											<html:select styleClass="normal" property="profileId">
												<logic:iterate indexId="j" id="limiteConsumo" name="listaLimiteConsumo" scope="request" type="com.tmmas.cl.scl.pv.customerorder.commonapp.dto.LimiteConsumoDTO">
													<html:option value="<%=String.valueOf(limiteConsumo.getProfileId())%>"> <%=String.valueOf(limiteConsumo.getDescProfileId())%> </html:option>
												</logic:iterate>
											</html:select>
									</div>
								</td>
								<td>
									<div align="center"><input type="button" value="Actualizar" class="fondoFilaPlomo" onclick="return Actualizar('<%=String.valueOf(count.intValue())%>')" /></div>
								</td>
							</tr>
						</logic:equal>
						<logic:notEqual name="UpdateSSClienteForm" property="editar" value="<%=String.valueOf(count.intValue())%>">
							<%colorFila = null;
							if ((count.intValue() % 2) == 0) {
								colorFila = "fondoFilaBlanco";
							} else {
							...
							<tr class="<%=colorFila%>">
								...
								<td>
									<div align="center"><bean:write name="fila" property="descExceedId" /></div>
								</td>
								<td>
								...
								<td>
									<div align="center">
										<input type="button" value="Editar" class="fondoFilaPlomo" onclick="accion.value='Editar';editar.value='<%=String.valueOf(count.intValue())%>';submit();" />
									</div>
								</td>
							</tr>
						</logic:notEqual>
