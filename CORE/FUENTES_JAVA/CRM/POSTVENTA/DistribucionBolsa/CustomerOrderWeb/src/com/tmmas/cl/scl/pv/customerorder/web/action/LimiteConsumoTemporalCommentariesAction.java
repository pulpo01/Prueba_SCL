...
public class LimiteConsumoTemporalCommentariesAction extends Action{
		...
		throws Exception 
	{
		...
		LimiteClienteDTO limiteAbonado;
		limiteAbonado = (LimiteClienteDTO)request.getSession().getAttribute("limiteAbonado");		
		limiteAbonado.getOoss().setComentario(request.getParameter("comentarios").toString()); 
		
		
		logger.debug("Antes de Delegate");
		delegate.setServiceLimitTemporally(limiteAbonado);
