...
function callbackModalidadPago(data)	{
	...
	if (data != null)	{
		...
		if (!flag) borrarError();
			...
			/**
		@author : rlozano
		@description : validacion para prepago;
		*/
		filtraModalidadPagoPrepago();
...
function bloquearSerie(accion)	{
	...
	if ((accion == "bloquear") && (flagPagRecargada==true) && (document.getElementById("flagBloqueo").value==1)) 	{
		alert("La serie ya se encuentra reservada.");
		return;
	}
	...
	if (flagValidacion == false)	{
		...
		if (flagPagRecargada==false)
			if (document.getElementById("flagBloqueo").value != 1 && accion == "desbloquear")	{
				alert("Antes, debe reservar un número de serie.");
				return;
			}

			if (document.getElementById("flagBloqueo").value == 1 && accion == "bloquear")	{
				alert("La serie ya se encuentra reservada. Sólo puede cancelar su reserva.");
				return;
			}
...
// ------------------------------------------------------------------------------------  	
	...
	function filtraModalidadPagoPrepago(){
		var contrato=document.getElementById("des_tipcontrato").value;
		var isPrepago=contrato=="PREPAGO"?true:false;
		var modalidadPago =document.getElementById("modalidadPago");
		if (isPrepago){
			for (i=0;i<modalidadPago.length;i++)
			{
				//es contado
				var modPago=modalidadPago[i].value
				if ("1"==modPago)
				{
					modalidadPago.selectedIndex=i;
					modalidadPago.disabled=true;
					return;
				}
			}
		}
	...
	function desReservaSerieP(nroSerieSim){
	   try{
			CusIntManDWRController.desReservaSerieP(nroSerieSim,callbackDesReservaSerie);
		}
		catch(e){
			alert(""+e.message);
		}	
