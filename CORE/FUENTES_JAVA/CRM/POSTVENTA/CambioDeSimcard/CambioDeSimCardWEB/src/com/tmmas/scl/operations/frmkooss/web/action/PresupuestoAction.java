		...
		try{
			...
			if(formPresupuesto.getAccion()!=null&&!((String)session.getAttribute("accionAutDesc")).equalsIgnoreCase("")){
				...
				if(formPresupuesto.getAccion().equalsIgnoreCase("Siguiente")){
					...
					String numSecuencia=(String)session.getAttribute("numSecuencia");
					cierreVenta(presup,sessionData.getCliente().getCodCliente(),Float.parseFloat(impTotal), formPresupuesto.getMontoAbono(),numSecuencia);
				...
				CambioDeSimCardForm cambioSimcardForm = (CambioDeSimCardForm)session.getAttribute("CambioSimcardForm");
				formPresupuesto.setRbCiclo(cambioSimcardForm.getAbonoDiferido());
