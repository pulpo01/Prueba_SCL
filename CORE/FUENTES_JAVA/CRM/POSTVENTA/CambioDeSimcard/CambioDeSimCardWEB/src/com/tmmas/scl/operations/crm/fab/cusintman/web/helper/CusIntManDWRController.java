...
//	 ------------------------------------------------------------------------------------
	...
	public MensajeRetornoDTO bloquearSerie ( long num_abonado, 
	    ...
	    try		{
	    	infoserie = delegate.reservaSimcard(accionProceso, serie, usuarioSistema, usuarioAbonado, sessionData, causa, terminal, modalidadPago );
    		...
    		if (accionProceso.equals("bloquear"))
    			mensaje.setMensaje("La serie solicitada ha sido reservada.");
    		else
    			mensaje.setMensaje("La reserva de la serie solicitada ha sido cancelada.");
	    }
	    	...
	    	mensaje.setNumMovimientoBloqDesb(0);
	    } // catch
...
// INICIO RRG 71913 16-02-2009 COL
	public RetornoDTO desReservaSerie()throws CusIntManException{
		...
		try{
			...
			SerieDTO serie = new SerieDTO();
			CambioDeSimCardForm cambioDeSerieForm=(CambioDeSimCardForm)session.getAttribute("cambioDeSimCardForm");
			String serieEquipo=cambioDeSerieForm==null?"":cambioDeSerieForm.getNroSerie();
			...
			if (!"".equals(serieEquipo)){
			 ...
			 String serieSimcard=cambioDeSerieForm==null?"":cambioDeSerieForm.getNroSerie();
	...
	public RetornoDTO desReservaSerieP(String nroSerieSim)throws CusIntManException{
			RetornoDTO retValue=null;
			try{
