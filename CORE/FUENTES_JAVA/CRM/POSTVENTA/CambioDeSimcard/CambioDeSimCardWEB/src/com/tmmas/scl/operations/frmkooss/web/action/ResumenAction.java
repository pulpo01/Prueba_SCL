...
public class ResumenAction extends Action {
	...
	private static final String COD_OS_CAMBIO_SIMCARD = "10271";
...
//		Se procede a realizar el cierre de la venta
		...
		carta.setCodOOSS(COD_OS_CAMBIO_SIMCARD);
		...
		final String numSerie = cambioDeSimCardForm.getNroSerie();
	    ...
	    /* }*/
		logger.debug("textoCarta ini");
		logger.debug(textoCarta);
		logger.debug("textoCarta fin");
		...
		if(botonSeleccionado!=null&&botonSeleccionado.equals("visualizar")){
			...
			//parametros.put("Codigo",""+sessionData.getCliente().getCodCliente());
			parametros.put("Codigo",""+sessionData.getNumAbonado());
			//parametros.put("Referencia","???");
			parametros.put("Referencia","CAMBIO DE SIMCARD");
			parametros.put("NomCliente",nombreCliente);
			//parametros.put("NomVendedor", session.getAttribute("vendedor").toString());
			parametros.put("NomVendedor", sessionData.getUsuario());
			//parametros.put("TextCarta",resumenForm.getComentario());//
