...
public class PresupuestoAction extends Action{
	...
	//				 //cierre venta
					...
					logger.debug("cierreVenta Exception[" + loge + "]");
	...
	public void cierreVenta(PresupuestoDTO presupuestoDTO,long codCliente, float impTotal, Float montoAbono){
		...
		try{
			if (presupuestoDTO.getNumVenta()>0){
				...
				gaVentasDTO.setIndAbono(montoAbono.floatValue()>0?new Long(1):new Long(0));
			...
			e.printStackTrace(); 
			String loge = StackTraceUtl.getStackTrace(e);
			logger.debug("cierreVenta GeneralException[" + loge + "]");
		}
	}
