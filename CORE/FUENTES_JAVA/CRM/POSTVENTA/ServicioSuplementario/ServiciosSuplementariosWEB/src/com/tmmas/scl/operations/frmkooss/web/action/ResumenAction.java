...
//		Se procede a realizar el cierre de la venta
    	...
    	if (session.getAttribute("textoContratados").toString().trim().length() > 0)
        	textoCarta = textoCarta.concat(session.getAttribute("textoContratados").toString());
    	else
    		textoCarta = textoCarta.concat("\nNo han sido solicitados.");

    	textoCarta = textoCarta.concat("\n\n> DESACTIVACION");
    	if (session.getAttribute("textoNoContratados").toString().trim().length() > 0)
        	textoCarta = textoCarta.concat(session.getAttribute("textoNoContratados").toString());
    	else
    		textoCarta = textoCarta.concat("\nNo han sido solicitados.");
		...
		if(botonSeleccionado!=null&&botonSeleccionado.equals("visualizar")){
			...
			ouputStream.close();
				...
				delegateServiciosSuplementarios.guardaMensajesError(request, "Ha ocurrido un problema al intentar imprimir el reporte.", "Consulte por favor el log del sistema para averiguar la razón del inconveniente.");
