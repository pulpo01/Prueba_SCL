...
// ------------------------------------------------------------------------------------  	
	...
	function buscarCodigo() {
			...
			for (var indice=0; indice < listaServiciosDisp.length; indice++)
				if (listaServiciosDisp[indice][0] == cod)	{
					...
					document.getElementById("col1fila"+filaSel+"TblServDisp").style.backgroundColor=colorNormal;
					// Resalto la nueva
					document.getElementById("col1fila"+indice+"TblServDisp").style.backgroundColor=colorResaltado;
	...
	// ------------------------------------------------------------------------------------------		
	function obtListaServSupDef(codServ){
	 try{
			idServWeb = devuelveIdServicio(codServ);
			CusIntManDWRController.getObtieneListCodServPorDef(codServ, callbackListaServSupDef); 
		}
		catch (e)	{
			alert("método obtListaServSupDef [" + e.name + " - "+e.message + "]");
		}
		
	}
	
// -----------------------------------------------------------------------------------------------------------------------
	function callbackListaServSupDef(data) {
	 try{
		if (data.descripcion!=null){
			var numAbonado= document.getElementById('num_abonado').value;
			var desc=data.descripcion;
			
			
			if (semaforoDWRRetorno==0){
				delay(500);
				verificaServCorreoContr(numAbonado,desc);
			}
		...
		semaforoDWRRetorno=0;
		 ...
		 alert("método callbackListaServSupDef [" + e.name + " - "+e.message + "]");
		}
...
// -----------------------------------------------------------------------------------------------------------------------
function verificaServCorreoContr(numAbonado,codServ){
	 try{
			CusIntManDWRController.getEstadoCorreoServSupl(numAbonado,codServ, callbackListaServCorreoContr); 
		}
		catch (e)	{
			alert("método obtListaServSupDef [" + e.name + " - "+e.message + "]");
		}
...
// -----------------------------------------------------------------------------------------------------------------------
	function callbackListaServCorreoContr(data) {
	var idServ = "";
	...
	try{
		...
		if (data.descripcion!=null){
			semaforoRespAplica=0;
			if (!confirm('La desactivación de este servicio, implica la desactivación del servicio de correo. Desea continuar?'))
			{
							if (idServOrg!=0){
								document.getElementById(idServOrg).checked = false;
							}
							document.getElementById(idServWeb).checked = true;	
			}
			else{
			//document.getElementById(idServOrg).checked = false;
			semaforoRespAplica=1;
			}
		}
		semaforoDWRRetorno=0;
	}		
