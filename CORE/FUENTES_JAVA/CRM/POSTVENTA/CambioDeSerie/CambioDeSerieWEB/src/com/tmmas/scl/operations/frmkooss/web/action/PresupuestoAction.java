...
public class PresupuestoAction extends Action{
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form,
		...
		try{
			...
			if(formPresupuesto.getAccion()!=null&&!((String)session.getAttribute("accionAutDesc")).equalsIgnoreCase("")){
				logger.debug("accionAutDesc         ["+session.getAttribute("accionAutDesc")+"]");
				logger.debug("formPresup.getAccion()["+formPresupuesto.getAccion()+"]");
				...
				if(formPresupuesto.getAccion().equalsIgnoreCase("Siguiente")){
					...
					cierreVenta(presup,sessionData.getCliente().getCodCliente(),Float.parseFloat(impTotal), formPresupuesto.getMontoAbono(),(String)session.getAttribute("numSecuencia"));
				...
				logger.debug("objetoSesion.setNumeroTransaccionVenta numSecuencia["+numSecuencia+"]");
                ...
                objetoSesion.setNombreUsuario(sessionData.getUsuario()); // COL 94740|25-06-2009|SAQL

				retValConst.setObjetoSesion(objetoSesion);
				
				//registra cargos y devuelve presupuesto
				logger.debug("parametrosRegistrarCargos:antes");
				resultadoRegistroCargos = delegate.parametrosRegistrarCargos(retValConst);
				logger.debug("parametrosRegistrarCargos:despues");
				...
				CambioDeSerieForm cambioSerieForm = (CambioDeSerieForm)session.getAttribute("CambioDeSerieForm");
				formPresupuesto.setRbCiclo(cambioSerieForm.getAbonoDiferido());
				...
				if(cargosForm!=null){
					...
					if(tablaCargosDTO!=null){
						...
						totalesPresupuesto[0] = Util.formatearNumeroMoneda(String.valueOf(ImporteBase),numDecimales);
						totalesPresupuesto[1] = Util.formatearNumeroMoneda(String.valueOf(Impuestos),numDecimales);
						totalesPresupuesto[2] = Util.formatearNumeroMoneda(String.valueOf(-ImporteDescuentos),numDecimales); 
						totalesPresupuesto[3] = Util.formatearNumeroMoneda(String.valueOf(ImporteTotal),numDecimales);
	...
	//				 //cierre venta
					...
					cierreVenta(presup,sessionData.getCliente().getCodCliente(),ImporteTotal, formPresupuesto.getMontoAbono(), String.valueOf(numSecuencia));
					
				}
				
			    }catch(Exception e){
			    	textoMensajeRestricciones="Error al Registrar Cargos o Prebilling";
					delegate.guardaMensajesError(request, "Error en el Módulo de Cargos o Prebilling", textoMensajeRestricciones);
