NOMBRE=FPCargaUltPago

include $(FA_ENV_PRECOM)

OBJ_EXTERNO=$(GE_LIB_PATH)/geora.o $(GE_LIB_PATH)/rutinasgen.o $(GE_LIB_PATH)/GenFA.o $(GE_LIB_PATH)/errores.o $(GE_LIB_PATH)/ORAcarga.o $(GE_LIB_PATH)/trazafact.o $(GE_LIB_PATH)/FaORA.o $(GE_LIB_PATH)/New_Interfact.o 

EXE=$(XPF_EXE)/

APLICACION=$(EXE)$(NOMBRE)
OBJETO_LOCAL=$(O)$(NOMBRE).o
INTERMEDIO_C=$(C)$(NOMBRE).c
FUENTE_PC=$(PC)$(NOMBRE).pc
CABECERA_H=$(H)$(NOMBRE).h

all: $(APLICACION)

$(APLICACION) : $(OBJETO_LOCAL) $(OBJ_EXTERNO)
	$(CC) $(CFLAGS) -o $(APLICACION) $(OBJETO_LOCAL) $(OBJ_EXTERNO) $(PROLDLIBS)

$(OBJETO_LOCAL) : $(INTERMEDIO_C)
	$(CC) -o $(OBJETO_LOCAL) $(CFLAGS) $(FA_INCLUDE) -c $(INTERMEDIO_C)

$(INTERMEDIO_C) : $(FUENTE_PC)
	$(PROC) $(FA_PROCFLAGS) userid=$(USERPASS) iname=$(FUENTE_PC) oname=$(INTERMEDIO_C)

debug: clean
	$(PROC) $(FA_PROCFLAGS) userid=$(USERPASS) iname=$(FUENTE_PC) oname=$(INTERMEDIO_C)
	$(CC) -o $(OBJETO_LOCAL) $(GFLAG) $(OPTIMIZEG) $(CDEBUG) $(CCFLAGS) $(PFLAGS) $(SHARED_CFLAG) $(USRFLAGS) $(FA_INCLUDE) -c $(INTERMEDIO_C)
	$(CC) $(GFLAG) $(OPTIMIZEG) $(CDEBUG) $(CCFLAGS) $(PFLAGS) $(SHARED_CFLAG) $(USRFLAGS) -o $(APLICACION) $(OBJETO_LOCAL) $(OBJ_EXTERNO) $(PROLDLIBS) 

clean:
	$(RMF) $(INTERMEDIO_C)
	$(RMF) $(OBJETO_LOCAL)

cleanall: clean
	$(RMF) $(APLICACION)
