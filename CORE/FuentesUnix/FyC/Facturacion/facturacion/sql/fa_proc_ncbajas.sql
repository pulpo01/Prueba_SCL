...
/*  piCodCiclFact:	Ciclo de Facturación del documento a rebajar.    */
/* ----------------------------------------------------------------- */
...
/* ----------------------------------------------------------------- */
	...
	iErr1	NUMBER(2) := 1;	/* NO EXISTE FACTURACION CICLO.          */
	iErr2	NUMBER(2) := 2;	/* ERROR GRAVE. SE ABORTA LA EJECUCION.  */
/* ----------------------------------------------------------------- */
...
/* ----------------------------------------------------------------- */
	...
	iCodConcepto	FA_HISTCONC.COD_CONCEPTO%TYPE;
	iImpConcepto	FA_HISTCONC.IMP_CONCEPTO%TYPE;
	iColumna		FA_HISTCONC.COLUMNA%TYPE;
...
/* ----------------------------------------------------------------- */
	...
	EXCEPTION
		WHEN NO_DATA_FOUND THEN
		    iDesError :='NO EXISTE FACTURA O DOCUMENTO DE CICLO.';
			iCodError := iErr1;
			RAISE ERROR_PROCESO;
	END;
/* ----------------------------------------------------------------- */
...
/* -- Calcula el factor usado para calcular el monto por el cual se hará la NC */
...
/* ----------------------------------------------------------------- */
	...
	END IF;
/* ----------------------------------------------------------------- */
/* Actualiza los montos sobre los que se aplicará la rebaja.         */
/* y aplica la rebaja, en función del factor.                        */
/* ----------------------------------------------------------------- */
	iMtoNC := 0;
	FOR i IN 0..stConc_Ind LOOP
...
/* ----------------------------------------------------------------- */
	...
	EXCEPTION
		WHEN OTHERS THEN
			iCodError := iErr0;
	END;
	IF iCodRetorno != 0 AND szDesCadena = '' THEN
		iDesError :='ERROR EN EJECUCION DE STORED PROCEDURE.';
		iCodError := iErr2;
		RAISE ERROR_PROCESO;
	END IF;
	iCodError := iErr0;
