...
static unsigned int sqlctx = 27608771;
...
{12,4130,2,0,0,
5,0,0,1,0,0,27,89,0,0,4,4,0,1,0,1,5,0,0,1,5,0,0,1,10,0,0,1,10,0,0,
36,0,0,2,125,0,4,93,0,0,4,3,0,1,0,2,5,0,0,1,5,0,0,1,5,0,0,1,5,0,0,
};
...

{
 ...
 sqlstm.sqladtp = &sqladt;
 sqlstm.sqltdsp = &sqltds;
 sqlstm.stmt = "select flg_ejecucion into :b0  from sch_script where ((trim(\
cod_proc)=:b1 and trim(cod_subpro)=:b2) and trim(cod_script)=:b3)";
 ...
 sqlstm.sqlety = (unsigned short)256;
 sqlstm.occurs = (unsigned int  )0;
 sqlstm.sqhstv[0] = (unsigned char  *)flag_ejec;
