...
{
    22,
    "./pc/AsigEJecCobCen.pc"
};


static unsigned int sqlctx = 220751291;
...
TIDAD=B.COD_ENTIDAD) and C.TIP_ENTIDAD=:b1)           ";
...
{12,4138,1,0,0,
5,0,0,1,157,0,4,95,0,0,3,2,0,1,0,2,97,0,0,1,3,0,0,1,3,0,0,
32,0,0,2,208,0,9,132,0,0,2,2,0,1,0,1,97,0,0,1,97,0,0,
55,0,0,2,0,0,13,140,0,0,2,0,0,1,0,2,97,0,0,2,3,0,0,
78,0,0,2,0,0,15,158,0,0,0,0,0,1,0,
93,0,0,3,59,0,5,172,0,0,2,2,0,1,0,1,97,0,0,1,3,0,0,
116,0,0,4,249,0,4,195,0,0,4,3,0,1,0,2,3,0,0,1,97,0,0,1,3,0,0,1,97,0,0,
147,0,0,5,59,0,5,231,0,0,2,2,0,1,0,1,97,0,0,1,3,0,0,
};
...
/*
    Tipo        :  ACCION
    Nombre      :  AsigEJecCobCen.pc ("AEPLA"->szfnAsigEJecCobCen)
    Descripcion :  ASIGna un EJECutivo de COBranza por CEntral ( Plataforma )
...
/* ============================================================================= */
char  *szfnAsigEJecCobCen(FILE **ArchLog, long lCliente, sql_context ctxCont )
{
...
char modulo[] = "szfnAsigEJecCobCen";
...
struct sqlca sqlca;
   ...
   if (ifnRecuperaGedParametro(&pfLog, "ENTIDAD_GESTION_COB", "CO", szValParametro, CXX )!=0) 
	...
	if( !strcmp( szValParametro, "C" ) )	/* la cobranza esta dirigida al cliente */
	{
	    ...
	    iRet = ifnAsigEjecCliente(&pfLog, lCliente, "P", CXX );
	    if ( iRet < 0 )
	...
	SELECT A.COD_COMUNA 
	 ...
	 WHERE B.COD_CLIENTE = :lhCodCliente
	   AND B.COD_TIPDIRECCION = :ihValor_tres
...
CCLI B where ((B.COD_CLIENTE=:b1 and B.COD_TIPDIRECCION=:b2) and A.COD_DIRECCI\
ON=B.COD_DIRECCION)";
...

{
    ...
    sqlstm.sqadto[0] = (unsigned short )0;
    sqlstm.sqtdso[0] = (unsigned short )0;
    sqlstm.sqhstv[1] = (unsigned char  *)szhLetra_P;
...
.COD_CLIENTE=A.COD_CLIENTE) and B.COD_TIPDIRECCION=:b2) and C.COD_DIRECCION=B.\
COD_DIRECCION) and C.COD_COMUNA=:b3)";
    sqlstm.iters = (unsigned int  )1;
    ...
    sqlcxt(&CXX, &sqlctx, &sqlstm, &sqlfpn);
			...
			for( i = 0; i < lTotalRows; i++ ) /* procesar cada agente (elegir cual asignar) */
			{
				...
				ifnTrazaHilos( modulo,&pfLog, "i[%ld] COD_AGENTE => [%s] fPorcentajeDefinido => [%f] fPorcentajeActual => [%f].",
