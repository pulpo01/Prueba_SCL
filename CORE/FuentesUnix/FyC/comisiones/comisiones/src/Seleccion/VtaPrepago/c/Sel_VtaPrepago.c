...
{
    22,
    "./pc/Sel_VtaPrepago.pc"
};


static unsigned int sqlctx = 221814051;
...
extern void sqliem(/*_ char *, int * _*/);
 ...
 static char *sq0001 = 
"select distinct VEN1.COD_VENDEDOR ,NVL(ABO.COD_VENDEALER,0) ,ABO.NUM_VENTA ,\
TO_CHAR(ABO.FEC_ACTIVACION,'DD-MM-YYYY HH24:MI:SS') ,ABO.COD_CLIENTE ,VTAS.COD\
_OFICINA ,CLIE.NUM_IDENT ,ABO.NUM_ABONADO ,ABO.NUM_CELULAR ,ABO.NUM_SERIE ,ABO\
.IND_PROCEQUI  from VE_REDVENTAS_TD RVEN ,VE_VENDEDORES VEN1 ,GA_ABOAMIST ABO \
,GE_CLIENTES CLIE ,GA_VENTAS VTAS where (((((((RVEN.COD_TIPORED=:b0 and RVEN.C\
OD_VENDEDOR=VEN1.COD_VENDEDOR) and VEN1.COD_TIPCOMIS=:b1) and VEN1.COD_VENDEDO\
R=ABO.COD_VENDEDOR) and ABO.IND_PROCEQUI='I') and ABO.FEC_ACTIVACION between T\
O_DATE(:b2,'DD-MM-YYYY') and TO_DATE(:b3,'DD-MM-YYYY')) and ABO.NUM_VENTA=VTAS\
.NUM_VENTA) and ABO.COD_CLIENTE=CLIE.COD_CLIENTE)           ";
...
{12,4128,2,27,0,
5,0,0,1,682,0,9,183,0,0,4,4,0,1,0,1,3,0,0,1,97,0,0,1,97,0,0,1,97,0,0,
36,0,0,1,0,0,13,187,0,0,11,0,0,1,0,2,3,0,0,2,3,0,0,2,3,0,0,2,97,0,0,2,3,0,0,2,
97,0,0,2,97,0,0,2,3,0,0,2,3,0,0,2,97,0,0,2,97,0,0,
95,0,0,1,0,0,15,231,0,0,0,0,0,1,0,
110,0,0,2,410,0,4,262,0,0,4,2,0,1,0,2,3,0,0,2,3,0,0,1,3,0,0,1,97,0,0,
141,0,0,3,54,0,4,357,0,0,1,0,0,1,0,2,3,0,0,
160,0,0,4,404,0,3,380,0,0,19,19,0,1,0,1,3,0,0,1,97,0,0,1,3,0,0,1,3,0,0,1,3,0,0,
1,97,0,0,1,97,0,0,1,3,0,0,1,97,0,0,1,3,0,0,1,3,0,0,1,97,0,0,1,3,0,0,1,3,0,0,1,
3,0,0,1,97,0,0,1,97,0,0,1,3,0,0,1,97,0,0,
251,0,0,5,48,0,1,561,0,0,0,0,0,1,0,
266,0,0,6,0,0,30,607,0,0,0,0,0,1,0,
};
...

{
         ...
         sqlstm.sqhstv[10] = (unsigned char  *)chIndProcequi;
...

{
            ...
            sqlstm.sqladtp = &sqladt;
            sqlstm.sqltdsp = &sqltds;
            sqlstm.stmt = "select FABR.COD_FABRICANTE ,EQUI1.COD_ARTICULO in\
to :b0,:b1  from AL_FABRICANTES FABR ,AL_ARTICULOS ARTI ,GA_EQUIPABOSER EQUI1 \
where ((((EQUI1.NUM_ABONADO=:b2 and EQUI1.NUM_SERIE=:b3) and EQUI1.FEC_ALTA=(s\
elect max(R.FEC_ALTA)  from GA_EQUIPABOSER R where (R.NUM_ABONADO=EQUI1.NUM_AB\
ONADO and R.NUM_SERIE=EQUI1.NUM_SERIE))) and EQUI1.COD_ARTICULO=ARTI.COD_ARTIC\
ULO) and ARTI.COD_FABRICANTE=FABR.COD_FABRICANTE)";
            sqlstm.iters = (unsigned int  )1;
...

{
         ...
         sqlstm.sqladtp = &sqladt;
         sqlstm.sqltdsp = &sqltds;
         sqlstm.stmt = "insert into CMT_HABIL_PREPAGO (NUM_GENERAL,COD_TIPCO\
MIS,COD_COMISIONISTA,COD_AGENCIA,COD_VEND_FINAL,COD_OFICINA,FEC_HABILITACION,C\
OD_CLIENTE,NUM_IDENT,NUM_ABONADO,NUM_CELULAR,NUM_SERIE,COD_ARTICULO,COD_FABRIC\
ANTE,COD_PERIODO,ID_PERIODO,IND_PROCEQUI,COD_TIPORED,COD_TIPVENDEDOR) values (\
:b0,:b1,:b2,:b3,:b4,:b5,TO_DATE(:b6,'DD-MM-YYYY HH24:MI:SS'),:b7,:b8,:b9,:b10,\
:b11,:b12,:b13,:b14,:b15,:b16,:b17,:b18)";
         ...
         sqlstm.sqadto[3] = (unsigned short )0;
         sqlstm.sqtdso[3] = (unsigned short )0;
         sqlstm.sqhstv[4] = (unsigned char  *)&lhCodVendDealer;
         sqlstm.sqhstl[4] = (unsigned long )sizeof(long);
         ...
         sqlstm.sqhstv[14] = (unsigned char  *)&lhCodPeriodo;
         ...
         sqlstm.sqhstv[15] = (unsigned char  *)szhIdPeriodo;
