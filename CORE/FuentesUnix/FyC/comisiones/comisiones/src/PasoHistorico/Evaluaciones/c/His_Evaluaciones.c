...
{
    24,
    "./pc/His_Evaluaciones.pc"
};


static unsigned int sqlctx = 885085059;
...
{12,4128,2,27,0,
5,0,0,1,490,0,3,48,0,0,1,1,0,1,0,1,97,0,0,
24,0,0,2,51,0,2,115,0,0,1,1,0,1,0,1,97,0,0,
43,0,0,3,48,0,1,240,0,0,0,0,0,1,0,
58,0,0,4,0,0,30,299,0,0,0,0,0,1,0,
};


/***************************************************************************/
/* Programa encargado de generar registros historicos de tabla Evaluaciones*/
/*-------------------------------------------------------------------------*/
...
int  iCargaTablasHist()
{
	...
	/* EXEC SQL
	    INSERT INTO CM_EVALUACIONES_TH
                (NUM_GENERAL, 
                 NUM_SOLICITUD, 
                 FEC_SOLICITUD, 
                 IND_TIPO_SOLICITUD, 
...

{
 ...
 sqlstm.stmt = "insert into CM_EVALUACIONES_TH (NUM_GENERAL,NUM_SOLICITUD,FE\
C_SOLICITUD,IND_TIPO_SOLICITUD,COD_ESTADO,NUM_IDENT,NOM_CLIENTE,COD_TIPCOMIS,C\
OD_COMISIONISTA,NUM_VENTA,COD_PERIODO,ID_PERIODO,COD_TIPORED,COD_TIPVENDEDOR,C\
OD_VENDEDOR)(select NUM_GENERAL ,NUM_SOLICITUD ,FEC_SOLICITUD ,IND_TIPO_SOLICI\
TUD ,COD_ESTADO ,NUM_IDENT ,NOM_CLIENTE ,COD_TIPCOMIS ,COD_COMISIONISTA ,NUM_V\
ENTA ,COD_PERIODO ,ID_PERIODO ,COD_TIPORED ,COD_TIPVENDEDOR ,COD_VENDEDOR  fro\
m CMT_EVALUACIONES where ID_PERIODO=:b0)";
 ...
 if (sqlca.sqlcode < 0) vSqlError();
    ...
    {
         fprintf(pfLog , "\n[iCargaTablasHist] Error al insertar en CM_EVALUACIONES_TH ORA[%d]\n",sqlca.sqlcode);
         fprintf(stderr, "\n[iCargaTablasHist] Error al insertar en CM_EVALUACIONES_TH ORA[%d]\n",sqlca.sqlcode);
         return FALSE;
    }
    
    strcpy(szNomTablaProceso, "CMT_EVALUACIONES");
    strcpy(szNomTablaHistorica, "CM_EVALUACIONES_TH");
...
int iBorraTablasTo()
{
    ...
    /* EXEC SQL DELETE FROM CMT_EVALUACIONES WHERE ID_PERIODO = :szhIdPeriodo; */ 

{
    ...
    if ( sqlca.sqlcode != SQLOK && sqlca.sqlcode != SQLNOTFOUND)
    {
         fprintf(pfLog,  "\n [iBorraTablasTo] Error al borrar CMT_EVALUACIONES ORA[%d]\n",sqlca.sqlcode);
         fprintf(stderr,  "\n [iBorraTablasTo] Error al borrar CMT_EVALUACIONES ORA[%d]\n",sqlca.sqlcode);
         return FALSE;
    }   
...
/*---------------------------------------------------------------------------*/
	...
	fprintf(pfLog,  "Registros pasados a historico Evaluaciones  : [%d]\n"  , stStatusProc.lRegInsertos); 

	fprintf(stderr, "Estadistica del proceso\n");
	fprintf(stderr,"------------------------\n");
	fprintf(stderr, "Segundos Reales Utilizados                  : [%d]\n\n", stStatusProc.lSegProceso);
	fprintf(stderr, "Registros pasados a historico Evaluaciones  : [%d]\n"  , stStatusProc.lRegInsertos); 
