...
static unsigned int sqlctx = 6912963;
...
{12,4128,2,27,0,
5,0,0,1,777,0,3,50,0,0,1,1,0,1,0,1,97,0,0,
24,0,0,2,52,0,2,136,0,0,1,1,0,1,0,1,97,0,0,
43,0,0,3,48,0,1,263,0,0,0,0,0,1,0,
58,0,0,4,0,0,30,323,0,0,0,0,0,1,0,
};


/************************************************************************/
/* Programa encargado de generar registros historicos de tablas de BAJAS*/
/*----------------------------------------------------------------------*/
...
int  iCargaTablasHist()
{
	...
	/* EXEC SQL
    	INSERT INTO CM_BAJAS_CELULAR_TH
                (NUM_GENERAL, 
                 COD_TIPCOMIS, 
                 COD_COMISIONISTA, 
                 COD_AGENCIA, 
...

{
 ...
 sqlstm.stmt = "insert into CM_BAJAS_CELULAR_TH (NUM_GENERAL,COD_TIPCOMIS,CO\
D_COMISIONISTA,COD_AGENCIA,COD_CLIENTE,NUM_VENTA,COD_VEND_FINAL,NUM_IDENT,COD_\
CATEGCLIENTE,COD_PLANTARIF,NUM_CONTRATO,NUM_ABONADO,NUM_CELULAR,FEC_VENTA,FEC_\
RECEPCION,FEC_ACEPTACION,FEC_BAJA,COD_CAUSABAJA,NUM_SERIE,COD_ARTICULO,COD_FAB\
RICANTE,IND_PROCEQUI,COD_PERIODO,ID_PERIODO,COD_TIPORED,COD_TIPVENDEDOR)(selec\
t NUM_GENERAL ,COD_TIPCOMIS ,COD_COMISIONISTA ,COD_AGENCIA ,COD_CLIENTE ,NUM_V\
ENTA ,COD_VEND_FINAL ,NUM_IDENT ,COD_CATEGCLIENTE ,COD_PLANTARIF ,NUM_CONTRATO\
 ,NUM_ABONADO ,NUM_CELULAR ,FEC_VENTA ,FEC_RECEPCION ,FEC_ACEPTACION ,FEC_BAJA\
 ,COD_CAUSABAJA ,NUM_SERIE ,COD_ARTICULO ,COD_FABRICANTE ,IND_PROCEQUI ,COD_PE\
RIODO ,ID_PERIODO ,COD_TIPORED ,COD_TIPVENDEDOR  from CMT_BAJAS_CELULAR where \
ID_PERIODO=:b0)";
 ...
 if (sqlca.sqlcode < 0) vSqlError();
    ...
    {
         fprintf(pfLog , "\n[iCargaTablasHist] Error al insertar en CM_BAJAS_CELULAR_TH ORA[%d]\n",sqlca.sqlcode);
         fprintf(stderr, "\n[iCargaTablasHist] Error al insertar en CM_BAJAS_CELULAR_TH ORA[%d]\n",sqlca.sqlcode);
         return FALSE;
    }
    strcpy(szNomTablaProceso, "CMT_BAJAS_CELULAR");
    strcpy(szNomTablaHistorica, "CM_BAJAS_CELULAR_TH");
...
int iBorraTablasTo()
{
    ...
    /* EXEC SQL DELETE FROM CMT_BAJAS_CELULAR WHERE ID_PERIODO = :szhIdPeriodo; */ 

{
    ...
    if ( sqlca.sqlcode != SQLOK )
    {
         fprintf(pfLog , "\n[iBorraTablasTo]Error al borrar CMT_BAJAS_CELULAR ORA[%d]\n",sqlca.sqlcode);
         fprintf(stderr, "\n[iBorraTablasTo]Error al borrar CMT_BAJAS_CELULAR ORA[%d]\n",sqlca.sqlcode);
         return FALSE;
    }
...
/*---------------------------------------------------------------------------------------------*/
    vFechaHora();
	fprintf(pfLog, "[%s]\n", (char *)pszGetDateLog());
	fprintf(pfLog ,"\n(iCargaTablasHist) Carga Tabla Historica.\n");
	fprintf(stderr,"\n(iCargaTablasHist) Carga Tabla Historica.\n");
...
/*---------------------------------------------------------------------------*/
	...
	fprintf(pfLog,  "Registros pasados a historico Bajas   : [%d]\n", stStatusProc.lRegInsertos);   

	fprintf(stderr, "Estadistica del proceso\n");
	fprintf(stderr,"------------------------\n");
	fprintf(stderr, "Segundos Reales Utilizados            : [%d]\n\n", stStatusProc.lSegProceso);
	fprintf(stderr, "Registros pasados a historico Bajas   : [%d]\n", stStatusProc.lRegInsertos);   
